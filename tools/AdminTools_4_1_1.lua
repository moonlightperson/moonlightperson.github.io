...
local colors               = imgui.CreateTextureFromFile(getGameDirectory() ..
	"\\moonloader\\AdminToolsGromov\\color.png")
local authors              = imgui.CreateTextureFromFile(getGameDirectory() ..
	"\\moonloader\\AdminToolsGromov\\authors.png")
...
local font                 = renderCreateFont(Tahoma, 13, 13)
...
local acceptforma          = false
local ffi                  = require 'ffi'
local boost                = 0.02
local alladm               = false
local givepun              = false
local counter              = {}
local arena_amount         = imgui.ImBuffer(256)
local arena_time           = imgui.ImBuffer(256)
local arena_type           = imgui.ImInt(0)
local arena_nameacs        = imgui.ImBuffer(256)
local arena_numrub         = imgui.ImBuffer(256)
local arena_numchas        = imgui.ImBuffer(256)
local arena_numcoins       = imgui.ImBuffer(256)
local arena_numbtc         = imgui.ImBuffer(256)
local arena_acs            = imgui.ImBuffer(256)
local arena_people         = imgui.ImBuffer(256)
local view                 = { u8 "Åâðî", u8 "Áèòêîèíû", u8 "ÀÇ Ðóáëè", u8 "AZ Coins" }
local arena_code           = {}
local ctime                = imgui.ImInt(10)
local actime               = imgui.ImInt(1)
...
local cfg                  = inicfg.load({
	main              = {
		...
		nead               = 60,
		mdm                = 5,
		sendgoodplay       = tostring(u8 '/ao Àäìèíèñòðàöèÿ Arizona RP æåëàåò Âàì ïðèÿòíîé èãðû íà ïðîñòîðàõ íàøåãî ñåðâåðà!'),
		cmdHouse           = 'ohome',
		cmdBusiness        = 'obiz',
		nameaudioforform   = 'form.mp3',
		adminrab           = tostring(u8 'Àäìèíû ðàáîòàåì! Îòâå÷àåì íà ðåïîðò, ïðîâîäèì ñëåòû, êàï÷è.'),
		infopunish         = tostring(u8 'Åñëè âû íå ñîãëàñíû ñ íàêàçàíèåì - îáðàòèòåñü íà ôîðóì íàøåãî ñåðâåðà.'),
		infopunish1        = tostring(u8 'Ññûëî÷êà íà íàø ôîðóì - *** ññûëêà *** | Ïðèÿòíîé èãðû!'),
		kapchas            = '',
		present            = '',
		formis             = '',
		akkpass            = '',
		...
		give_nrg_report    = 'Óâàæàåìûé {nick_rep}, âûäàë Âàì ÍÐÃ!',
		slezhka_author     = 'Óâàæàåìûé {nick_rep}, ñïåøó íà ïîìîùü!',
		slezhka_narush     = 'Óâàæàåìûé {nick_rep}, âñòóïàþ â ñëåæêó çà íàðóøèòåëåì!',
		give_license       = 'Óâàæàåìûé {nick_rep}, âûäàë Âàì ëèöåíçèè!',
		spawn_player       = 'Óâàæàåìûé {nick_rep}, çàñïàâíèë Âàñ!',
		health_player      = 'Óâàæàåìûé {nick_rep}, âûäàë Âàì ÕÏ!',
		pereslat_adm       = 'Óâàæàåìûé {nick_rep}, ïåðåäàë âàø ðåïîðò àäìèíèñòðàöèè!',
	...
	autoopra          = {
		...
		house = false,
		uved = false,
		myreasonhouse = 'îïðà äîì ¹{idhouse}',
		myreasonbiz = 'îïðà áèç ¹{idbiz}',
		settingbiz = true,
		settinghouse = true
	},
	reportname        = {},
	reportotvet       = {},
	reportstyle       = {}
}, "AdminToolsGromov\\AdminTools.ini")
...
local posX123               = imgui.ImInt(cfg.pos.x)
local posY123               = imgui.ImInt(cfg.pos.y)
local posX999               = imgui.ImInt(cfg.checkerPosition.x)
local posY999               = imgui.ImInt(cfg.checkerPosition.y)
local posX456               = imgui.ImInt(cfg.pos.x3)
local posY456               = imgui.ImInt(cfg.pos.y3)

local posX228               = imgui.ImInt(cfg.pos.x4)
local posY228               = imgui.ImInt(cfg.pos.y4)
local posX333               = imgui.ImInt(cfg.pos.x5)
local posY333               = imgui.ImInt(cfg.pos.y5)

local fonts1                = {
	circle = renderCreateFont("Arial", 11, FCR_BOLD + FCR_SHADOW)
}
local popupslezhka          = imgui.ImBool(true)
local moonimgui_text_buffer = imgui.ImBuffer('', 256)
local show_checkip2         = false
local show_checkip3         = false
...
local selector_list = {
	fa.ICON_FA_USER_COG .. u8 ' Îñíîâíîå',
	fa.ICON_FA_PALETTE .. u8 ' Èçìåíåíèå öâåòîâ',
	fa.ICON_FA_KEYBOARD .. u8 ' Íàçíà÷åíèå êíîïîê',
	fa.ICON_FA_BAN .. u8 ' Âûäà÷à íàêàçàíèé',
	fa.ICON_FA_ROBOT .. u8 ' Ñëåæêà [Ðåêîí]',
	fa.ICON_FA_DESKTOP .. u8 ' ×åêåðû',
	fa.ICON_FA_SKULL_CROSSBONES .. u8 ' Ñïèñîê óáèéñòâ',
	fa.ICON_FA_CLIPBOARD .. u8 ' Áûñòðûå êîìàíäû',
	fa.ICON_FA_HANDSHAKE .. u8 ' Ôîðìû ñ àäì. ÷àòà',
	fa.ICON_FA_LIGHTBULB .. u8 ' Ðåïîðò [îòâåòû]',
	fa.ICON_FA_CAMERA .. u8 ' Àâòîîïðîâåðæåíèå',
	fa.ICON_FA_SKULL .. u8 ' Òðåéñåð ïóëü',
	fa.ICON_FA_REGISTERED .. u8 ' Ôóíêöèè äëÿ Àðåíû',
	fa.ICON_FA_INFO_CIRCLE .. u8 ' Àâòîð ñêðèïòà'
}
...
fractions = {
	{ u8 'Ïîëèöèÿ ËÑ', 1543.4702, -1675.3417, 13.5463, 269.6526 },
	{ u8 'Îáëàñòíàÿ ïîëèöèÿ', 634.2316, -572.0031, 16.3359, 89.4022 },
	{ u8 'ÔÁÐ', -2452.8354, 503.9023, 30.0814, 90.7232 },
	{ u8 'Ïîëèöèÿ ÑÔ', -1605.5938, 719.8052, 11.9767, 181.8999 },
	{ u8 'Áîëüíèöà ËÑ', 1179.0513, -1323.3345, 14.1503, 89.0809 },
	{ u8 'Ïðàâèòåëüñòâî', 1495.1583, -1284.2275, 14.5310, 0.2762 },
	{ u8 'ÒÑÐ ËÂ', -12.3242, 1880.9875, 17.6982, 359.7662 },
	{ u8 'Áîëüíèöà ÑÔ', -2668.9612, 635.8420, 14.4531, 358.9167 },
	{ u8 'Àâòîøêîëà', -2040.8459, -84.7864, 35.4024, 240.2197 },
	{ u8 'ÑÌÈ ËÑ', 1654.0095, -1657.4438, 22.5156, 3.0646 },
	...
	{ u8 'Ðóññêàÿ ìàôèÿ', 941.4848, 1730.9229, 8.8516, 88.1602 },
	{ u8 'ßêóäçà', -2460.0029, 135.0315, 35.1719, 133.6574 },
	{ u8 'Ëà Êîñà Íîñòðà', 1461.7233, 2773.3672, 10.8203, 90.0215 },
	{ u8 'Áàéêåðû', -2189.1516, -2351.4141, 30.6250, 41.9276 },
	{ u8 'Àðìèÿ ËÑ', 2731.0066, -2447.8936, 17.5937, 179.3509 },
	{ u8 'Öåíòðàëüíûé áàíê', 1473.1865, -1738.4867, 13.5469, 193.0428 },
	{ u8 'Áîëüíèöà ËÂ', 1606.7427, 1821.3120, 10.8203, 184.813 },
	{ u8 'Ïîëèöèÿ ËÂ', 2287.1809, 2427.3298, 10.8203, 1.6715 },
	{ u8 'ÑÌÈ ËÂ', 2637.6292, 1179.7631, 10.8203, 358.5604 },
	{ u8 'Night Wolves', 2472.9592, -1421.0908, 28.8392, 180.9914 },
	{ u8 'ÑÌÈ ÑÔ', -1941.2107, 463.7813, 35.1719, 181.763 },
	{ u8 'Àðìèÿ ÑÔ', -1369.7841, 498.7395, 11.1953, 86.9582 }
}

locations = {
	{ 'Àâòîñàëîí ËÑ', -484.0909, -557.7690, 25.5634 },
	{ 'Ôåðìà íîâè÷êîâ', -86.9788, 88.2501, 3.1172 },
	{ 'Àâòîñàëîí ËÂ', 947.2376, 2185.3940, 10.8203 },
	{ 'Ñïàâí LS ¹2', 2219.6565, -1146.2649, 25.7819 },
	{ 'Öåíòðàëüíûé ðûíîê', 1125.7731, -1425.4581, 15.7969 },
	{ 'ÂàéíÂóä', 1383.2190, -894.9438, 37.1505 },
	{ 'Èíôîðìàöèîííûé öåíòð', -2238.2156, 575.6461, 35.1719 },
	{ 'Àììî LS', 1363.6930, -1279.8029, 13.5469 },
	{ 'Ñïàâí LS ¹1', 1759.4504, -1900.2614, 13.6300 },
	{ 'Çàâîä äëÿ ÌÏ', 2508.1938, 2767.6387, 10.8203 },
	{ 'Øàõòà', 522.3231, 885.0934, -37.4623 },
	{ 'Àâòîñàëîí ÑÔ', -2663.9944, -1.8084, 4.3267 },
	{ 'Êàçèíî ËÂ', 2030.6858, 998.3827, 10.8203 },
	{ 'Àâòîçàâîä', 1576.6055, 713.8749, 10.8203 },
	{ 'Àâòîñàëîí ÑÔ ïîääåëüíûõ àâòî (íåëåãàëêè)', -2466.1277, 2251.1570, 4.7997 },
	{ 'Ñïàâí LV ¹1', 2844.6235, 1291.8997, 11.3906 },
	{ 'Ñïàâí LV ¹2', -71.9333, 1224.7753, 19.6904 },
	{ 'Ñïàâí SF', -1986.2416, 147.1740, 27.6875 },
	{ 'Àâòîáàçàð', -2136.6956, -750.2112, 32.0234 },
	{ 'Ëåñ', -1649.5634, -2237.9788, 30.1431 },
	{ 'Áàíê LV', 2375.4661, 2316.0024, 8.1406 },
	{ 'Öåíòðàëüíûé Áàíê', 1469.8745, -1743.1466, 13.5469 },
	{ 'Çàáðîøåííûé àýðîïîðò', 376.8152, 2511.9451, 16.5445 },
	{ 'Öåíòð çàíÿòîñòè', 1287.5400, -1275.4004, 13.5407 }
}
...
buffer104 = imgui.ImInt(0)
buffer105 = imgui.ImBuffer('-1', 256)
ffi.cdef [[
struct stKillEntry
{
	char					szKiller[25];
...
local messageChangeMode = {
	[1] = 'èãðîâûõ íàçâàíèé òðàíñïîðòà',
	[2] = 'ñèñòåìíûõ íàçâàíèé ôàéëîâ òðàíñïîðòà',
}
local reportnow = 0
local withID = { {}, {} }
...
changelog = {
	[0] = {
		...
		['ver'] = 'Patch 8.0',
		['info'] = {
			'Äîáàâëåíà îòïðàâêà ðåïîðòà íà Enter',
			'Äîáàâëåíî óäàëåíèå ôëóäà îò /ot',
			'Äîáàâëåíî àâòîìàòè÷åñêîå ïðèíÿòèå ôîðì ñ èíäèâèäóàëüíîé íàñòðîéêîé',
			"Äîáàâëåíà âîçìîæíîñòü èçìåíåíèÿ ñêîðîñòè JetPack'a",
			'Â ðåïîðò äîáàâëåíû ñòîêîâûå êíîïêè "Ôëèï", "ÒÏ ê àâòîðó ðåïîðòà"',
			'Äîáàâëåíà âîçìîæíîñòü ëîããèðîâàíèÿ ïðèíÿòûõ ôîðì â ôàéë AcceptForms',
			'Ñïèñîê êîìàíä ïåðåíåñ¸í â îòäåëüíîå îêíî (/helpa)',
			'Äîáàâëåíî çâóêîâîå îïîâåùåíèå ïðè ïîëó÷åíèè ôîðì ñ èíäèâèäóàëüíîé íàñòðîéêîé äëÿ êàæäîé êîìàíäû',
			'Äîáàâëåí ðåäèçàéí ñïèñêà àäìèíèñòðàöèè (/admins)',
			'Äîáàâëåíî àâòîçàêðûòèå äèàëîãà îò àíòè÷èòà',
			'Äîáàâëåíî àâòîìàòè÷åñêîå ïðèíÿòèå ôîðì ñ èíäèâèäóàëüíîé íàñòðîéêîé äëÿ êàæäîé êîìàíäû',
			'Äîáàâëåíà âîçìîæíîñòü èçìåíåíèÿ çàêðóãëåíèÿ óãëîâ êíîïîê è èíòåðôåéñà',
			'Óáðàíû òåìû "Êðàñíàÿ", "Çåëåíàÿ", äîáàâëåíà "Ò¸ìíî-çåëåíàÿ" òåìà',
			'Äîáàâëåíû êîìàíäû: \n{E78284}/tpm {84A6E7}- òåëåïîðò ïî ìåòêå,\n{E78284}/tpm 1 {84A6E7}- òåëåïîðò ïî ìàðêåðó, \n{E78284}/arep {84A6E7}- ïî÷èíêà òðàíñïîðòà, â êîòîðîì âû ñèäèòå, \n{E78284}/ammo {84A6E7}- ïîïîëíåíèå ñêëàäà àìóíèöèé, \n{E78284}/clr [êîë-âî ñòðîê] {84A6E7}- âèçóàëüíàÿ î÷èñòêà ÷àòà, \n{E78284}/admall {84A6E7}- ñïèñîê âñåé àäìèíèñòðàöèè ñåðâåðà, \n{E78284}/color {84A6E7}- òàáëèöà IDoâ öâåòîâ, \n{E78284}/cal [ïðèìåð] {84A6E7}- êàëüêóëÿòîð, \n{E78284}/crd {84A6E7}- ñêîïèðîâàòü êîîðäèíàòû íà êîòîðûõ âû íàõîäèòåñü, \n{E78284}/rek_off {84A6E7}- îòêëþ÷èòü/âêëþ÷èòü ñîîáùåíèÿ "Ïîäîçðåíèå íà ðåêëàìó", \n{E78284}/ab {84A6E7}- ñîêðàùåíèå êîìàíäû /asellbiz, \n{E78284}/ah {84A6E7}- ñîêðàùåíèå êîìàíäû /asellhouse',
			'Äîáàâëåíî àâòîìàòè÷åñêîå îòêëþ÷åíèå àâòîîïðîâåðæåíèÿ ïðè âûõîäå â AFK',
			'Äîáàâëåíà âîçìîæíîñòü ðåãóëèðîâàíèÿ âðåìåíè äåìîðãàíà ïðè çàïðîñå îïðîâåðæåíèÿ íà ëîâëþ èìóùåñòâà',
			"Äîáàâëåíî îïîâåùåíèå ïðè âêëþ÷åíèè/âûêëþ÷åíèè WallHack'a",
			"Äîáàâëåíû äîïîëíèòåëüíûå êëàâèøè äëÿ ïîëåòà íà AirBrak'e: ëåâûé øèôò - âíèç, ïðîáåë - ââåðõ",
		},
		['patches'] = {
			['show'] = false,
			['info'] = {
				"Èñïðàâëåíà ïðîáëåìà GodMod'a",
				'Èç ðåïîðòà óáðàíû êíîïêè "×àñòûå âîïðîñû", "Óðîâíè ðàáîò"',
				'Èñïðàâëåíà êîäèðîâêà ïðè êîïèðîâàíèè äèñêîðäà ðàçðàáîò÷èêà',
				'Èñïðàâëåíà îòïðàâêà ñîáñòâåííîãî áèíäåðà ¹1 â âèçóàëüíûé ÷àò',
				'Èñïðàâëåí êðàø ñêðèïòà ïðè îòïðàâêå ñîáñòâåííîãî áèíäåðà ¹4 ÷åðåç êîìàíäó /my4',
				'Èñïðàâëåí íåâèäèìûé ñêðîëë-áàð â ìàëèíîâîé òåìå',
				'Èçìåíåí ïðèöíöèï ðàáîòû àâòîîïðîâåðæåíèÿ, òåïåðü ñîîáùåíèÿ î ëîâëå âèäíû â ÷àòå [by kotofeev]',
				'Èñïðàâëåíà ñêîðîñòü AirBrake ïðè ïîëåòå ââåðõ/âíèç (áûëà â 2 ðàçà ìåíüøå íàñòîÿùåé)',
				'Èñïðàâëåíî àííóëèðîâàíèå ñòàòèñòèêè êàæäûå ñóòêè',
			}
		}
	},
	[1] = {
		...
		['ver'] = 'Patch 8.3',
		['info'] = {
			'Òåïåðü ïðè âûáîðå êíîïêè "ÒÏ ê èãðîêó" (â ðåïîðòå) ñêðèïò àâòîìàòè÷åñêè áóäåò îòâå÷àòü åìó, ÷òî âû ïîïðîáóåòå ïîìî÷ü',
		},
		['patches'] = {
			['show'] = false,
			['info'] = {
				'Èñïðàâëåíà êðàø ñêðèïòà ïðè âõîäå â ñëåæêó ïî íèêó (ïðèìåð: /re mishka"',
				'Èñïðàâëåíî åæåäíåâíîå îáíóëåíèå ïóíêòà ñòàòèñòèêè "Îíëàéí çà íåäåëþ"',
				'Èñïðàâëåí êðàø ñêðèïòà, êîãäà ïðè ïåðâîì âõîäå â íîâûé äåíü îí âûëåòàë',
				'Èñïðàâëåíà ïîäñêàçêà â ïóíêòå "Ïåðåõîä ñëåæêè íà > è <" (íå ïîêàçûâàëàñü)',
			}
		}
	},
	[2] = {
		...
		['ver'] = 'Patch 9.0',
		['info'] = {
			'Äîáàâëåíû ñëåäóþùèå ñîôòû: AntiAFK, áåñêîíå÷íûå ïàòðîíû, îòñóòñòâèå ïåðåçàðÿäêè, SpeedHack',
			'Äîáàâëåíà àâòîìàòè÷åñêàÿ ëîâëÿ ðåïîðòà ñ òðåìÿ ñïîñîáàìè ëîâëè: ïî òåêñòó â ÷àòå/ïî íàäïèñè "REPORT ++"/îáû÷íûé ôëóä /ot',
			'Äîáàâëåíî çàòåìíåíèå ïðè îòêðûòèè äîïîëíèòåëüíîãî îêíà â Ò¸ìíî-çåë¸íîé òåìå',
			'Äîáàâëåíà âîçìîæíîñòü âðó÷íóþ çàïðàøèâàòü îïðîâåðæåíèå íà ëîâëþ äîìà/áèçíåñà ñ âîçìîæíîñòüþ èçìåíåíèÿ êîìàíäû',
			'Óáðàíî öåíòðèðîâàíèå òåêñòà â ñòàòèñòèêå',
			'Äîáàâëåíà êîìàíäà /online äëÿ ïðîñìîòðà íåäåëüíîãî îíëàéíà',
			'Äîáàâëåí ïðåäîñìîòð âèäà îêíà ñòàòèñòèêè â ìåíþ åå íàñòðîåê',
			'Òåïåðü ôóíêöèè, ñâÿçàííûå ñ ïåðåêðàøèâàíèåì ñîîáùåíèåì â ÷àòå, íå áóäóò ðàáîòàòü â AFK äëÿ óëó÷øåíèÿ ïðîèçâîäèòåëüíîñòè',
			'Òåïåðü ïðè àâòîìàòè÷åñêîì ïðèíÿòèè ôîðìû êîëè÷åñòâî ïðèíÿòûõ ôîðì â ñòàòèñòèêå áóäåò óâåëè÷èâàòüñÿ',
			'Äîáàâëåíà ðàíäîìíàÿ ãåíåðàöèÿ êàï÷è â /capcha',
			'Óáðàíî ÊÄ ìåæäó îòïðàâêîé ñîîáùåíèé â òóëñå â ñâÿçè ñ åãî óäàëåíèåì íà ñòîðîíå ñåðâåðà',
			'Äîáàâëåí áèíä äëÿ áûñòðîãî îòêðûòèÿ êàðòû',
			'Âîçâðàùåíî óâåäîìëåíèå àäìèíèñòðàòîðó ïðè íåóäà÷íîé âûäà÷å ôîðìû (èãðîê óæå â ìàøèíå/äåìîðãàíå/ìóòå/íå çàáàíåí/ïðàâèëüíàÿ ïîäà÷à ôîðìû)',
			'Äîáàâëåíà ïðîâåðêà íà íàëè÷èå ôàéëà ñî ñïèñêîì êîìàíä/íàâèãàöèè (èñïîëüçóåòñÿ â îêíå ðåïîðòà)',
		},
		['patches'] = {
			['show'] = false,
			['info'] = {
				'(Âîçìîæíî) èñïðàâëåí êðàø ñêðèïòà ïðè îòêðûòèè ðåïîðòà',
				'Èñïðàâëåí áàã ñ ðàçìåðîì ïîäñêàçîê ó ôóíêöèé',
			}
		}
	},
	[3] = {
		...
		['info'] = {
			'Äîáàâëåí êëèêâàðï ',
			'Äîáàâëåíà ôóíêöèÿ ñêðûòèÿ ÈÏ àäðåñîâ ',
			'Äîáàâëåíà âîçìîæíîñòü âûêëþ÷åíèÿ êëàâèøè F5 ',
			'Äîáàâëåíà âîçìîæíîñòü âêëþ÷àòü/âûêëþ÷àòü ïîêàç ÈÄîâ èãðîêîâ â êèëë ëèñòå ',
			'Äîáàâëåíà íåâèäèìêà (èíâèç) ',
			'Äîáàâëåíû ÷åêåð èãðîêîâ, ÷åêåð îòêëþ÷åíèé, ÷åêåð ïîäêëþ÷åíèé, ÷åêåð ðåãèñòðàöèé ñ ãèáêîé íàñòðîéêîé ',
			'Äîáàâëåíû òðåéñåðà ñâîèõ è ÷óæèõ ïóëü ñ ãèáêîé íàñòðîéêîé ',
			'Ìàêñèìàëüíûé êîýôôèöåíò óñêîðåíèÿ ñïèäõàêîì òåïåðü 300 (áûëî 100) ',
			'Äîáàâëåí àâòî÷åê äàííûõ (ÈÏîâ) ïðè ïðîáèòèè /getip (â òåñòîâîì ðåæèìå) ',
			'Âîçâðàùåí ÂÕ íà òðàíñïîðò, òåïåðü áåç áàãîâ, èñïîëüçóéòå /carnames äëÿ íàñòðîéêè ðåæèìà ïîêàçà íàçâàíèÿ àâòî÷åê ',
			'Äîáàâëåíà àâòîðåêëàìà ñ âîçìîæíîñòüþ âûáîðà ðåêëàìíîãî áèíäåðà è èíòåðâàëîì ðåêëàìû ',
			'Äîáàâëåíû âàðíèíãè íà òèìêèëë ',
			'Óáðàíî àíòèAFK ',
			'Óáðàíà âîçìîæíîñòü èçìåíåíèÿ áîëüøèíñòâà öâåòîâ ÷àòà (íàõóé íàäî) ',
			'Óáðàí ìàëèíîâûé ñòèëü ',
			'Äîáàâëåíû ñòèëè: êðàñíûé, ðîçîâûé (ôóëë êîïèÿ äèëëñà) ',
			'Äîáàâëåíà âîçìîæíîñòü èçìåíåíèÿ áèíäîâ: îòêðûòü êàðòó, âêëþ÷èòü/âûêëþ÷èòü àâòîëîâëþ ðåïîðòà, îòêðûòü ìåíþ âûäà÷è íàêàçàíèé, âêëþ÷èòü/âûêëþ÷èòü àèðáðåéê, ïîâûñèòü/ïîíèçèòü ñêîðîñòü àèðáðåéêà, âçàèìîäåéñòâîâàòü ñ èãðîêàìè â ðàäèóñå, òåëåïîðòèðîâàòüñÿ â àäìèí-çîíó ',
			'Ïåðåäåëàí ïóíêò "âûäà÷à íàêàçàíèé" ',
			'Èñïðàâëåíî èçìåíåíèå ïîçèöèè ìåíþ áûñòðûõ íàêàçàíèé ',
			'Êîìàíäà äëÿ àêòèâàöèè îêíà íàñòðîåê èçìåíåíà íà /amenu ',
			'Î÷åíü êðóòî èçìåíåí äèçàéí ïóíêòà "Èíôîðìàöèÿ" ',
			'Èçìåíåíà öâåòîâàÿ ãàììà ñîîáùåíèé îò ñêðèïòà ',
			'Óáðàíû ëèøíèå ïóíêòû ðåäàêòèðîâàíèÿ îòâåòà íà ðåïîðò (áûëî ðåäàêòèðîâàíèå íåñóùåñòâóþùèõ ïóíêòîâ) ',
			'Â "Ãëàâíîå > Îñíîâíîå" îñòàâëåíû êíîïêè òîëüêî äëÿ îòïðàâêè áèíäîâ, îñòàëüíîå âçàèìîäåéñòâèå ÷åðåç êîìàíäû ',
			'Òåïåðü åñëè áèíä êëàâèøè îòñóòñòâóåò, òî âìåñòî"No" ïîêàçûâàåòñÿ "Îòñóòñòâóåò" ',
			'Èçìåíåí äèçàéí ïåðåêëþ÷àòåëÿ ',
			'Èñïðàâëåí êðàø ñêðèïòà ïðè îòïðàâêå íåñóùåñòâóþùåãî ðåïîðòà â àäìèí÷àò (íå òîê ðåïîðòà) ',
			'Äîáàâëåíà âîçìîæíîñòü ïðèíÿòèÿ ôîðìû íà banipoff ',
			'Òåïåðü âûáðàííûé ïóíêò â ìåíþ ñîîòâåòñòâåííî ïîäñâå÷èâàåòñÿ ',
			'Òåïåðü ó äîï. îêîí äëÿ ðåäàêòèðîâàíèÿ áèíäîâ åñòü çàãîëîâêè ',
			'Â ãëàâíîå ìåíþ äîáàâëåíà êíîïêà äëÿ ñîõðàíåíèÿ êîíôèãà ñêðèïòà ',
			'Òåïåðü êîíôèã ñêðèïòà õðàíèòñÿ â ïàïêå ñêðèïòà, à íå â ïàïêå config äëÿ âñåãî ìóíëîàäåðà (ïóòü: moonloader/config/admintoolsgromov/AdminTools.ini) '
		},
		['patches'] = {
			['show'] = false,
			['info'] = {
				"Èñïðàâëåí êðàø ïðè îòêðûòèè ðåïîðòà (òåðü 100 ïðîöåíòîâ)"
			}
		}
	},
	[4] = {
		...
		['ver'] = 'Patch 10.6',
		['info'] = {
			'Òåêñò êëèêâàðïà ïðè íàâåäåíèè íà òðàíñïîðò ïåðåâåäåí íà ðóññêèé',
			'Äîáàâëåíà âîçìîæíîñòü íàñòðîéêè òèïà ïîêàçà íàçâàíèÿ àâòî â ÂÕ íà òðàíñïîðò',
			'Äîáàâëåíû áèíäû äëÿ ïîâûøåíèÿ/ïîíèæåíèÿ ñêîðîñòè ÄæåòÏàêà',
			'Äîáàâëåíû ÷åðíàÿ, ôèîëåòîâàÿ òåìû',
			'Èçìåíåí ñïîñîá øèôðîâàíèÿ ñîäåðæèìåãî ñêðèïòà'
		},
		['patches'] = {
			['show'] = false,
			['info'] = {
				'Èñïðàâëåí êðàø ïðè îòêðûòèè íàñòðîåê íàêàçàíèÿ "ÈÇÏ"',
				'Èñïðàâëåíà àâòîíàñòðîéêà ôîðì ïîä óðîâåíü àäì. (ñêðèïò êðàøèëñÿ åñëè àäìèíêà áûëà 3, 6 óðîâíåé)',
			}
		}
	}
}
...
TeleportPlayerWindow = imgui.ImBool(false)
reconusers_state = imgui.ImBool(false)
ReportWindow = imgui.ImBool(false)
ColorsWindow = imgui.ImBool(false)
tttttt = imgui.ImInt(cfg.main.mylvladmin)
ttttttt = imgui.ImInt(cfg.main.mylvlfd)
...
local function createNewJsonTable(table)
	if not doesFileExist(getWorkingDirectory() .. "\\AdminToolsGromov\\" .. table[2] .. ".json") then
		local f = io.open(getWorkingDirectory() .. "\\AdminToolsGromov\\" .. table[2] .. ".json", 'w+')
		if f then
			f:write(encodeJson(table[1])):close()
		end
	else
		local f = io.open(getWorkingDirectory() .. "\\AdminToolsGromov\\" .. table[2] .. ".json", "r")
		if f then
...
pathOffLog = getWorkingDirectory() .. "\\AdminToolsGromov\\Ëîãè\\AutoOpraLogs.txt"
log_full = {}
...
function main()
	...
	connect = renderCreateFont(cfg.main.ConnectFontChecker, cfg.main.connectSize, cfg.main.connectStyle,
		FCR_BOLD + FCR_BORDER)
	admincheck = renderCreateFont(cfg.main.ConnectFontChecker, cfg.main.connectSize, cfg.main.connectStyle,
		FCR_BOLD + FCR_BORDER)
	reg = renderCreateFont(cfg.main.RegFontChecker, cfg.main.regSize, cfg.main.regStyle, FCR_BOLD + FCR_BORDER)
	fonts = {
		playersChecker = renderCreateFont(cfg.main.fontchecker, cfg.main.sizechecker, cfg.main.stylechecker,
			FCR_BOLD + FCR_SHADOW)
	}
	fonts123 = {
	...
	if thisScript().version ~= lastver then
        ...
        sampAddChatMessage(pinktag..'Âûøëî îáíîâëåíèå äëÿ ñêðèïòà! Âàøà âåðñèÿ: '..thisScript().version..', àêòóàëüíàÿ: '..lastver, -1)
        sampAddChatMessage(pinktag..'Äëÿ óñòàíîâêè îáíîâëåíèÿ èñïîëüçóéòå êîìàíäó /load.update èëè âûáåðèòå íåîáõîäèìûé ïóíêò â âñïëûâàþùåì îêíå.', -1)
		UpdateWindow.v = true
    end
	local _, myid = sampGetPlayerIdByCharHandle(playerPed)
	mynick = sampGetPlayerNickname(myid)
	print(sampGetCurrentServerAddress())
	if not checkServer(sampGetCurrentServerAddress()) and mynick ~= "moonlight" then
		sampAddChatMessage(tag .. 'Ñêðèïò ðàáîòàåò òîëüêî íà ñåðâåðå Arizona Arena!', -1)
		thisScript():unload()
	end
	...
	loadbinders()
	if not doesFileExist("moonloader/AdminToolsGromov/authors.png") then
		sampAddChatMessage(tag .. 'Ðåñóðñ äëÿ ðàáîòû ñêðèïòà íå áûëà íàéäåí, íà÷àëîñü àâòîìàòè÷åñêîå ñêà÷èâàíèå..', -1)
		download_id = downloadUrlToFile(
			'https://drive.google.com/u/0/uc?id=195nPyJYkauJ-kPpAYCNAK9U6amBWZ0PZ&export=download',
			'moonloader/AdminToolsGromov/authors.png', function(id, status, p1, p2)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					sampAddChatMessage(tag .. 'Ñêà÷èâàíèå ðåñóðñà óñïåøíî çàâåðøåíî.', -1)
					authors = imgui.CreateTextureFromFile(getGameDirectory() ..
						"\\moonloader\\AdminToolsGromov\\authors.png")
				end
			end)
	end
	if not doesFileExist("moonloader/AdminToolsGromov/commands.json") then
		sampAddChatMessage(tag .. 'Ðåñóðñ äëÿ ðàáîòû ñêðèïòà íå áûëà íàéäåí, íà÷àëîñü àâòîìàòè÷åñêîå ñêà÷èâàíèå..', -1)
		download_id = downloadUrlToFile(
			'https://drive.google.com/u/0/uc?id=1s2sG8RXexZhXH9yK8eELuY4fQUAvtMRh&export=download',
			'moonloader/AdminToolsGromov/commands.json', function(id, status, p1, p2)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					sampAddChatMessage(tag .. 'Ñêà÷èâàíèå ðåñóðñà óñïåøíî çàâåðøåíî.', -1)
					authors = imgui.CreateTextureFromFile(getGameDirectory() ..
						"\\moonloader\\AdminToolsGromov\\authors.png")
	...
	if not doesFileExist("moonloader/AdminToolsGromov/gps.json") then
		sampAddChatMessage(tag .. 'Ðåñóðñ äëÿ ðàáîòû ñêðèïòà íå áûëà íàéäåí, íà÷àëîñü àâòîìàòè÷åñêîå ñêà÷èâàíèå..', -1)
		download_id = downloadUrlToFile(
			'https://drive.google.com/u/0/uc?id=1oA7BA96qq_jUgTD8RjpYlyrz0yGn4UJi&export=download',
			'moonloader/AdminToolsGromov/gps.json', function(id, status, p1, p2)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					sampAddChatMessage(tag .. 'Ñêà÷èâàíèå ðåñóðñà óñïåøíî çàâåðøåíî.', -1)
					authors = imgui.CreateTextureFromFile(getGameDirectory() ..
						"\\moonloader\\AdminToolsGromov\\authors.png")
	...
	--https://cdn.discordapp.com/attachments/927882105441095720/984023505475629056/KillList.rar
	local withOutID = {
		[1] = {
			"Landstalker", "Bravura", "Buffalo", "Linerunner", "Perrenial", "Sentinel", "Dumper", "Firetruck",
			"Trashmaster", "Stretch", "Manana", "Infernus",
			"Voodoo", "Pony", "Mule", "Cheetah", "Ambulance", "Leviathan", "Moonbeam", "Esperanto", "Taxi", "Washington",
			"Bobcat", "Whoopee", "BF Injection", "Hunter",
			"Premier", "Enforcer", "Securicar", "Banshee", "Predator", "Bus", "Rhino", "Barracks", "Hotknife",
			"Article Trailer", "Previon", "Coach", "Cabbie", "Stallion", "Rumpo",
			"RC Bandit", "Romero", "Packer", "Monster", "Admiral", "Squalo", "Seasparrow", "Pizzaboy", "Tram",
			"Trailer #2", "Turismo", "Speeder", "Reefer", "Tropic", "Flatbed",
			"Yankee", "Caddy", "Solair", "Berkley's RC Van", "Skimmer", "PCJ-600", "Faggio", "Freeway", "RC Baron",
			"RC Raider", "Glendale", "Oceanic", "Sanchez", "Sparrow",
			"Patriot", "Quad", "Coastguard", "Dinghy", "Hermes", "Sabre", "Rustler", "ZR-350", "Walton", "Regina",
			"Comet", "BMX", "Burrito", "Camper", "Marquis", "Baggage",
			"Dozer", "Maverick", "NewsChopper", "Rancher", "FBI Rancher", "Virgo", "Greenwood", "Jetmax", "Hotring",
			"Sandking", "Blista Compact", "Police Maverick",
			"Boxville #1", "Benson", "Mesa", "RC Goblin", "Hotring Racer A", "Hotring Racer B", "Bloodring Banger",
			"Rancher Lure", "SuperGT", "Elegant", "Journey", "Bike",
			"Mountain Bike", "Beagle", "Cropduster", "Stunt", "Tanker", "Roadtrain", "Nebula", "Majestic", "Buccaneer",
			"Shamal", "Hydra", "FCR-900", "NRG-500", "HPV1000",
			"Cement Truck", "Tow Truck", "Fortune", "Cadrona", "FBI Truck", "Willard", "Forklift", "Tractor", "Combine",
			"Feltzer", "Remington", "Slamvan", "Blade", "Freight",
			"Streak", "Vortex", "Vincent", "Bullet", "Clover", "Sadler", "Firetruck", "Hustler", "Intruder", "Primo",
			"Cargobob", "Tampa", "Sunrise", "Merit", "Utility", "Nevada",
			"Yosemite", "Windsor", "Monster A", "Monster B", "Uranus", "Jester", "Sultan", "Stratum", "Elegy",
			"Raindance", "RC Tiger", "Flash", "Tahoma", "Savanna", "Bandito",
			"Freight Flat Trailer", "Streak Trailer", "Kart", "Mower", "Dune", "Sweeper", "Broadway", "Tornado", "AT-400",
			"DFT-30", "Huntley", "Stafford", "BF-400", "News Van",
			"Tug", " Petrol Trailer", "Emperor", "Wayfarer", "Euros", "Hotdog", "Club", "FreightBox", "Trailer",
			"Andromada", "Dodo", "RC Cam", "Launch", "LS:PD Cruiser", "SF:PD Cruiser",
			"LV:PD Cruiser", "Police Ranger", "Picador", "S.W.A.T", "Alpha", "Phoenix", "Glendale Shit", "Sadler Shit",
			"Luggage A", "Luggage B", "Stairs", "Boxville #2", "Tiller",
			"Utility Trailer"
		},

		[2] = {
			'landstal', 'bravura', 'buffalo', 'linerun', 'peren', 'sentinel', 'dumper', 'firetruk', 'trash', 'stretch',
			'manana', 'infernus', 'voodoo', 'pony', 'mule', 'cheetah',
			'ambulan', 'leviathn', 'moonbeam', 'esperant', 'taxi', 'washing', 'bobcat', 'mrwhoop', 'bfinject', 'hunter',
			'premier', 'enforcer', 'securica', 'banshee', 'predator',
			'bus', 'rhino', 'barracks', 'hotknife', 'artict1', 'previon', 'coach', 'cabbie', 'stallion', 'rumpo',
			'rcbandit', 'romero', 'packer', 'monster', 'admiral', 'squalo',
			'seaspar', 'pizzaboy', 'tram', 'artict2', 'turismo', 'speeder', 'reefer', 'tropic', 'flatbed', 'yankee',
			'caddy', 'solair', 'topfun', 'skimmer', 'pcj600', 'faggio',
			'freeway', 'rcbaron', 'rcraider', 'glendale', 'oceanic', 'sanchez', 'sparrow', 'patriot', 'quad', 'coastg',
			'dinghy', 'hermes', 'sabre', 'rustler', 'zr350', 'walton',
			'regina', 'comet', 'bmx', 'burrito', 'camper', 'marquis', 'baggage', 'dozer', 'maverick', 'vcnmav', 'rancher',
			'fbiranch', 'virgo', 'greenwoo', 'jetmax', 'hotring',
			'sandking', 'blistac', 'polmav', 'boxville', 'benson', 'mesa', 'rcgoblin', 'hotrina', 'hotrinb',
			'bloodra/bloodrb', 'rnchlure', 'supergt', 'elegant', 'journey', 'bike',
			'mtbike', 'beagle', 'cropdust', 'stunt', 'petro', 'rdtrain', 'nebula', 'majestic', 'buccanee', 'shamal',
			'hydra', 'fcr900', 'nrg500', 'copbike', 'cement', 'towtruck',
			'fortune', 'cadrona', 'fbitruck', 'willard', 'forklift', 'tractor', 'combine', 'feltzer', 'remingtn',
			'slamvan', 'blade', 'freight', 'streak', 'vortex', 'vincent',
			'bullet', 'clover', 'sadler', 'firela', 'hustler', 'intruder', 'primo', 'cargobob', 'tampa', 'sunrise',
			'merit', 'utility', 'nevada', 'yosemite', 'windsor', 'monstera',
			'monsterb', 'uranus', 'jester', 'sultan', 'stratum', 'elegy', 'raindanc', 'rctiger', 'flash', 'tahoma',
			'savanna', 'bandito', 'freiflat', 'streakc', 'kart', 'mower',
			'duneride', 'sweeper', 'broadway', 'tornado', 'at400', 'dft30', 'huntley', 'stafford', 'bf400', 'newsvan',
			'tug', 'petrotr', 'emperor', 'wayfarer', 'euros', 'hotdog',
			'club', 'freibox', 'artict3', 'androm', 'dodo', 'rccam', 'launch', 'copcarla', 'copcarsf', 'copcarvg',
			'copcarru', 'picador', 'swatvan', 'alpha', 'phoenix', 'glenshit', 'sadlshit',
			...
			if param:find('^%d') and tonumber(param) <= 2 then
				if tonumber(param) ~= tonumber(cfg.Settings.mode) then
					for k, v in pairs(messageChangeMode) do
						if (param + 1) == k then
							sampAddChatMessage(string.format("Âû âêëþ÷èëè îòîáðàæåíèå %s.", v), -1)
							cfg.Settings.mode = (k - 1); inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini"); break
						end
					end
				else
					sampAddChatMessage(
						string.format('Â äàííûé ìîìåíò èòàê âêëþ÷åíî îòîáðàæåíèå %s [{1E90FF}%d{FFFFFF}].',
							messageChangeMode[param + 1], param), -1)
				end
			else
				sampAddChatMessage(
					string.format('Òàêîãî ïàðàìåòðà íå ñóùåñòâóåò! Êîëè÷åñòâî äîñòóïíûõ ïàðàìåòðîâ: {1E90FF}%d{FFFFFF}.',
						#messageChangeMode), -1)
				for k, v in pairs(messageChangeMode) do
					sampAddChatMessage(string.format('[{1E90FF}%d{FFFFFF}] - Îòîáðàæåíèå %s.', (k - 1), v), -1)
				end
			end
		end },
	...
	while true do
		...
		if cfg.main.leaveChecker then
			...
			renderFontDrawTextAlign(leave, "×åêåð âûõîäîâ: ", cfg.leavePosition.x, cfg.leavePosition.y, -1,
				cfg.main.aligntwo)
			for k, v in pairs(leavePlayers) do
				y = y + cfg.main.leaveIndent
				renderFontDrawTextAlign(leave,
					"{" ..
					leavePlayers[k].playerColor ..
					"}[" ..
					leavePlayers[k].timeQuit ..
					"] " ..
					leavePlayers[k].nickname ..
					"[" .. leavePlayers[k].id .. "] îòêëþ÷èëñÿ. Ïðè÷èíà: " .. leavePlayers[k].reason, x, y, -1,
					cfg.main.aligntwo)
		...
		if cfg.main.connectChecker then
			...
			renderFontDrawTextAlign(connect, "×åêåð âõîäîâ: ", cfg.connectPosition.x, cfg.connectPosition.y, -1,
				cfg.main.alignthree)
			for k, v in pairs(connectPlayers) do
				y = y + cfg.main.connectIndent
				renderFontDrawTextAlign(connect,
					"{" ..
					connectPlayers[k].playerColor ..
					"}[" ..
					connectPlayers[k].timeJoin ..
					"] " .. connectPlayers[k].nickname .. "[" .. connectPlayers[k].id .. "] ïîäêëþ÷èëñÿ.", x, y, -1,
					cfg.main.alignthree)
		...
		if cfg.main.regChecker then
			...
			renderFontDrawTextAlign(reg, "×åêåð ðåãèñòðàöèé: ", cfg.RegPosition.x, cfg.RegPosition.y, -1,
				cfg.main.alignfour)
			for k, v in pairs(regPlayers) do
				y = y + cfg.main.regIndent
				renderFontDrawTextAlign(reg,
					"{FFFFFF}[" ..
					regPlayers[k].timeReg ..
					"] " ..
					regPlayers[k].nickname ..
					"[" ..
					regPlayers[k].id ..
					"] çàðåãèñòðèðîâàëñÿ | IP: " .. regPlayers[k].ip1 .. ".xx.xxx." .. regPlayers[k].ip4 .. "", x, y, -1,
					cfg.main.alignfour)
		...
		if isPlayerUsingJetpack(PLAYER_HANDLE) then
			...
			renderFontDrawText(my_font,
				'Èçìåíåíèå ñêîðîñòè: {E78284}' ..
				klavisha .. ' {84A6E7}è {E78284}' .. klavisha1 .. '\n{84A6E7}Òåêóùàÿ ñêîðîñòü: {E78284}' .. boost,
				cx + 500, windowCoordinates[2] - 32, 0xFF84A6E7)
			if isKeysDown(fjspeedup.v) then
				if boost + 0.03 > 0.7 then
					sampAddChatMessage(tag .. 'Ýòî ìàêñèìàëüíàÿ ñêîðîñòü äæåòïàêà!', -1)
				else
					boost = boost + 0.03
				end
			end
			if isKeysDown(fjspeeddown.v) then
				if boost - 0.03 < 0.02 then
					sampAddChatMessage(tag .. 'Ýòî ìèíèìàëüíàÿ ñêîðîñòü äæåòïàêà!', -1)
				else
					boost = boost - 0.03
				end
			end
		...
		if wasKeyPressed(188) and not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() and cfg.main.perehodik and ReconWindow.v then
			lua_thread.create(function()
				if spec_id - 1 < 0 then
					sampAddChatMessage(tag .. 'Ïðîøëûé ID íå â ñåòè, èñïîëüçóéòå òîëüêî >> NEXT', -1)
		...
		if wasKeyPressed(190) and not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() and cfg.main.perehodik and ReconWindow.v then
			lua_thread.create(function()
				if spec_id + 1 > 999 then
					sampAddChatMessage(tag .. 'Ñëåäóþùèé ID íå â ñåòè, èñïîëüçóéòå òîëüêî << BACK', -1)
		...
		if sampGetGamestate() ~= 3 and cfg.main.lovlyareporta then
			sampAddChatMessage(
				tag .. 'Àâòîëîâëÿ ðåïîðòà âûêëþ÷åíà, òàê êàê âû íàõîäèòåñü â ñòàòóñå "Ïîäêëþ÷åíèå ê ñåðâåðó"', -1)
			cfg.main.lovlyareporta = false
		end

		if sampGetGamestate() ~= 3 and cfg.main.autopiar then
			sampAddChatMessage(tag .. 'Àâòîðåêëàìà âûêëþ÷åíà, òàê êàê âû íàõîäèòåñü â ñòàòóñå "Ïîäêëþ÷åíèå ê ñåðâåðó"',
				-1)
		...
		if cfg.main.lovlyareporta and style_lovlyareporta.v == 3 and not isGamePaused() and not sampIsChatInputActive() and not isSampfuncsConsoleActive() and not sampIsDialogActive() and not ReportWindow.v then
			if sampGetGamestate() ~= 3 then
				sampAddChatMessage(tag .. 'Íåëüçÿ âêëþ÷èòü àâòîëîâëþ ðåïîðòà ïðè ïîäêëþ÷åíèè ê ñåðâåðó!', -1)
			else
				sampSendChat('/ot')
			end
		end
		...
		imgui.ShowCursor = MainWindow.v or TeleportPlayerWindow.v or AmemberWindow.v or TeleportWindow.v or
			ReportWindow.v or CaptchaWindow.v or UpdateWindow.v or HelpWindow.v or ChangeLogWindow.v or
			InteractionWindow.v or
			ColorsWindow.v or ChangePosWindow.v or ReconWindow.v or PunishWindow.v or InfoWindow.v or
			ChangePosWindowTwo.v or
			myOnline.v or ChangePosWindowThree.v or ChangePosWindowFour.v or ChangePosWindowFive.v

		imgui.Process = MainWindow.v or TeleportPlayerWindow.v or AmemberWindow.v or TeleportWindow.v or ReportWindow.v or
			ChangePosWindow.v or CaptchaWindow.v or UpdateWindow.v or HelpWindow.v or ChangeLogWindow.v or
			InteractionWindow.v or ReconWindow.v or ColorsWindow.v or to.v or PunishWindow.v or InfoWindow.v or
			ChangePosWindowTwo.v or myOnline.v or ChangePosWindowThree.v or ChangePosWindowFour.v or reconusers_state.v or
			ChangePosWindowFive.v

		local oTime = os.time()
		if elements.checkbox.drawBullets.v then
			for i = 1, bulletSync.maxLines do
				if bulletSync[i].other.time >= oTime then
					local result, wX, wY, wZ, wW, wH = convert3DCoordsToScreenEx(bulletSync[i].other.o.x,
						bulletSync[i].other.o.y, bulletSync[i].other.o.z, true, true)
					local resulti, pX, pY, pZ, pW, pH = convert3DCoordsToScreenEx(bulletSync[i].other.t.x,
						bulletSync[i].other.t.y, bulletSync[i].other.t.z, true, true)
					if result and resulti then
							...
							if bulletSync[i].other.id ~= -1 then
								if sampIsPlayerConnected(bulletSync[i].other.id) then
									if elements.checkbox.onlyId.v and elements.checkbox.onlyNick.v then
										renderFontDrawText(fonttr,
											sampGetPlayerNickname(bulletSync[i].other.id) ..
											'[' .. bulletSync[i].other.id .. ']', wX + 0.5, wY,
											bulletSync[i].other.colorText, false)
									elseif elements.checkbox.onlyId.v then
										renderFontDrawText(fonttr, '[' .. bulletSync[i].other.id .. ']', wX + 0.5, wY,
											bulletSync[i].other.colorText, false)
									elseif elements.checkbox.onlyNick.v then
										renderFontDrawText(fonttr, sampGetPlayerNickname(bulletSync[i].other.id),
											wX + 0.5, wY, bulletSync[i].other.colorText, false)
									end
								end
							end
						end
						...
						if elements.checkbox.cbEnd.v then
							renderDrawPolygon(pX, pY - 1, 3 + elements.int.sizeOffPolygonEnd.v,
								3 + elements.int.sizeOffPolygonEnd.v, 1 + elements.int.rotationPolygonEnd.v,
								elements.int.degreePolygonEnd.v, bulletSync[i].other.color)
						end
					end
				end
			end
		...
		if elements.checkbox.drawMyBullets.v then
			for i = 1, bulletSyncMy.maxLines do
				if bulletSyncMy[i].my.time >= oTime then
					local result, wX, wY, wZ, wW, wH = convert3DCoordsToScreenEx(bulletSyncMy[i].my.o.x,
						bulletSyncMy[i].my.o.y, bulletSyncMy[i].my.o.z, true, true)
					local resulti, pX, pY, pZ, pW, pH = convert3DCoordsToScreenEx(bulletSyncMy[i].my.t.x,
						bulletSyncMy[i].my.t.y, bulletSyncMy[i].my.t.z, true, true)
					if result and resulti then
						local xResolution = mem.getuint32(0x00C17044)
						if wZ < 1 then
							wX = xResolution - wX
						...
						if elements.checkbox.cbEndMy.v then
							renderDrawPolygon(pX, pY - 1, 3 + elements.int.sizeOffMyPolygonEnd.v,
								3 + elements.int.sizeOffMyPolygonEnd.v, 1 + elements.int.rotationMyPolygonEnd.v,
								elements.int.degreeMyPolygonEnd.v, bulletSyncMy[i].my.color)
						end
					end
				end
			end
		...
		if cfg.main.speedhack then
			if isCharInAnyCar(PLAYER_PED) and isKeyDown(VK_LMENU) then
				if getCarSpeed(storeCarCharIsInNoSave(PLAYER_PED)) * 2.01 <= shmax.v then
					...
					if Cos < 0 and cVecY > 0 then yforce = -yforce end
					applyForceToCar(storeCarCharIsInNoSave(PLAYER_PED), xforce * Sin, yforce * Cos, zforce / 2, 0.0, 0.0,
						0.0)
				end
			end
		end
			...
			if isKeysDown(flovlyareporta.v) then
				if sampGetGamestate() ~= 3 then
					...
					sampAddChatMessage(tag ..
						'Àâòîëîâëÿ ðåïîðòà ' .. (cfg.main.lovlyareporta and 'âêëþ÷åíà' or 'âûêëþ÷åíà'), -1)
				end
			end
		end
		...
		if enAirBrake then
			...
			renderFontDrawText(my_font,
				'Èçìåíåíèå ñêîðîñòè: {E78284}' ..
				klavisha ..
				' {84A6E7} è {E78284}' .. klavisha1 .. '\n{84A6E7}Òåêóùàÿ ñêîðîñòü: {E78284}' .. cfg.main.speed_airbrake,
				cx + 500, windowCoordinates[2] - 32, 0xFF84A6E7)
			...
			if not isSampfuncsConsoleActive() and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() then
				...
				if isKeyDown(VK_UP) or isKeyDown(VK_SPACE) then
					airBrkCoords[3] = airBrkCoords[3] +
						cfg.main.speed_airbrake
				end
				if isKeyDown(VK_DOWN) and airBrkCoords[3] or isKeyDown(VK_LSHIFT) and airBrkCoords[3] > -95.0 then
					airBrkCoords[3] =
						airBrkCoords[3] - cfg.main.speed_airbrake
				end
				if isKeysDown(upair.v) then
					if cfg.main.speed_airbrake + 0.2 > 10.0 then
						sampAddChatMessage(tag .. 'Ýòî ìàêñèìàëüíàÿ ñêîðîñòü!', -1)
					else
				...
				if isKeysDown(fspeeddown.v) then
					if cfg.main.speed_airbrake - 0.2 < 0.2 then
						sampAddChatMessage(tag .. 'Ýòî ìèíèìàëüíàÿ ñêîðîñòü!', -1)
					else
						cfg.main.speed_airbrake = cfg.main.speed_airbrake - 0.2
					end
				end
		...
		if not sampIsChatInputActive() and not isSampfuncsConsoleActive() and isPlayerPlaying(playerHandle) then
			if isKeysDown(fvizualrep.v) then
				if nickplayer == nil or idplayer == nil or report_text == nil or textplayer == nil then
					sampAddChatMessage(tag .. 'Çà òåêóùóþ ñåññèþ âû íå îòâåòèëè íè íà îäèí ðåïîðò!', -1)
				else
					sampAddChatMessage('--------------------------------------------------------------------', 0xFFCDAD00)
					sampAddChatMessage('Ðåïîðò îò {90EE90}' .. nickplayer .. '[' .. idplayer .. ']', 0xFFCD5C5C)
					sampAddChatMessage('Ñóòü ðåïîðòà: {90EE90}' .. report_text, 0xFFCD5C5C)
					sampAddChatMessage('Âàø îòâåò: {90EE90}' .. textplayer, 0xFFCD5C5C)
					sampAddChatMessage('Çà ñåãîäíÿ îòâåòîâ: {90EE90}' .. cfg.main.reportzaday, 0xFFCD5C5C)
					sampAddChatMessage('--------------------------------------------------------------------', 0xFFCDAD00)
				end
			end
		end
		...
		if not sampIsChatInputActive() and not isSampfuncsConsoleActive() and isPlayerPlaying(playerHandle) then
			if isKeysDown(farep.v) then
				if nickplayer == nil or idplayer == nil or report_text == nil or textplayer == nil then
					...
					sampSendChat("/a Ðåïîðò îò {84A6E7}" .. nickplayer .. "[" .. idplayer .. "]")
					sampSendChat("/a Ñóòü ðåïîðòà: {84A6E7}" .. report_text)
					sampSendChat("/a Ìîé îòâåò: {84A6E7}" .. textplayer)
				end
			end
		end
		...
		if not sampIsChatInputActive() and not isSampfuncsConsoleActive() and isPlayerPlaying(playerHandle) then
			if isKeysDown(fnorules.v) then
				if report_id == nil then
					...
					sampSendChat("/pm " .. report_id .. " 0 Èãðîê, íà êîòîðîãî âû æàëîâàëèñü, íå íàðóøàåò.")
				end
			end
		end
		...
		if not sampIsChatInputActive() and not isSampfuncsConsoleActive() and isPlayerPlaying(playerHandle) and ReportWindow.v then
			if isKeysDown(fperedam.v) then
				...
				local template = template:gsub('{my_name}', my_name)
				sampSendDialogResponse(6370, 1, _, template)
				sampSendChat("/a >>> [ÐÅÏÎÐÒ] " .. report_name .. "[" .. report_id .. "]: " .. report_text .. " ")
				text_buffer_rep.v = ''
				ReportWindow.v = false
			end
		end
		...
		if isKeysDown(facceptform.v) and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() and not isSampfuncsConsoleActive() and cfg.main.trueform then
			if active_report == 2 then
				lua_thread.create(function()
					...
					sampAddChatMessage(
						"[AdminTools] {" ..
						cfg.main.twocolorforms ..
						"}Ôîðìà >> {" ..
						cfg.main.colorforms .. "}/" .. cmd .. " " .. paramssss .. " {" ..
						cfg.main.twocolorforms .. "}<< âûäàíà!", "0xFF" .. cfg.main.colorforms)
					local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//AcceptForms.txt", 'a')
					f:write('[' ..
						os.date("%d.%m.%Y %X", os.time()) ..
						'] Ôîðìà: /' ..
						cmd .. ' ' .. paramssss .. ' | Îòïðàâèòåëü: ' .. admin_nick .. ' | Òèï ïðèíÿòèÿ ôîðìû: âðó÷íóþ\n')
					f:close()
...
function imgui.BeforeDrawFrame()
	if fa_font == nil then
		...
		fa_font = imgui.GetIO().Fonts:AddFontFromFileTTF('moonloader/resource/fonts/fa-solid-900.ttf', 17.0, font_config,
			fa_glyph_ranges)
	end
	if minimalfont == nil then
		minimalfont = imgui.GetIO().Fonts:AddFontFromFileTTF(
			'moonloader/resource/fonts/fa-solid-900.ttf', 15.0, font_config, fa_glyph_ranges)
	end
	if fontsize == nil then
		fontsize = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 20.0, nil,
			imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
	end
	if fs16 == nil then
		fs16 = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 16.0, nil,
			imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
	end
	if fs20 == nil then
		fs20 = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 20.0, nil,
			imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
	end
	if fs40 == nil then
		fs40 = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 40.0, nil,
			imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
	end
	if fs80 == nil then
		fs80 = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 80.0, nil,
			imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
	end
	if infoFs == nil then
		infoFs = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 35.0,
			nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
	end
	if fsClock == nil then
		fsClock = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 25.0, nil,
			imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
	end
end
...
function imgui.OnDrawFrame()
	...
	local colours = { u8 'Ðîçîâûé', u8 'Çåë¸íûé', u8 'Êðàñíûé', u8 'Ôèîëåòîâûé', u8 '×¸ðíûé' }
	local binds = { u8 'Çàùèòà àêêàóíòà', u8 'Ìíîæèòåëü äîíàòà', u8 'Ïðàâèëà ñåðâåðà', u8 'Äèñêîðä', u8 'Ïàáëèê', u8 'Ñîáñòâåííûé ¹1',
		u8 'Ñîáñòâåííûé ¹2', u8 'Ñîáñòâåííûé ¹3', u8 'Ñîáñòâåííûé ¹4' }
	if MainWindow.v then
		imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(900, 550), 1)
		imgui.Begin(fa.ICON_FA_TOOLS .. u8 ' Íàñòðîéêè ñêðèïòà ' .. fa.ICON_FA_TOOLS, MainWindow,
		...
		if imgui.Button(fa.ICON_FA_SYNC_ALT .. u8 '', imgui.ImVec2(54, 27)) then
			sampAddChatMessage(tag .. 'Ñêðèïò ïåðåçàãðóæåí.', -1)
			imgui.ShowCursor = false
			thisScript():reload()
		end
		imgui.Hint('reloadss', u8 'Ïåðåçàïóñòèòü ñêðèïò')
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_POWER_OFF .. u8 '', imgui.ImVec2(54, 27)) then
			sampAddChatMessage(tag .. 'Ñêðèïò óñïåøíî âûãðóæåí, äëÿ çàïóñêà èñïîëüçóéòå êîìáèíàöèþ êëàâèø CTRL + R.', -1)
			imgui.ShowCursor = false
			thisScript():unload()
		end
		imgui.Hint('off12322', u8 'Âûêëþ÷èòü ñêðèïò')
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_SAVE .. u8 '', imgui.ImVec2(54, 27)) then
			sampAddChatMessage(tag .. 'Íàñòðîéêè ñêðèïòà óñïåøíî ñîõðàíåíû.', -1)
			inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
		end
		imgui.Hint('saves', u8 'Ñîõðàíèòü íàñòðîéêè')
		...
		if menusel.v == 1 then
			...
			local _, myid = sampGetPlayerIdByCharHandle(playerPed)
			mynick = sampGetPlayerNickname(myid)
			imgui.Text(u8 'Âàø íèê: ' .. mynick .. '[' .. myid .. ']')
			imgui.Text(u8 'Âàø LVL àäìèíèñòðèðîâàíèÿ:')
			imgui.SameLine()
			...
			if cfg.main.mylvladmin == 8 then
				imgui.Text(u8 'Âàø LVL ôóëë äîñòóïà:')
				imgui.SameLine()
				imgui.PushItemWidth(20)
			...
			imgui.PopFont()
			imgui.Hint('asd5', u8 'Óêàæèòå íîìåð ñåðâåðà:\n1  Arena')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa11', u8 'Âû íèêîãäà íå áóäåòå ïàäàòü ñ äâóõêîëåñíîãî òðàíñïîðòà')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa12', u8 'Âû áóäåòå çàùèùåíû îò ïóëü, à òðàíñïîðò îò ïîâðåæäåíèé')
			...
			imgui.PopFont()
			imgui.Hint('helpa13',
				u8 'Àíàëîã ìåòëû, òîëüêî â ðàçû áûñòðåå\nÂêë./âûêë.: R.SHIFT\nÓñêîðåíèå: Num +\nÇàìåäëåíèå:Num -')
			if imgui.ToggleButton(u8 'Ïðèíÿòèå ôîðì èç àäìèí-÷àòà', imgui.ImBool(cfg.main.trueform)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa15', u8 'Âûïîëíåíèå íàêàçàíèé îò ìë. àäìèíèñòðàöèè')
			if imgui.ToggleButton(u8 'Ôðàêöèÿ ïðè àâòîðèçàöèè', imgui.ImBool(cfg.main.fraclogin)) then
			...
			imgui.PopFont()
			imgui.Hint('helpa3',
				u8 'Ââåäèòå ID ôðàêöèè â êîòîðóþ âû áóäåòå âñòóïàòü ïðè àâòîðèçàöèè\nÔóíêöèÿ äîñòóïíà ñ 4+ LVL')
			...
			if imgui.ToggleButton(u8 'Ñêèí ïðè àâòîðèçàöèè', imgui.ImBool(cfg.main.skinlogin)) then
			...
			imgui.PopFont()
			imgui.Hint('helpa2',
				u8 'Ââåäèòå ID ñêèíà êîòîðûé áóäåò âûäàâàòüñÿ ïðè àâòîðèçàöèè\nÔóíêöèÿ äîñòóïíà ñ 4+ LVL')
			...
			if imgui.ToggleButton(u8 'Èíòåðüåð ïðè àâòîðèçàöèè', imgui.ImBool(cfg.main.intlogin)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa4', u8 'Ââåäèòå ID èíòåðüåðà â êîòîðûé âû áóäåòå òåëåïîðòèðîâàòüñÿ ïðè àâòîðèçàöèè')
			...
			if imgui.ToggleButton(u8 'Àâòîâõîä â àêêàóíò', imgui.ImBool(cfg.main.autologin)) then
			...
			imgui.PopFont()
			imgui.Hint('helpa1',
				u8 'Äëÿ ðàáîòû äàííîé ôóíêöèè óêàæèòå ñâîé ïàðîëü îò àêêàóíòà â ñîîòâåòñòâóþùåì ïóíêòå.\nÏðè ïîäêëþ÷åíèè íà ñåðâåð ñêðèïò ñàì ìîìåíòàëüíî ââåäåò ïàðîëü')
			...
			if imgui.ToggleButton(u8 'Àâòîâõîä â àäìèíêó', imgui.ImBool(cfg.main.autoadm)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa16', u8 'Ïðè àâòîðèçàöèè íà ñåðâåð ñêðèïò àâòîìàòè÷åñêè ââåäåò ïàðîëü îò àäìèíêè')
			...
			if imgui.ToggleButton(u8 'Òåëåïîðò â ÀÇ ïðè âõîäå', imgui.ImBool(cfg.main.autoaz)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa17', u8 'Ïðè âõîäå â àäìèíêó ñêðèïò àâòîìàòè÷åñêè òåëåïîðòèðóåòñÿ â /AZ')
			if imgui.ToggleButton(u8 'Àâòîëîâëÿ ðåïîðòà', imgui.ImBool(cfg.main.lovlyareporta)) then
				cfg.main.lovlyareporta = not cfg.main.lovlyareporta
			end
			if imgui.SettingsButton() then imgui.OpenPopup(u8 'Íàñòðîéêà àâòîëîâëè ðåïîðòà') end
			if imgui.BeginPopupModal(u8 'Íàñòðîéêà àâòîëîâëè ðåïîðòà', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				imgui.CenterText(u8 'Âûáåðèòå ñïîñîá:')
				if imgui.RadioButton(u8 "Ïî òåêñòó â ÷àòå", style_lovlyareporta, 1) then
					cfg.main.stylelovlyareporta = 1
				end
				if imgui.RadioButton(u8 'Ïî íàäïèñè "REPORT ++"', style_lovlyareporta, 2) then
					cfg.main.stylelovlyareporta = 2
				end
				if imgui.RadioButton(u8 'Îáû÷íûé ôëóä /ot', style_lovlyareporta, 3) then
					cfg.main.stylelovlyareporta = 3
				end
				if imgui.ToggleButton(u8 'Àâòîìàòè÷åñêè âûêëþ÷àòü ïîñëå ëîâëè 1 ðåïîðòà', imgui.ImBool(cfg.main.autooff)) then
					cfg.main.autooff = not cfg.main.autooff
				end
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
					imgui.CloseCurrentPopup()
				end
				imgui.EndPopup()
			end
			if imgui.ToggleButton(u8 'Áåñêîíå÷íûå ïàòðîíû', imgui.ImBool(cfg.main.infammo)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa2313842381', u8 'Ïðè ñòðåëüáå ó âàñ íå áóäóò òðàòèòüñÿ ïàòðîíû, îíè ñòàíóò áåñêîíå÷íûìè')
			if imgui.ToggleButton(u8 'Ñêðûâàòü IP àäðåñà', imgui.ImBool(cfg.main.removeip)) then
			...
			imgui.PopFont()
			imgui.Hint('helpa113',
				u8 'ÈÏû â ñîîáùåíèÿõ áóäóò çàìåíÿòüñÿ òàêèì îáðàçîì: 93.õõ.õõ.186\nÏîëåçíàÿ ôóíêöèÿ, âåäü íà îïðàõ áîëüøå íå áóäåò ÈÏîâ èãðîêîâ/àäìèíîâ')
			if imgui.ToggleButton(u8 'Íîï F5', imgui.ImBool(cfg.main.nopf5)) then
				cfg.main.nopf5 = not cfg.main.nopf5
				if not cfg.main.nopf5 then
					sampAddChatMessage('[Èíôîðìàöèÿ] Ïîñëå ïåðåçàõîäà â èãðó êëàâèøà F5 ñíîâà áóäåò ðàáîòàñïîñîáà',
						14628149)
				end
			end
			imgui.SameLine()
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa211221', u8 'Îòêëþ÷àåò ðàáîòó êëàâèøè F5')
			if imgui.ToggleButton(u8 'Íåâèäèìêà', imgui.ImBool(cfg.main.invisible)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('invisibleasddsaw', u8 'Âû íå áóäåòå âèäíû äðóãèì èãðîêàì\nïîìîãàåò ïðè ñëåæêå çà êàïòàìè è ò.ï.')
			if imgui.ToggleButton(u8 '×åêåð èãðîêîâ', imgui.ImBool(cfg.main.playersChecker)) then
				cfg.main.playersChecker = not cfg.main.playersChecker
			end
			if imgui.ToggleButton(u8 '×åêåð îòêëþ÷åíèé', imgui.ImBool(cfg.main.leaveChecker)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa14', u8 'Ïðè îòêëþ÷åíèè èãðîêà â ÷àò áóäåò ïèñàòüñÿ åãî íèê è ïðè÷èíà âûõîäà')
			if imgui.ToggleButton(u8 "Òðåéñåð ñâîèõ ïóëü", elements.checkbox.drawMyBullets) then
				cfg.config.drawMyBullets = elements.checkbox.drawMyBullets.v
				inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
			end
			if imgui.SettingsButton() then menusel.v = 2 end
			if imgui.ToggleButton(u8 'Ñâå÷åíèå ñåòîâ +50', imgui.ImBool(cfg.main.offamulet)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpsadjdsaqa18', u8 'Äàííàÿ ôóíêöèÿ âèçóàëüíî îòêëþ÷àåò ñâå÷åíèå ñåòîâ +50 è âûøå')
			if imgui.ToggleButton(u8 'Ëîããèðîâàíèå äåéñòâèé', imgui.ImBool(cfg.main.logging)) then
				cfg.main.logging = not cfg.main.logging
			end

			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Ëîããèðîâàíèå äåéñòâèé') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Ëîããèðîâàíèå äåéñòâèé', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				if imgui.ToggleButton(u8 'Ëîããèðîâàíèå âûäà÷è áëîêèðîâîê IP â îôôëàéíå', imgui.ImBool(cfg.main.logbanipoff)) then
					cfg.main.logbanipoff = not cfg.main.logbanipoff
				end
				if imgui.ToggleButton(u8 'Ëîããèðîâàíèå âûäà÷è áëîêèðîâîê â îôôëàéíå', imgui.ImBool(cfg.main.logbanoff)) then
					cfg.main.logbanoff = not cfg.main.logbanoff
				end
				if imgui.ToggleButton(u8 'Ëîããèðîâàíèå âûäà÷è äåìîðãàíîâ â îôôëàéíå', imgui.ImBool(cfg.main.logjailoff)) then
					cfg.main.logjailoff = not cfg.main.logjailoff
				end
				if imgui.ToggleButton(u8 'Ëîããèðîâàíèå âûäà÷è çàãëóøåê â îôôëàéíå', imgui.ImBool(cfg.main.logmuteoff)) then
					cfg.main.logmuteoff = not cfg.main.logmuteoff
				end
				if imgui.ToggleButton(u8 'Ëîããèðîâàíèå ðàçáëîêèðîâîê IP', imgui.ImBool(cfg.main.logunbanip)) then
					cfg.main.logunbanip = not cfg.main.logunbanip
				end
				if imgui.ToggleButton(u8 'Ëîããèðîâàíèå ðàçáëîêèðîâîê', imgui.ImBool(cfg.main.logunban)) then
					cfg.main.logunban = not cfg.main.logunban
				end
				if imgui.ToggleButton(u8 'Ëîããèðîâàíèå âûïóñêîâ èç äåìîðãàíà', imgui.ImBool(cfg.main.logunjail)) then
					cfg.main.logunjail = not cfg.main.logunjail
				end
				if imgui.ToggleButton(u8 'Ëîããèðîâàíèå ñíÿòèé çàãëóøåê', imgui.ImBool(cfg.main.logunmute)) then
			...
			if imgui.ToggleButton(u8 'Àâòîâêëþ÷åíèå àíòèÒÏ', imgui.ImBool(cfg.main.antitp)) then
			...
			imgui.PopFont()
			imgui.Hint('antitps ',
				u8 'Ïðè âõîäå ïîä àäìèíêó ó âàñ àâòîìàòè÷åñêè áóäåò\nâêëþ÷àòüñÿ /antitp\n{fe4e4e}Äîñòóïíî ñ 4 LVL FD!')
			imgui.NextColumn()
			if imgui.ToggleButton(u8 'Ïåðåõîä ñëåæêè íà << è >>', imgui.ImBool(cfg.main.perehodik)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('heloa388433',
				u8 'Ïðè íàæàòèè êíîïêè > èëè < âû ïåðåéäåòå â ñëåæêó çà ñëåäóþùèì èëè ïðîøëûì èãðîêîì')
			if imgui.ToggleButton(u8 'Àâòî nRP (/b) ÷àò', imgui.ImBool(cfg.main.autob)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa18  ', u8 'Âåñü òåêñò áóäåò îòïðàâëÿòüñÿ â nRP (/b) ÷àò')
			if imgui.ToggleButton(u8 'Èíôîðìàöèÿ ïîñëå îòâåòà /ot', imgui.ImBool(cfg.main.inforeport)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa19', u8 'Äàííàÿ ôóíêöèÿ âêëþ÷àåò ñîîáùåíèå ñ èíôîðìàöèåé î îòâåòå íà ïîñëåäíèé ðåïîðò.')
			if imgui.ToggleButton(u8 "Èíôîðìàöèÿ/ñòàòèñòèêà", to) then
				cfg.statTimers.state = to.v
			end
			if imgui.SettingsButton() then imgui.OpenPopup(u8 'Íàñòðîéêà èíôîðìàöèè è ñòàòèñòèêè') end
			if imgui.BeginPopupModal(u8 'Íàñòðîéêà èíôîðìàöèè è ñòàòèñòèêè', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				kotofeevstatsadm()
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
					imgui.CloseCurrentPopup()
			...
			if imgui.ToggleButton(u8 'Ñïàâíèòüñÿ íà /spawnset', imgui.ImBool(cfg.main.spawnonset)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa20', u8 'Ïðè âõîäå íà ñåðâåð âû áóäåòå àâòîìàòè÷åñêè ñïàâíèòüñÿ íà óñòàíîâëåííîì ìåñòå')
			if imgui.ToggleButton(u8 'Íîï F4', imgui.ImBool(cfg.main.nopf4)) then
				cfg.main.nopf4 = not cfg.main.nopf4
				if not cfg.main.nopf4 then
					sampAddChatMessage('[Èíôîðìàöèÿ] Ïîñëå ïåðåçàõîäà â èãðó êëàâèøà F4 ñíîâà áóäåò ðàáîòàñïîñîáà',
						14628149)
				end
			end
			imgui.SameLine()
			...
			imgui.PopFont()
			imgui.Hint('helpa21',
				u8 'Îòêëþ÷àåò ðàáîòó êëàâèøè F4, ïðè ÷àñòîì ïîïàäàíèè â áîëüêó îòêëþ÷èòå ôóíêöèþ è ïåðåçàéäèòå')
			if imgui.ToggleButton(u8 'Ðàäàð â ðåêîíå', imgui.ImBool(cfg.main.radarinrecon)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa22', u8 'Äàííàÿ ôóíêöèÿ âêëþ÷àåò îòîáðàæåíèå õóäà/ðàäàðà â ðåêîíå')
			if imgui.ToggleButton(u8 'Óäàëåíèå ñòðîê â ÷àòå', imgui.ImBool(cfg.main.deleteactions)) then
				cfg.main.deleteactions = not cfg.main.deleteactions
			end

			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Óäàëåíèå äåéñòâèé â ÷àòå') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Óäàëåíèå äåéñòâèé â ÷àòå', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î ñëàïàõ", imgui.ImBool(cfg.main.deleteslap)) then
					cfg.main.deleteslap = not cfg.main.deleteslap
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î áàíàõ", imgui.ImBool(cfg.main.deleteban)) then
					cfg.main.deleteban = not cfg.main.deleteban
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î âûäà÷å âðåìåííîãî òñ", imgui.ImBool(cfg.main.deleteplveh)) then
					cfg.main.deleteplveh = not cfg.main.deleteplveh
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î áëîêèðîâêå ÷àòà", imgui.ImBool(cfg.main.deletemute)) then
					cfg.main.deletemute = not cfg.main.deletemute
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î âûäà÷å âàðíà", imgui.ImBool(cfg.main.deletewarn)) then
					cfg.main.deletewarn = not cfg.main.deletewarn
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î âûäà÷å äåìîðãàíà", imgui.ImBool(cfg.main.deletejail)) then
					cfg.main.deletejail = not cfg.main.deletejail
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î ñîîáùåíèè èãðîêó", imgui.ImBool(cfg.main.deletepm)) then
					cfg.main.deletepm = not cfg.main.deletepm
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê àäìèí-÷àòà", imgui.ImBool(cfg.main.deleteadminchat)) then
					cfg.main.deleteadminchat = not cfg.main.deleteadminchat
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î íàõîäêå êëàäà", imgui.ImBool(cfg.main.deleteclad)) then
					cfg.main.deleteclad = not cfg.main.deleteclad
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê îò Àíòè×èòà", imgui.ImBool(cfg.main.deleteanticheat)) then
					cfg.main.deleteanticheat = not cfg.main.deleteanticheat
				end
				if imgui.ToggleButton(u8 "Óäàëåíèå ñòðîê î íà÷àëå ñëåæêè", imgui.ImBool(cfg.main.deletespec)) then
			...
			if imgui.ToggleButton(u8 'Óäàëÿòü ôëóä â ÷àò îò /ot', imgui.ImBool(cfg.main.deleteot)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa23', u8 'Äàííàÿ ôóíêöèÿ îòêëþ÷àåò ñîîáùåíèÿ â ÷àòå îá îòñóòñòâèè ðåïîðòà')
			if imgui.ToggleButton(u8 'Àâòîçàêðûòèå äèàëîãà îò àíòè÷èòà', imgui.ImBool(cfg.main.deleteac)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa25', u8 'Äàííàÿ ôóíêöèÿ àâòîìàòè÷åñêè çàêðûâàåò äèàëîã î êèêå îò àíòè÷èòà (7+ LVL)')
			if imgui.ToggleButton(u8 'Ëîããèðîâàíèå ïðèíÿòûõ ôîðì â ôàéë', imgui.ImBool(cfg.main.logform)) then
			...
			imgui.PopFont()
			imgui.Hint('helpa26',
				u8 'Äàííàÿ ôóíêöèÿ ëîããèðóåò âñå ïðèíÿòûå ôîðìû â îòäåëüíûé ôàéë, äàáû ñîêðàòèòü ñëó÷àè ïîäñòàâû')
			if imgui.ToggleButton(u8 'Àâòîóñòàíîâêà óðîâíÿ àäìèíêè', imgui.ImBool(cfg.main.geniumlvl)) then
			...
			imgui.PopFont()
			imgui.Hint('helpa27',
				u8 'Ïðè âõîäå â àäìèíêó ñêðèïò áóäåò àâòîìàòè÷åñêè óñòàíàâëèâàòü óðîâåíü àäìèíèñòðàòîðà íà ñåðâåðíûé')
			if imgui.ToggleButton(u8 'Àâòîìàòè÷åñêàÿ âûäà÷à àööåïòà (ñåáå)', imgui.ImBool(cfg.main.geniumaccept)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa28', u8 'Äàííàÿ ôóíêöèÿ àâòîìàòè÷åñêè âûäàñò âàì àööåïò, åñëè åãî çàïðîñèò ñåðâåð (7+ LVL)')
			if imgui.ToggleButton(u8 'Îòñóòñòâèå ïåðåçàðÿäêè', imgui.ImBool(cfg.main.noreload)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa2311384122381',
				u8 'Ïðè ñòðåëüáå ó âàñ àâòîìàòè÷åñêè áóäåò ñêðîëëèòüñÿ îðóæèå, òåì ñàìûì îòñóòñòâîâàòü ïåðåçàðÿäêà')
			if imgui.ToggleButton(u8 'SpeedHack', imgui.ImBool(cfg.main.speedhack)) then
				cfg.main.speedhack = not cfg.main.speedhack
			end
			if imgui.SettingsButton() then imgui.OpenPopup(u8 "Íàñòðîéêà SpeedHack'a") end
			if imgui.BeginPopupModal(u8 "Íàñòðîéêà SpeedHack'a", MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				imgui.Text(u8 'Ìàêñèìàëüíàÿ ñêîðîñòü:')
				imgui.SameLine()
				imgui.PushItemWidth(150)
				imgui.SliderInt(u8 '##3huec', shmax, 80, 300)
				imgui.PopItemWidth()
				imgui.Text(u8 'Ïëàâíîñòü:')
				imgui.SameLine()
				imgui.PushItemWidth(150)
			...
			if imgui.ToggleButton(u8 'Àâòî÷åê ðåãîâ (/getip)', imgui.ImBool(cfg.main.regi)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('helpa1122311384122381',
				u8 'Ïðè ïðîïèñûâàíèè /getip ñêðèïò àâòîìàòè÷åñêè áóäåò îïðåäåëÿòü ìåñòîïîëîæåíèå è âûâîäèòü â /a ëèáî ëîêàëüíûé ÷àò')
			if imgui.SettingsButton() then imgui.OpenPopup(u8 "Íàñòðîéêà ïðîâåðêè ðåãîâ") end
			if imgui.BeginPopupModal(u8 "Íàñòðîéêà ïðîâåðêè ðåãîâ", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				imgui.TextColoredRGB('{84A6E7}Êóäà îòïðàâëÿòü äàííûå?')
				if imgui.RadioButton(u8 "Â àäì. ÷àò", style_reg, 1) then
					cfg.main.stylereg = 1
				end
				if imgui.RadioButton(u8 "Â ëîêàëüíûé ÷àò", style_reg, 2) then
					cfg.main.stylereg = 2
				end
				if cfg.main.stylereg == 1 then
					imgui.TextColoredRGB('{84A6E7}Â êàêèõ ñëó÷àÿõ îòïðàâëÿòü â /a?')
					if imgui.RadioButton(u8 "Òîëüêî ïðè ïðèíÿòèè ôîðìû", style_admin, 1) then
						cfg.main.styleadmin = 2
					end
					if imgui.RadioButton(u8 "Âñåãäà (äàæå åñëè íå ïî ôîðìå)", style_admin, 2) then
						cfg.main.styleadmin = 2
					end
				end
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
			...
			if imgui.ToggleButton(u8 'WallHack íà òðàíñïîðò', imgui.ImBool(cfg.Settings.renderText)) then
				cfg.Settings.renderText = not cfg.Settings.renderText
				inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
			end
			if imgui.SettingsButton() then imgui.OpenPopup(u8 'Íàñòðîéêè | WallHack íà òðàíñïîðò') end
			settingswallhackoncar()
			if imgui.ToggleButton(u8 'Àâòîðåêëàìà', imgui.ImBool(cfg.main.autopiar)) then
				cfg.main.autopiar = not cfg.main.autopiar
				sendpiar()
			end
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Àâòîðåêëàìà') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Àâòîðåêëàìà', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				imgui.Text(fa.ICON_FA_COFFEE .. u8 ' Âûáåðèòå áèíä äëÿ ðåêëàìû:')
				imgui.SameLine()
				imgui.PushItemWidth(150)
				...
				imgui.Text(u8 'Èíòåðâàë ìåæäó ðåêëàìîé:')
				imgui.SameLine()
				imgui.PushItemWidth(100)
			...
			if imgui.ToggleButton(u8 'Âàðíèíãè íà ÒÊ', imgui.ImBool(cfg.main.warningtk)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('warningtkss',
				u8 'Åñëè èãðîê óáüåò ñâîåãî íàïàðíèêà ïî îðãàíèçàöèè, òî ñêðèïò îïîâåñòèò âàñ îá ýòîì â ÷àòå')
			if imgui.ToggleButton(u8 '×åêåð ïîäêëþ÷åíèé', imgui.ImBool(cfg.main.connectChecker)) then
				cfg.main.connectChecker = not cfg.main.connectChecker
			end
			if imgui.ToggleButton(u8 '×åêåð ðåãèñòðàöèé', imgui.ImBool(cfg.main.regChecker)) then
				cfg.main.regChecker = not cfg.main.regChecker
			end
			if imgui.ToggleButton(u8 "Òðåéñåð ÷óæèõ ïóëü", elements.checkbox.drawBullets) then
			...
			if imgui.ToggleButton(u8 'Îòîáðàæàòü /b íàä ãîëîâîé', imgui.ImBool(cfg.main.underb)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('underbq', u8 'Îòîáðàæàåò âåñü òåêñò, íàïèñàííûé â /b ÷àò íàä ãîëîâîé èãðîêà')
			if imgui.ToggleButton(u8 'Ñëåæêà ïî êèëë-ëèñòó', imgui.ImBool(cfg.main.spectatekill)) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.PopFont()
			imgui.Hint('spectatekillaa',
				u8 'Ïðè íàæàòèè ïî íèêíåéìó èãðîêà â êèëë-ëèñòå\nâû àâòîìàòè÷åñêè ïåðåéäåòå â ñëåæêó çà íèì')
			imgui.Columns(1)
			imgui.Separator()
			if imgui.Button(u8 'Çàùèòà àêêàóíòà', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Çàùèòà àêêàóíòà') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Çàùèòà àêêàóíòà', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				imgui.InputTextMultiline('##kotofee1vnoyob', zawitabinder, imgui.ImVec2(650, 250))
				imgui.Text(u8 'Çàäåðæêà ìåæäó îòïðàâêîé ñòðîê:')
				imgui.SameLine()
				imgui.PushItemWidth(100)
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//zawitabinder.txt", "w")
			...
			imgui.SameLine()
			if imgui.Button(u8 'Ìíîæèòåëü äîíàòà', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ìíîæèòåëü äîíàòà') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ìíîæèòåëü äîíàòà', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//donatebinder.txt", "w")
			...
			imgui.SameLine()
			if imgui.Button(u8 'Ïðàâèëà ñåðâåðà', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ïðàâèëà ñåðâåðà') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ïðàâèëà ñåðâåðà', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//rulesbinder.txt", "w")
			...
			if imgui.Button(u8 'Äèñêîðä', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Äèñêîðä') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Äèñêîðä', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//discordbinder.txt", "w")
			...
			imgui.SameLine()
			if imgui.Button(u8 'Ïàáëèê ÂÊ', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ïàáëèê ÂÊ') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ïàáëèê ÂÊ', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//groupbinder.txt", "w")
			...
			imgui.SameLine()
			if imgui.Button(u8 'Ñîáñòâåííûé ¹1', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ñîáñòâåííûé ¹1') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ñîáñòâåííûé ¹1', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				imgui.InputTextMultiline('##õóé', mybinder1, imgui.ImVec2(650, 250))
				imgui.Text(u8 'Çàäåðæêà ìåæäó îòïðàâêîé ñòðîê:')
				imgui.SameLine()
				imgui.PushItemWidth(100)
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder1.txt", "w")
			...
			if imgui.Button(u8 'Ñîáñòâåííûé ¹2', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ñîáñòâåííûé ¹2') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ñîáñòâåííûé ¹2', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder2.txt", "w")
			...
			imgui.SameLine()
			if imgui.Button(u8 'Ñîáñòâåííûé ¹3', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ñîáñòâåííûé ¹3') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ñîáñòâåííûé ¹3', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder3.txt", "w")
			...
			imgui.SameLine()
			if imgui.Button(u8 'Ñîáñòâåííûé ¹4', imgui.ImVec2(200, 22)) then
				lua_thread.create(function()
			...
			if imgui.SettingsButton() then imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ñîáñòâåííûé ¹4') end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Ñîáñòâåííûé ¹4', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
				imgui.InputTextMultiline('##kotofee1vnoyob', mybinder4, imgui.ImVec2(650, 250))
				imgui.Text(u8 'Çàäåðæêà ìåæäó îòïðàâêîé ñòðîê:')
				imgui.SameLine()
				imgui.PushItemWidth(100)
				imgui.InputInt(u8("##õóéîûàâîûàâûâô"), buffer41)
				...
				if imgui.Button(u8 'Ñîõðàíèòü è çàêðûòü', imgui.ImVec2(-1, 25)) then
					local file = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder4.txt", "w")
		...
		if menusel.v == 8 then
			...
			if imgui.Button(u8 'Íàñòðîéêè óâåäîìëåíèÿ î ïðàâèëüíîé ïîäà÷å æàëîáû', imgui.ImVec2(-1, 25)) then
				imgui.OpenPopup(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Óâåäîìëåíèå î ïîäà÷å æàëîáû')
			end
			if imgui.BeginPopupModal(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Óâåäîìëåíèå î ïîäà÷å æàëîáû', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
			...
			if imgui.BeginPopupModal(u8 'Flood', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
				imgui.InputText(u8 "##1244443555", buffer91)
				cfg.main.typeflood = string.format('%s', tostring(buffer91.v))
				imgui.PopItemWidth()
				imgui.Text(u8 'Óêàæèòå ñðîê íàêàçàíèÿ çà äàííîå íàðóøåíèå:')
				imgui.SameLine()
				imgui.PushItemWidth(100)
			...
			imgui.SameLine()
			if imgui.Button(u8 'DriveBy êîâøîì [/' .. cfg.main.typedbk .. ' ' .. cfg.main.dbk .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'DriveBy êîâøîì')
			end
			if imgui.BeginPopupModal(u8 'DriveBy êîâøîì', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			imgui.Button(u8 'Ñáèâ àíèìàöèè [warn]', imgui.ImVec2(330, 25))
			imgui.SameLine()
			if imgui.Button(u8 'Óïîìèíàíèå ðîäíè [/' .. cfg.main.typeupom .. ' ' .. cfg.main.upomrod .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'Óïîìèíàíèå ðîäíè')
			end
			if imgui.BeginPopupModal(u8 'Óïîìèíàíèå ðîäíè', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			if imgui.Button(u8 'Îñêîðáëåíèå ðîäíè [/' .. cfg.main.typeosk .. ' ' .. cfg.main.oskrod .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'Îñêîðáëåíèå ðîäíè')
			end
			if imgui.BeginPopupModal(u8 'Îñêîðáëåíèå ðîäíè', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			imgui.SameLine()
			if imgui.Button(u8 'Íåàäåêâàòíîå ïîâåäåíèå [/' .. cfg.main.typenead .. ' ' .. cfg.main.nead .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'Íåàäåêâàòíîå ïîâåäåíèå')
			end
			if imgui.BeginPopupModal(u8 'Íåàäåêâàòíîå ïîâåäåíèå', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			if imgui.Button(u8 'NonRolePlay òþíèíã [/' .. cfg.main.typentune .. ' ' .. cfg.main.ntune .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'NonRolePlay òþíèíã')
			end
			if imgui.BeginPopupModal(u8 'NonRolePlay òþíèíã', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			imgui.SameLine()
			if imgui.Button(u8 'Èñïîëüçîâàíèå çàïðåùåííûõ ïðîãðàìì [/' .. cfg.main.typecheat .. ' ' .. cfg.main.cheat .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'Èñïîëüçîâàíèå çàïðåùåííûõ ïðîãðàìì')
			end
			if imgui.BeginPopupModal(u8 'Èñïîëüçîâàíèå çàïðåùåííûõ ïðîãðàìì', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			if imgui.Button(u8 'NonRolePlay ïîëèöåéñêèé [/' .. cfg.main.typencop .. ' ' .. cfg.main.ncop .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'NonRolePlay ïîëèöåéñêèé')
			end
			if imgui.BeginPopupModal(u8 'NonRolePlay ïîëèöåéñêèé', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			imgui.SameLine()
			if imgui.Button(u8 'Îñêîðáëåíèå àäìèíèñòðàöèè [/' .. cfg.main.typeoskadm .. ' ' .. cfg.main.oskadm .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'Îñêîðáëåíèå àäìèíèñòðàöèè')
			end
			if imgui.BeginPopupModal(u8 'Îñêîðáëåíèå àäìèíèñòðàöèè', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			if imgui.Button(u8 'Ñáèâ àíèìàöèè ñòðåëüáû (+Ñ) [warn]', imgui.ImVec2(330, 25)) then

			end
			imgui.SameLine()
			if imgui.Button(u8 'Ìàññîâûé DeathMatch [/' .. cfg.main.typemdm .. ' ' .. cfg.main.mdm .. ']', imgui.ImVec2(330, 25)) then
				imgui.OpenPopup(u8 'Ìàññîâûé DeathMatch')
			end
			if imgui.BeginPopupModal(u8 'Ìàññîâûé DeathMatch', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
				imgui.Text(u8 "Óêàæèòå êîìàíäó äëÿ âûäà÷è íàêàçàíèÿ (áåç /):")
				imgui.PushItemWidth(90)
				imgui.SameLine()
			...
			imgui.Separator()
			imgui.PushFont(fs16)
			imgui.CenterTextColoredRGB('{A77CD0}Ïðî÷åå')
			imgui.PopFont()
			imgui.Text(u8 "/desc [ID] - óäàëèòü îïèñàíèå ïåðñîíàæà")

			imgui.EndChild()
		end
		if menusel.v == 2 then
			imgui.BeginChild("Childdw1", imgui.ImVec2(-1, -1), true)
			imgui.Text(fa.ICON_FA_COFFEE .. u8 ' Ñòèëü èíòåðôåéñà:')
			imgui.SameLine()
			...
			elseif result.v == 1 then
				cfg.main.theme = 'darkgreen'
				DarkGreen()
			elseif result.v == 2 then
				cfg.main.theme = 'red'
				Red()
			elseif result.v == 3 then
				cfg.main.theme = 'purple'
				Purple()
			elseif result.v == 4 then
				cfg.main.theme = 'dark'
				Dark()
			end
			imgui.Text(u8 'Çàêðóãëåíèå èíòåðôåéñà:')
			imgui.SameLine()
			imgui.PushItemWidth(100)
			...
			imgui.Text(u8 'Çàêðóãëåíèå êíîïîê:')
			imgui.SameLine()
			imgui.PushItemWidth(100)
		...
		if menusel.v == 4 then
			...
			imgui.SameLine()
			imgui.Text(u8 'Îòêðûòü ìåíþ âûäà÷è íàêàçàíèé')
			if imgui.ToggleButton(u8 'Àâòîìàòè÷åñêàÿ âûäà÷à ðàçáàíà', imgui.ImBool(cfg.main.autounban)) then
				cfg.main.autounban = not cfg.main.autounban
			end
			imgui.Text(u8 'Óêàæèòå çàäåðæêó ìåæäó âûäà÷åé ôîðì:')
			imgui.SameLine()
			imgui.PushItemWidth(100)
			...
			imgui.PopItemWidth()
			imgui.SameLine()
			imgui.Text(u8 'ìèëèñåê.')
			imgui.Text(u8 'Óêàæèòå ãîòîâûå ôîðìû:')
			imgui.InputTextMultiline("##inp", text_buffer1, imgui.ImVec2(-1, 375))
			if imgui.Button(u8("Çàïóñòèòü âûäà÷ó ôîðì"), imgui.ImVec2(-1, 25)) then
				givepun = true
				lua_thread.create(function()
					...
					givepun = false
					sampAddChatMessage(tag .. 'Âûäà÷à ôîðì îêîí÷åíà, âûäàíî ôîðì âñåãî: {E78284}' .. formsall, -1)
					formsall = 0
				end)
			end
			imgui.EndChild()
		...
		if menusel.v == 5 then
			...
			imgui.CenterTextColoredRGB('{fe4e4e}Ñòèëü èíôîðìàöèîííîé ïàíåëè â ñëåæêå')
			if imgui.RadioButton(u8 "Ïàíåëü èíôîðìàöèè â ðåêîíå ¹1", style_information, 1) then
				cfg.main.styleinformation = 1
			end
			if imgui.RadioButton(u8 "Ïàíåëü èíôîðìàöèè â ðåêîíå ¹2", style_information, 2) then
				cfg.main.styleinformation = 2
			end
			imgui.Separator()
			imgui.CenterTextColoredRGB('{fe4e4e}Ñòèëü ïàíåëè óïðàâëåíèÿ â ñëåæêå')
			if imgui.RadioButton(u8 "Ñòàíäàðòíàÿ ïàíåëü óïðàâëåíèÿ", style_controlpanel, 1) then
				cfg.main.stylerecon = 1
			end
			if imgui.RadioButton(u8 "Êàñòîìíàÿ ïàíåëü óïðàâëåíèÿ ¹1", style_controlpanel, 2) then
				cfg.main.stylerecon = 2
			end
			imgui.Separator()
			if imgui.ToggleButton(u8 'Ïåðåõîä ñëåæêè íà << >>', imgui.ImBool(cfg.main.perehodik)) then
				cfg.main.perehodik = not cfg.main.perehodik
			end
			if imgui.ToggleButton(u8 'Ñëàï ââåðõ/âíèç íà /\\ è \\/', imgui.ImBool(cfg.main.slapinrecon)) then
				cfg.main.slapinrecon = not cfg.main.slapinrecon
			end
			if imgui.ToggleButton(u8 'Ïàíåëü âçàèìîäåéñòâèÿ ñ èãðîêîì', imgui.ImBool(cfg.main.menurecon1)) then
				cfg.main.menurecon1 = not cfg.main.menurecon1
			end
			if imgui.ToggleButton(u8 'Ïàíåëü áûñòðîé âûäà÷è íàêàçàíèé', imgui.ImBool(cfg.main.menurecon)) then
				cfg.main.menurecon = not cfg.main.menurecon
			end
			imgui.Text(u8 'Ìåñòîïîëîæåíèå ïàíåëè âçàèìîäåéñòâèÿ ñ èãðîêîì:')
			imgui.SameLine()
			if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '##panelvzaim') then
				if ReconWindow.v then
					ChangePosWindowTwo.v = true
				else
					sampAddChatMessage(tag .. 'Çàéäèòå â ðåêîí äëÿ ïîëó÷åíèÿ âîçìîæíîñòè èçìåíåíèÿ äàííîãî ïàðàìåòðà.',
						-1)
				end
			end
			imgui.Text(u8 'Ìåñòîïîëîæåíèå ïàíåëè î÷åðåäè ñëåæêè:')
			imgui.SameLine()
			if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '##ocheredslezhka') then
				if reconusers_state.v then
					...
					sampAddChatMessage(
						tag .. 'Äëÿ èçìåíåíèÿ äàííîãî ïàðàìåòðà ó âàñ äîëæíà áûòü îòêðûòà ïàíåëü î÷åðåäè ñëåæêè.', -1)
				end
			end
			imgui.Text(u8 'Ìåñòîïîëîæåíèå ïàíåëè áûñòðîé âûäà÷è íàêàçàíèå:')
			imgui.SameLine()
			if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '##saddsaejjadd') then
				if PunishWindow.v then
					changePositionRecon = true
					MainWindow.v = false
					sampAddChatMessage(tag .. 'Äëÿ ïîäòâåðæäåíèÿ èçìåíåíèÿ íàæìèòå 1, ÷òîáû îòìåíèòü ñîõðàíåíèå - 2.', -1)
				else
					sampAddChatMessage(tag .. 'Çàéäèòå â ðåêîí äëÿ ïîëó÷åíèÿ âîçìîæíîñòè èçìåíåíèÿ äàííîãî ïàðàìåòðà.',
						-1)
				end
			end
			imgui.Text(u8 'Ìåñòîïîëîæåíèå èíôîðìàöèîííîé ïàíåëè:')
			imgui.SameLine()
			if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '##infaslezhka') then
				if InfoWindow.v then
					...
					sampAddChatMessage(
						tag .. 'Äëÿ èçìåíåíèÿ äàííîãî ïàðàìåòðà ó âàñ äîëæíà áûòü âêëþ÷åíà èíôîðìàöèîííàÿ ïàíåëü.', -1)
				end
			end
			imgui.Separator()
			imgui.EndChild()
		...
		if menusel.v == 10 then
			...
			imgui.Text(u8 'Òýãè:')
			imgui.Text(u8 '{id_rep} - ID ñ ðåïîðòà')
			imgui.Text(u8 '{nick_rep} - NICK ñ ðåïîðòà áåç íèæíåãî ïîä÷åðêèâàíèÿ')
			imgui.Text(u8 '{my_id} - ñîáñòâåííûé ID')
			imgui.Text(u8 '{my_name} - ñîáñòâåííûé NICK')
			imgui.Separator()
			imgui.Text(u8 'Ñëåæó çà íàðóø:')
			imgui.SameLine()
			...
			imgui.VoidText(reason_settings_report[4].v, 'Ïîëå íå äîëæíî áûòü ïóñòîå!')
			imgui.Text(u8 'Ïîìî÷ü àâòîðó:')
			imgui.SameLine()
			...
			imgui.VoidText(reason_settings_report[2].v, 'Ïîëå íå äîëæíî áûòü ïóñòîå!')
			imgui.Text(u8 'Ïåðåäàë ðåï. àäì:')
			imgui.SameLine()
			...
			imgui.VoidText(reason_settings_report[9].v, 'Ïîëå íå äîëæíî áûòü ïóñòîå!')
			imgui.Separator()
			imgui.Text(u8 'Âûäàòü èãðîêó ÍÐÃ:')
			imgui.SameLine()
			...
			imgui.VoidText(reason_settings_report[1].v, 'Ïîëå íå äîëæíî áûòü ïóñòîå!')
			imgui.Text(u8 'Âûäàòü ëèöåíçèè:')
			imgui.SameLine()
			imgui.PushItemWidth(-1)
			...
			imgui.VoidText(reason_settings_report[5].v, 'Ïîëå íå äîëæíî áûòü ïóñòîå!')
			imgui.Text(u8 'Çàñïàâíèòü:')
			imgui.SameLine()
			imgui.PushItemWidth(-1)
			...
			imgui.VoidText(reason_settings_report[6].v, 'Ïîëå íå äîëæíî áûòü ïóñòîå!')
			imgui.Text(u8 'Âûäàòü ÕÏ:')
			imgui.SameLine()
			imgui.PushItemWidth(-1)
			...
			imgui.VoidText(reason_settings_report[7].v, 'Ïîëå íå äîëæíî áûòü ïóñòîå!')
			imgui.Separator()
			if imgui.Button(u8("Äîáàâèòü êíîïêó îòâåòà â ðåïîðò"), imgui.ImVec2(-1, 25)) then
				inputreportaddname.v = ""
				inputreportaddotvet.v = ""
				comboStylereport.v = 1
				retpor_text_edit = false
				imgui.OpenPopup(u8("Äîáàâëåíèå êíîïêè"))
			end
			imgui.Separator()

			viborreps = {
				"Ìîìåíòàëüíî",
				"Ñ ïîìîùüþ êíîïêè ÎÒÏÐÀÂÈÒÜ"
			}

			for ui, op in pairs(cfg.reportname) do
				imgui.Text(u8("Êíîïêà: " .. cfg.reportname[ui]))
				...
				if cfg.reportotvet[ui] then
					imgui.Text(u8("Îòâåò: " .. cfg.reportotvet[ui]))
					imgui.Text(u8("Îòïðàâêà: " .. viborreps[tonumber(cfg.reportstyle[ui]) + 1]))
					imgui.Separator()
				end
			end

			if imgui.BeginPopupModal(u8("Äîáàâëåíèå êíîïêè"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				imgui.Text(u8("Îòïðàâëÿòü ðåïîðò: "))
				imgui.SameLine()
				imgui.PushItemWidth(260)

				if imgui.Combo(u8("##viborf"), comboStylereport, {
						u8("Ìîìåíòàëüíî"),
						u8("Òîëüêî ñ ïîìîùüþ êíîïêè ÎÒÏÐÀÂÈÒÜ")
					}) then
				...
				imgui.SameLine()
				imgui.Text(u8("Îòâåò ïî íàæàòèþ êíîïêè"))
				imgui.Separator()
				imgui.PopItemWidth()

				if retpor_text_edit then
					if imgui.Button(u8("Èçìåíèòü"), imgui.ImVec2(400, 20)) then
						if inputreportaddname.v == "" or inputreportaddname.v == nil or inputreportaddotvet.v == nil or inputreportaddotvet.v == "" then
							...
							sampAddChatMessage("[Îøèáêà] Çàïîëíèòå âñå ïóíêòû!", 15352900)
							imgui.PopStyleColor()
						else
							...
							sampAddChatMessage(
								"Âû óñïåøíî îòðåäàêòèðîâàëè êíîïêó \"" .. u8:decode(inputreportaddname.v) .. "\"",
								15352900)
				...
				elseif imgui.Button(u8("Äîáàâèòü"), imgui.ImVec2(400, 20)) then
					if inputreportaddname.v == "" or inputreportaddname.v == nil or inputreportaddotvet.v == nil or inputreportaddotvet.v == "" then
						imgui.PushStyleColor(imgui.Col.Text, imgui.ImColor(253, 65, 63, 255):GetVec4())
						sampAddChatMessage("[Îøèáêà] Çàïîëíèòå âñå ïóíêòû!", 15352900)
						imgui.PopStyleColor()
						...
						inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
						sampAddChatMessage("Âû óñïåøíî äîáàâèëè êíîïêó \"" .. u8:decode(inputreportaddname.v) .. "\"",
							15352900)
				...
				if imgui.Button(u8("Çàêðûòü"), imgui.ImVec2(400, 20)) then
					imgui.CloseCurrentPopup()
				end
		...
		if menusel.v == 9 then
			...
			imgui.Text(u8 'Áèíä ïðèíÿòèÿ:')
			imgui.SameLine()
			imadd.HotKey('##123123123', facceptform, {}, 100)
			imgui.SameLine()
			imgui.Text(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
			imgui.Hint('bindformi', u8 'Êíîïêà ïðèíÿòèÿ ôîðìû èç àäìèí-÷àòà')
			if imgui.ToggleButton(u8 'Îïîâåùåíèå â àäìèí ÷àò ïî ïðèíÿòèþ ôîðìû', imgui.ImBool(cfg.main.formaplus)) then
				cfg.main.formaplus = not cfg.main.formaplus
			end
			imgui.Separator()
			imgui.CenterText(fa.ICON_FA_BELL .. u8 ' Çâóêîâîå îïîâåùåíèå ' .. fa.ICON_FA_BELL)
			imgui.NewLine()
			if imgui.ToggleButton(u8 'Çâóêîâîå îïîâåùåíèå ïðè ïîëó÷åíèè ôîðì', imgui.ImBool(cfg.main.formbell)) then
				cfg.main.formbell = not cfg.main.formbell
			end
			imgui.Text(u8 'Íàçâàíèå çâóêîâîãî ôàéëà:')
			imgui.SameLine()
			imgui.PushItemWidth(100)
			imgui.InputText(u8 "##1", nameaudio)
			cfg.main.nameaudioforform = string.format('%s', tostring(nameaudio.v))
			imgui.PopItemWidth()
			imgui.Text(u8 'Ãðîìêîñòü îïîâåùåíèÿ:')
			imgui.SameLine()
			imgui.PushItemWidth(100)
		...
		if menusel.v == 13 then
			imgui.BeginChild("ChildWindow177", imgui.ImVec2(-1, -1), true)
			ArenaHelper()
			imgui.EndChild()
		end
		if menusel.v == 14 then
			imgui.BeginChild("ChildWindow7", imgui.ImVec2(-1, -1), true)
			imgui.PushFont(fs16)
			imgui.CenterTextColoredRGB('Àâòîð ñêðèïòà:')
			imgui.PopFont()
			imgui.PushFont(fs80)
			...
			imgui.PopFont()
			imgui.PushFont(fs16)
			imgui.CenterTextColoredRGB('Ïîìîùü â ðàçðàáîòêå:')
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('rPoMoB').x) / 2)
			imgui.TextColored(imgui.ImVec4(rainbowtext(4)), 'kotofeev')
			imgui.PopFont()
			imgui.PushFont(fs16)
			imgui.CenterTextColoredRGB('Ñåìåéíîå ôîòî:')
			imgui.PopFont()
		...
		if menusel.v == 6 then
			...
			if imgui.Button(u8 'Âõîäû', imgui.ImVec2(131, 40)) then
				checker_selected.v = 1
			end
			imgui.SameLine()
			if imgui.Button(u8 'Àäìèíû', imgui.ImVec2(131, 40)) then
				checker_selected.v = 2
			end
			imgui.SameLine()
			if imgui.Button(u8 'Èãðîêè', imgui.ImVec2(131, 40)) then
				checker_selected.v = 3
			end
			imgui.SameLine()
			if imgui.Button(u8 'Ðåãèñòðàöèè', imgui.ImVec2(131, 40)) then
				checker_selected.v = 4
			end
			imgui.SameLine()
			if imgui.Button(u8 'Âûõîäû', imgui.ImVec2(131, 40)) then
				checker_selected.v = 5
			end
			imgui.Separator()
			if checker_selected.v == 1 then
				imgui.PushFont(fs16)
				imgui.CenterTextColoredRGB('{E3C47F}×åêåð âõîäîâ')
				imgui.PopFont()
				if imgui.ToggleButton(u8 'Âêëþ÷èòü ÷åêåð ïîäêëþ÷åíèé', imgui.ImBool(cfg.main.connectChecker)) then
					cfg.main.connectChecker = not cfg.main.connectChecker
				end
				...
				if imgui.InputInt(u8("##asdsadjrejasd"), ConnectBufferSize) then
					...
					connect = renderCreateFont(cfg.main.QuitFontChecker, cfg.main.connectSize, cfg.main.connectStyle,
						FCR_BOLD + FCR_BORDER)
				end
				imgui.PopItemWidth()
				...
				imgui.PopItemWidth()
				imgui.Text(u8 'Îòñòóï ìåæäó ñòðîêàìè:')
				imgui.SameLine()
				imgui.PushItemWidth(100)
				...
				if imgui.InputInt(u8("##connectLines"), ConnectBufferLines) then
					...
					leave = renderCreateFont(cfg.main.ConnectFontChecker, cfg.main.leaveSize, cfg.main.leaveStyle,
						FCR_BOLD + FCR_BORDER)
				end
				imgui.PopItemWidth()
				...
				if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '') then
					if not editPosition2 then
						...
						MainWindow.v = false
						lua_thread.create(function()
							while editPosition2 do
								wait(0)
								sampSetCursorMode(CMODE_LOCKCAM)
								local x, y = getCursorPos()
								cfg.connectPosition.x, cfg.connectPosition.y = x, y
								if isKeyDown(VK_LBUTTON) then
									sampAddChatMessage(tag .. 'Ïîçèöèÿ ÷åêåðà óñïåøíî ñîõðàíåíà.', -1)
									inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
						...
						sampAddChatMessage("Äëÿ íà÷àëà çàâåðøèòå ïðîøåäøåå ðåäàêòèðîâàíèå.", -1)
					end
				end
			elseif checker_selected.v == 2 then
				imgui.PushFont(fs16)
				imgui.CenterTextColoredRGB('{E3C47F}×åêåð àäìèíèñòðàöèè')
				imgui.PopFont()
			elseif checker_selected.v == 3 then
				imgui.PushFont(fs16)
				imgui.CenterTextColoredRGB('{E3C47F}×åêåð èãðîêîâ')
				imgui.PopFont()
				if imgui.ToggleButton(u8 'Âêëþ÷èòü ÷åêåð èãðîêîâ', imgui.ImBool(cfg.main.playersChecker)) then
					cfg.main.playersChecker = not cfg.main.playersChecker
				end
				if imgui.ToggleButton(u8 'Îòîáðàæàòü AFK â ÷åêåðå', imgui.ImBool(cfg.main.afkinchecker)) then
					cfg.main.afkinchecker = not cfg.main.afkinchecker
				end
				imadd.HotKey('##fchecker', fchecker, {}, 100)
				imgui.SameLine()
				imgui.Text(u8 'Âêëþ÷èòü/âûêëþ÷èòü ÷åêåð')
				imgui.Text(u8 'Ìåñòîïîëîæåíèå:')
				imgui.SameLine()
				if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 "") then
							...
							while editPosition do
								...
								if isKeyDown(VK_LBUTTON) then
									...
									for k, v in pairs(J_) do
										if doesFileExist(getWorkingDirectory() .. "\\AdminToolsGromov\\" .. J_[k][2] .. ".json") then
											local f = io.open(
												getWorkingDirectory() .. "\\AdminToolsGromov\\" .. J_[k][2] .. ".json",
												"w+")
											if f then
				...
				imgui.Separator()
				imgui.CenterTextColoredRGB('{fe4e4e}Íàñòðîéêè ñòèëÿ ÷åêåðà')
				if imgui.ColorEdit3(u8 'Ñòàíäàðòíûé öâåò íèêîâ', stockcolorchecker, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
					stock_colorchecker()
				end
				if imgui.ColorEdit3(u8 'Öâåò AFK', afkcolorchecker, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
					afk_colorchecker()
				end
				imgui.Text(u8 'Øðèôò ÷åêåðà:')
				imgui.SameLine()
				...
				if imgui.InputText('##fontchecker', fontchecker) then
					...
					fonts = {
						playersChecker = renderCreateFont(cfg.main.fontchecker, cfg.main.sizechecker,
							cfg.main.stylechecker, FCR_BOLD + FCR_SHADOW)
					}
				end
				imgui.PopItemWidth()
				imgui.Text(u8 'Ðàçìåð øðèôòà:')
				...
				imgui.CenterText(u8 'Äîáàâëåíèå èãðîêà')
				imgui.Separator()
				...
				if imgui.IsItemClicked() then
					if addPlayerInChecker ~= "" then
						...
						for k, v in pairs(J_) do
							if doesFileExist(getWorkingDirectory() .. "\\AdminToolsGromov\\" .. J_[k][2] .. ".json") then
								local f = io.open(getWorkingDirectory() .. "\\AdminToolsGromov\\" .. J_[k][2] .. ".json",
									"w+")
								if f then
									f:write(encodeJson(J_[k][1])):close()
								end
						...
						sampAddChatMessage("Ââåäèòå íèêíåéì èãðîêà!")
					end
				end
				imgui.EndChild()
				imgui.SetCursorPosX(width / 2 - 95)
				if #J_.PLAYERS_CHECKER[1] == 0 then
					imgui.CenterText(u8 "Ïîêà ÷òî âû íå äîáàâèëè íè îäíîãî èãðîêà â ÷åêåð")
				end
			...
			elseif checker_selected.v == 4 then
				imgui.PushFont(fs16)
				imgui.CenterTextColoredRGB('{E3C47F}×åêåð ðåãèñòðàöèé')
				imgui.PopFont()
				if imgui.ToggleButton(u8 'Âêëþ÷èòü ÷åêåð ðåãèñòðàöèé', imgui.ImBool(cfg.main.regChecker)) then
					cfg.main.regChecker = not cfg.main.regChecker
				end
				imgui.Text(u8 'Ðàçìåð øðèôòà:')
				imgui.SameLine()
				imgui.PushItemWidth(100)
				if imgui.InputInt(u8("##ôûâóôâûâûôó"), RegBufferSize) then
					cfg.main.regSize = string.format('%s', tostring(RegBufferSize.v))
					reg = renderCreateFont(cfg.main.QuitFontChecker, cfg.main.regSize, cfg.main.regStyle,
						FCR_BOLD + FCR_BORDER)
				end
				imgui.PopItemWidth()
				...
				if imgui.InputText('##RegFontChecker', RegFontChecker) then
					...
					reg = renderCreateFont(cfg.main.RegFontChecker, cfg.main.regSize, cfg.main.regStyle,
						FCR_BOLD + FCR_BORDER)
				end
				imgui.PopItemWidth()
				imgui.Text(u8 'Îòñòóï ìåæäó ñòðîêàìè:')
				...
				if imgui.InputInt(u8("##connectLines"), ConnectBufferLines) then
					cfg.main.connectLines = string.format('%s', tostring(ConnectBufferLines.v))
					leave = renderCreateFont(cfg.main.RegFontChecker, cfg.main.regSize, cfg.main.regStyle,
						FCR_BOLD + FCR_BORDER)
				end
				imgui.PopItemWidth()
				if ConnectBufferLines.v <= 1 then
			...
			elseif checker_selected.v == 5 then
				imgui.PushFont(fs16)
				imgui.CenterTextColoredRGB('{E3C47F}×åêåð âûõîäîâ')
				imgui.PopFont()
				if imgui.ToggleButton(u8 'Âêëþ÷èòü ÷åêåð îòêëþ÷åíèé', imgui.ImBool(cfg.main.leaveChecker)) then
					cfg.main.leaveChecker = not cfg.main.leaveChecker
				end
				imgui.Text(u8 'Ðàçìåð øðèôòà:')
				imgui.SameLine()
				...
				if imgui.InputInt(u8("##ssswsqssas"), LeaveBufferSize) then
					...
					leave = renderCreateFont(cfg.main.QuitFontChecker, cfg.main.leaveSize, cfg.main.leaveStyle,
						FCR_BOLD + FCR_BORDER)
				end
				imgui.PopItemWidth()
	...
	if AmemberWindow.v then
		...
		imgui.SetNextWindowPos(imgui.ImVec2((sw / 2), sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(u8 "Âûáåðèòå îðãàíèçàöèþ", AmemberWindow,
			imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)

		imgui.Text(u8 'Óêàæèòå ðàíã:')
		imgui.SameLine()
	...
	if TeleportWindow.v then
		...
		imgui.Begin('TP MENU', TeleportWindow,
			imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)

		imgui.BeginChild("Cindow", imgui.ImVec2(150, -1), true)
		if imgui.BoolButton(pizdas == 1, u8 'Èçáðàííîå', imgui.ImVec2(-1, 25)) then
			pizdas = 1
		end
		if imgui.BoolButton(pizdas == 2, u8 'Ôðàêöèè', imgui.ImVec2(-1, 25)) then
			pizdas = 2
		end
		if imgui.Button(u8 'ÒÏ ïî ìåòêå', imgui.ImVec2(-1, 25)) then
			_, x, y, z = getTargetBlipCoordinatesFixed()
			if not _ then
				sampAddChatMessage(tag .. "Ìåòêà äëÿ òåëåïîðòà {E78284}íå íàéäåíà{84A6E7}.", -1)
			else
				setCharCoordinates(PLAYER_PED, x, y, z)
			end
		end
		if imgui.Button(u8 'Òåëåïîðò íà ìàðêåð', imgui.ImVec2(-1, 25)) then
			tpcssss()
		end
		imgui.EndChild()
		imgui.SameLine()
		if pizdas == 1 then
			...
			for i = 1, #locations do
				if imgui.Button(u8(locations[i][1]), imgui.ImVec2(-1, 25)) then
					sampSendChat('/vv 0')
		...
		elseif pizdas == 2 then
			...
			if imgui.BoolButton(selectgroup == 1, u8 'Óëèöû', imgui.ImVec2(165, 30)) then
				selectgroup = 1
			end
			imgui.SameLine()
			if imgui.BoolButton(selectgroup == 2, u8 'Ñïàâíû', imgui.ImVec2(165, 30)) then
				selectgroup = 2
			end
			imgui.Separator()
			if selectgroup == 2 then
				for i = 1, #fractions do
					if imgui.Button('[' .. i .. '] ' .. u8('Ñïàâí') .. ' ' .. fractions[i][1], imgui.ImVec2(-1, 25)) then
						sampSendChat('/tp ' .. i)
					end
				end
			elseif selectgroup == 1 then
				for i = 1, #fractions do
					if imgui.Button('[' .. i .. '] ' .. u8('Óëèöà') .. ' ' .. fractions[i][1], imgui.ImVec2(-1, 25)) then
						sampSendChat('/vv 0')
	...
	if HelpWindow.v then
		...
		imgui.Begin(u8 ' Ïîìîùü ïî ñêðèïòó', HelpWindow,
			imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)
		imgui.CenterText(u8 '[Êîìàíäû Admin Tools]')
		imgui.NewLine()
		imgui.TextColoredRGB(
			'{E78284}/tpp {84A6E7}- òåëåïîðò èãðîêîâ ïî òî÷êàì [ARENA]\n{E78284}/rvanka {84A6E7}- âûäàòü áëîêèðîâêó IP çà ðâàíêó\n{E78284}/rekb {84A6E7}- âûäàòü áëîêèðîâêó IP çà ðåêëàìó\n{E78284}/cbiz {84A6E7}- îòêðûòü îòäåë êîììåðöèè\n{E78284}/chome {84A6E7}- îòêðûòü îòäåë íåäâèæèìîñòè\n{E78284}/nick [ID] {84A6E7}- êèêíóòü èãðîêà ñ ïðè÷èíîé nRP NickName\n{E78284}/om [ID] {84A6E7}- âûïóñòèòü èãðîêà ñ ïðè÷èíîé "Îïðîâåðæåíèå îòêëîíåíî"\n{E78284}/op [ID] {84A6E7}- âûïóñòèòü èãðîêà ñ ïðè÷èíîé "Îïðîâåðæåíèå îäîáðåíî"\n{E78284}/rlvl [radius] {84A6E7}- óçíàòü óðîâíè èãðîêîâ â ðàäèóñå (ïîëåçíî äëÿ ÷åêà ïóñòûøåê íà ñëåòå)\n{E78284}/gb {84A6E7}- ñîêðàùåíèå êîìàíäû /gotobiz\n{E78284}/gh {84A6E7}- ñîêðàùåíèå êîìàíäû /gotohouse\n{E78284}/ab {84A6E7}- ñîêðàùåíèå êîìàíäû /asellbiz\n{E78284}/ah {84A6E7}- ñîêðàùåíèå êîìàíäû /asellhouse\n{E78284}/rek_off {84A6E7}- Îòêëþ÷èòü/Âêëþ÷èòü ñîîáùåíèÿ [Ïîäîçðåíèå íà ðåêëàìó]\n{E78284}/crd {84A6E7}- ñêîïèðîâàòü êîîðäèíàòû íà êîòîðûõ âû ñåé÷àñ ñòîèòå\n{E78284}/cal [ïðèìåð]{84A6E7} - êàëüêóëÿòîð\n{E78284}/color {84A6E7} - òàáëèöà ÈÄîâ öâåòîâ äëÿ ïîêðàñêè àâòî\n{E78284}/admall {84A6E7} - ñïèñîê âñåé àäìèíèñòðàöèè ñåðâåðà\n{E78284}/clr [êîë-âî ñòðîê]{84A6E7} - âèçóàëüíî î÷èñòèòü ÷àò\n{E78284}/ammo {84A6E7}- ïîïîëíèòü ñêëàä àììóíèöèé\n{E78284}/arep {84A6E7}- ïî÷èíêà òðàíñïîðòà â êîòîðîì âû ñèäèòå\n{E78284}/amember {84A6E7}- îòêðûòü ìåíþ âñòóïëåíèÿ âî ôðàêöèþ\n{E78284}/spawnset {84A6E7}- óñòàíîâèòü òî÷êó ñïàâíà\n{E78284}/ss [id player] [id skin] {84A6E7}- âûäàòü âå÷íûé ñêèí è çàñïàâíèòü èãðîêà (äëÿ ðàçäà÷)\n{E78284}/tp {84A6E7}- îòêðûòü ìåíþ òåëåïîðòà\n{E78284}/spcarall [sek] {84A6E7}- íà÷àòü ñïàâí òðàíñïîðòà\n{E78284}/flood [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ôëóä\n{E78284}/rules {84A6E7}- îòïðàâèòü áèíäåð "Ïðàâèëà ñåðâåðà"\n{E78284}/dm [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ÄÌ\n{E78284}/db [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ÄÁ\n{E78284}/oskrod [id player] {84A6E7}- âûäàòü íàêàçàíèå çà îñêîðáëåíèå ðîäíûõ\n{E78284}/sbiv [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ñáèâ àíèìàöèè\n{E78284}/upomrod [id player] {84A6E7}- âûäàòü íàêàçàíèå çà óïîìèíàíèå ðîäíûõ\n{E78284}/dbk [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ÄÁ êîâøîì\n{E78284}/ncop [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ÍÐÏ êîïà\n{E78284}/cheat [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ÈÇÏ\n{E78284}/ntune [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ÍÐÏ òþíèíã\n{E78284}/nead [id player] {84A6E7}- âûäàòü íàêàçàíèå çà íåàäåêâàòíîå ïîâåäåíèå\n{E78284}/desc [id player] {84A6E7}- óäàëèòü îïèñàíèå è âûäàòü íàêàçàíèå çà ÍÐÏ îïèñàíèå\n{E78284}/protection {84A6E7}- îòïðàâèòü áèíäåð "Çàùèòà àêêàóíòà"\n{E78284}/oskadm [id player] {84A6E7}- âûäàòü íàêàçàíèå çà îñêîðáëåíèå àäìèíèñòðàöèè\n{E78284}/pc [id player] {84A6E7}- âûäàòü íàêàçàíèå çà +Ñ\n{E78284}/xdonate {84A6E7}- îòïðàâèòü áèíäåð "Ìíîæèòåëü äîíàòà"\n{E78284}/mdm [id player] {84A6E7}- âûäàòü íàêàçàíèå çà Ìàññîâûé ÄÌ\n{E78284}/sk [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ÑÊ\n{E78284}/tk [id player] {84A6E7}- âûäàòü íàêàçàíèå çà ÒÊ\n{E78284}/vk {84A6E7}- îòïðàâèòü áèíäåð "Ïàáëèê ÂÊ"\n{E78284}/capcha {84A6E7}- îòêðûòü ìåíþ óíèâåðñàëüíîé êàï÷è\n{E78284}/discord {84A6E7}- îòïðàâèòü áèíäåð "Äèñêîðä"\n{E78284}/my1 {84A6E7}- îòïðàâèòü áèíäåð "Ñîáñòâåííûé ¹1"\n{E78284}/my2 {84A6E7}- îòïðàâèòü áèíäåð "Ñîáñòâåííûé ¹2"\n{E78284}/my3 {84A6E7}- îòïðàâèòü áèíäåð "Ñîáñòâåííûé ¹3"\n{E78284}/my4 {84A6E7}- îòïðàâèòü áèíäåð "Ñîáñòâåííûé ¹4"\n{E78284}/tpcar [id player] [id car] {84A6E7}- òåëåïîðòèðîâàòü òðàíñïîðò ê èãðîêó\n{E78284}/amenu {84A6E7}- îòêðûòü íàñòðîéêè ñêðèïòà\n{E78284}/autoopra, /aopra{84A6E7} - âêëþ÷èòü àâòîîïðîâåðæåíèå\n{E78284}/toolsoff {84A6E7}- ïðèíóäèòåëüíî âûêëþ÷èòü ñêðèïò\n{E78284}/toolsreload {84A6E7}- ïðèíóäèòåëüíî ïåðåçàãðóçèòü ñêðèïò')
		imgui.End()
	end
	...
	if UpdateWindow.v then
		...
		imgui.PushFont(fs16)
		imgui.CenterTextColoredRGB("{FF4578}Äîñòóïíî îáíîâëåíèå!")
		imgui.PopFont()
		imgui.CenterTextColoredRGB("{cccccc}Ñïèñîê èçìåíåíèé áóäåò äîñòóïåí ê ïðîñìîòðó\nïîñëå óñòàíîâêè îáíîâëåíèÿ.")
		imgui.CenterTextColoredRGB("{fe4e4e}Âñå îáíîâëåíèÿ óñòàíàâëèâàþòñÿ èñêëþ÷èòåëüíî íà âàø ñòðàõ è ðèñê!")
		...
		if imgui.Button(u8 "Îòëîæèòü", imgui.ImVec2(150, 25)) then
		...
		if imgui.Button(u8 "Çàãðóçèòü îáíîâëåíèå", imgui.ImVec2(150, 25)) then
			update():download()
			UpdateWindow.v = false
		end
		imgui.End()
	...
	if CaptchaWindow.v then
		...
		imgui.Begin(fa.ICON_FA_COGS .. u8 ' Íàñòðîéêè | Êàï÷à', CaptchaWindow,
			imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)
		imgui.CenterText(u8 'Êóäà îòïðàâëÿòü êàï÷ó?')
		imgui.RadioButton(u8 "Â îáùèé ÷àò (/ao)", when_captcha, 1)
		imgui.RadioButton(u8 "Â VIP ÷àò (/vr)", when_captcha, 2)
		imgui.Text(u8 'Êàï÷à:')
		imgui.SameLine()
		imgui.PushItemWidth(-1)
		...
		imgui.PopItemWidth()
		if imgui.Button(u8 'Ñãåíåðèðîâàòü ðàíäîìíóþ êàï÷ó', imgui.ImVec2(-1, 25)) then
			if cfg.main.onzero then
				local a = math.random(1000, 9999) .. '0'
				sampAddChatMessage(tag .. 'Ñãåíåðèðîâàíà ðàíäîìíàÿ êàï÷à ñ îêîí÷àíèåì íà íîëü: ' .. a, -1)
				buffer27.v = a
			else
				local a = math.random(10000, 99999)
				sampAddChatMessage(tag .. 'Ñãåíåðèðîâàíà ðàíäîìíàÿ êàï÷à: ' .. a, -1)
				buffer27.v = a
			end
		end
		if imgui.ToggleButton(u8 'Ïÿòûé ñèìâîë â êàï÷å äîëæåí áûòü 0', imgui.ImBool(cfg.main.onzero)) then
			cfg.main.onzero = not cfg.main.onzero
		end
		imgui.BeginChild("qeuqewiqweiqwe5", imgui.ImVec2(-1, 60), true)
		imgui.PushFont(fontsize)
		imgui.CenterText(u8 'Ïðåäîñìîòð:')
		imgui.PopFont()
		if when_captcha.v == 2 then
			imgui.CenterText(u8('/vr Âíèìàíèå! Êàï÷à: "' .. u8:decode(cfg.main.kapchas)) ..
				u8('". Ïðèç: ' .. u8:decode(cfg.main.present)) .. '!')
		else
			imgui.CenterText(u8('/ao Âíèìàíèå! Êàï÷à: "' .. u8:decode(cfg.main.kapchas)) ..
				u8('". Ïðèç: ' .. u8:decode(cfg.main.present)) .. '!')
		end
		imgui.EndChild()
		if imgui.Button(u8 'Îòïðàâèòü êàï÷ó', imgui.ImVec2(-1, 25)) then
			if when_captcha.v == 2 then
				sampSendChat('/vr Âíèìàíèå! Êàï÷à: "' ..
					u8:decode(cfg.main.kapchas) .. '". Ïðèç: ' .. u8:decode(cfg.main.present) .. '!')
			else
				sampSendChat('/ao Âíèìàíèå! Êàï÷à: "' ..
					u8:decode(cfg.main.kapchas) .. '". Ïðèç: ' .. u8:decode(cfg.main.present) .. '!')
			end
		end
		imgui.End()
	...
	if InteractionWindow.v then
		...
		imgui.Begin(u8("##dasads"), InteractionWindow,
			imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)
		imgui.PushFont(fontsize)
		imgui.CenterText(u8 "Âçàèìîäåéñòâèå ñ èãðîêîì " .. nick)
		imgui.PopFont()
		imgui.BeginChild("qeuqewiqweiqwe5", imgui.ImVec2(220, 260), true)
		if imgui.Button(u8 'Ôëóä', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/mute ' .. nick .. ' ' .. cfg.main.flood .. ' Ôëóä')
		end
		imgui.SameLine()
		if imgui.Button(u8 'Îñê. ðîäíûõ', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/mute ' .. nick .. ' ' .. cfg.main.oskrod .. ' Îñêîðáëåíèå ðîäíûõ')
		end
		if imgui.Button(u8 'Óïîì. ðîäíûõ', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/mute ' .. nick .. ' ' .. cfg.main.upomrod .. ' Óïîìèíàíèå ðîäíûõ')
		end
		imgui.SameLine()
		if imgui.Button(u8 'Íåàäåêâàò', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/mute ' .. nick .. ' ' .. cfg.main.nead .. ' íåàäåêâàòíîå ïîâåäåíèå')
		end
		...
		if imgui.Button(u8 'DeathMatch', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/jail ' .. nick .. ' ' .. cfg.main.dm .. ' DeathMatch [ÄÌ]')
		end
		if imgui.Button(u8 'DriveBy', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/jail ' .. nick .. ' ' .. cfg.main.db .. ' DriveBy [ÄÁ]')
		end
		imgui.SameLine()
		if imgui.Button(u8 'ÈÇÏ', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/jail ' .. nick .. ', ' .. cfg.main.cheat .. ' ÈÇÏ')
		end
		if imgui.Button(u8 'DriveBy êîâøîì', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/jail ' .. nick .. ', ' .. cfg.main.dbk .. ' DriveBy êîâøîì')
		end
		imgui.SameLine()
		if imgui.Button(u8 'nRP òþíèíã', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/jail ' .. nick .. ', ' .. cfg.main.ntune .. ' nRP òþíèíã')
		end
		imgui.SameLine()
		if imgui.Button(u8 'Ìàññîâûé DM', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/ban ' .. nick .. ', ' .. cfg.main.mdm .. ' Ìàññîâûé DM')
		end
		if imgui.Button(u8 '+Ñ', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/warn ' .. nick .. ', +Ñ')
		end
		imgui.SameLine()
		if imgui.Button(u8 'Ñáèâ', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/warn ' .. nick .. ', Ñáèâ àíèìàöèè')
		end
		if imgui.Button(u8 'SpawnKill', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/warn ' .. nick .. ', ÑÊ')
		end
		imgui.SameLine()
		if imgui.Button(u8 'Óäàëèòü îïèñ.', imgui.ImVec2(100, 25)) then
			sampSendChat('/pm ' .. nick .. ', 1 Îïèñàíèå âàøåãî ïåðñîíàæà áûëî óäàëåíî. Ïðè÷èíà: nRP îïèñàíèå', -1)
		...
		if imgui.Button(u8 '/pm Âû òóò?', imgui.ImVec2(-1, 25)) then
			sampSendChat('/pm ' .. muteid .. ' 0 Óâàæàåìûé ' .. nick .. ', Âû òóò? Îòâåòüòå "Äà" â /b (/n) ÷àò.')
		end
		imgui.Separator()
		imgui.Columns(2, "Columns1", true)
		imgui.Text(u8 'Óðîâåíü')
		imgui.NextColumn()
		...
		if imgui.Button(u8 'Ïîìåõà', imgui.ImVec2(100, 25)) then
			sampSendChat('/kick ' .. nick .. ' ïîìåõà')
		end
		imgui.SameLine()
		if imgui.Button(u8 'Òèõèé êèê', imgui.ImVec2(100, 25)) then
			sampSendChat('/skick ' .. nick)
		end
		if imgui.Button(u8 'Âûäàòü NRG-500', imgui.ImVec2(100, 25)) then
			sampSendChat('/plveh ' .. nick .. ' 522 1')
		end
		imgui.SameLine()
		if imgui.Button(u8 'Ôëèï', imgui.ImVec2(100, 25)) then
		...
		imgui.SameLine()
		if imgui.Button(u8 'Çàáðàòü îðóæèå', imgui.ImVec2(100, 25)) then
			sampSendChat('/weap ' .. muteid)
		end
		if imgui.Button(u8 'Çàìîðîçèòü', imgui.ImVec2(100, 25)) then
			sampSendChat('/freeze ' .. muteid)
		end
		imgui.SameLine()
		if imgui.Button(u8 'Ðàçìîðîçèòü', imgui.ImVec2(100, 25)) then
	...
	if ColorsWindow.v then
		...
		imgui.Begin(u8 'Òàáëèöà öâåòîâ', ColorsWindow,
	...
	if TeleportPlayerWindow.v then
		local sw, sh = getScreenResolution()
		imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(550, 480), imgui.Cond.FirstUseEver)
		imgui.Begin(fa.ICON_FA_TH_LIST .. u8 ' Òåëåïîðò èãðîêîâ | STools', TeleportPlayerWindow,
			imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoSavedSettings)
		imgui.Text(u8 'Ââåäèòå èä èãðîêà:')
		imgui.SameLine()
		imgui.PushItemWidth(-1)
		imgui.InputInt(u8 "Ââåäèòå èä èãðîêà:#tpwindow", buffer104, 0)
		imgui.PopItemWidth()
		local result = sampIsPlayerConnected(buffer104.v)
		if not result then
			imgui.Text(u8 'Èãðîê ïîä òàêèì ID íå â ñåòè!')
		else
			imgui.Text(u8 'Âûáðàííûé èãðîê: ' .. sampGetPlayerNickname(buffer104.v) .. '[' .. buffer104.v .. ']')
		end
		imgui.Separator()
		if imgui.Button(u8 'Òåëåïîðò íà ïîïóëÿðíûå òî÷êè', imgui.ImVec2(530, 20)) then
			imgui.OpenPopup(u8 'Ïîïóëÿðíûå òî÷êè')
		end
		if imgui.BeginPopupModal(u8 'Ïîïóëÿðíûå òî÷êè', TeleportPlayerWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
			for i = 1, #locations do
				if imgui.Button(u8(locations[i][1]), imgui.ImVec2(530, 20)) then
					sampAddChatMessage(
						tag ..
						'Òåëåïîðòèðóþ èãðîêà ' ..
						sampGetPlayerNickname(buffer104.v) .. '[' .. buffer104.v .. '] íà òî÷êó ' .. locations[i][1], -1)
					lua_thread.create(function()
						sampSendChat('/tpcor ' ..
							buffer104.v .. ' ' .. locations[i][2] .. ' ' .. locations[i][3] .. ' ' .. locations[i][4])
						wait(600)
						sampSendChat('/fakesms ' ..
							buffer104.v ..
							' Âû áûëè òåëåïîðòèðîâàíû íà òî÷êó ' ..
							locations[i][1] .. '. Ïðèÿòíîé èãðû by samsa center <3')
						sampAddChatMessage(tag .. 'Òåëåïîðò ïðîøåë óñïåøíî!', -1)
					end)
					imgui.CloseCurrentPopup()
				end
			end
		...
		imgui.Separator()
		imgui.PushItemWidth(-1)
		imgui.CenterTextColoredRGB('{ff5606}Òåëåïîðò íà äîì/áèçíåñ')
		imgui.PopItemWidth()
		imgui.RadioButton(u8 "Äîì", when_tp, 1)
		imgui.RadioButton(u8 "Áèçíåñ", when_tp, 2)
		imgui.Text(u8 'Ââåäèòå èä èìóùåñòâà:')
		imgui.SameLine()
		imgui.PushItemWidth(-1)
		imgui.InputText(u8 "Ââåäèòå èä èìóùåñòâà:#tpwindow", buffer105)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Òåëåïîðòèðîâàòü íà èìóùåñòâî', imgui.ImVec2(530, 30)) then
			lua_thread.create(function()
				if when_tp.v == 1 then
					sampSendChat('/findihouse ' .. buffer105.v)
					test__ = 'äîìà'
				elseif when_tp.v == 2 then
					sampSendChat('/findibiz ' .. buffer105.v)
					test__ = 'áèçíåñà'
				end
				wait(1000)
				_, x, y, z = SearchMarker()
				if not _ then
					sampAddChatMessage(tag .. 'Íå óäàëîñü îáíàðóæèòü ìàðêåð èìóùåñòâà', -1)
				else
					sampAddChatMessage(
						tag .. 'Òåëåïîðòèðóþ èãðîêà ' .. sampGetPlayerNickname(buffer104.v) .. '[' .. buffer104.v .. ']',
						-1)
					sampSendChat('/tpcor ' .. buffer104.v .. ' ' .. x .. ' ' .. y .. ' ' .. z)
					wait(600)
					sampSendChat('/fakesms ' ..
						buffer104.v ..
						' Âû áûëè òåëåïîðòèðîâàíû íà òî÷êó ' ..
						test__ .. ' ' .. buffer105.v .. '. Ïðèÿòíîé èãðû by samsa center <3')
					sampAddChatMessage(tag .. 'Òåëåïîðò ïðîøåë óñïåøíî!', -1)
				end
			end)
		end
	...
	if ReconWindow.v and cfg.main.menurecon1 then
		...
		if style_controlpanel.v == 1 then
			...
			imgui.SetNextWindowSize(imgui.ImVec2(480, 270), imgui.Cond.FirstUseEver)
			imgui.Begin(u8("##reconPanel2"), _,
				imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove +
				imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
			...
			if imgui.Button(fa.ICON_FA_BACKWARD .. u8 "" .. fa.ICON_FA_BACKWARD, imgui.ImVec2(50, 20)) then
				lua_thread.create(function()
					if spec_id - 1 < 0 then
						sampAddChatMessage("BACK ID = -1 | Äîñòóïíî òîëüêî NEXT", 16711680)
			...
			if imgui.Button(u8("Ðàçìîðîçèòü")) then
				sampSendChat("/unfreeze " .. spec_id)
			end

			imgui.SameLine()

			if imgui.Button(u8("Çàìîðîçèòü")) then
				sampSendChat("/freeze " .. spec_id)
			end

			imgui.SetCursorPos(imgui.ImVec2(58, 33))

			if imgui.Button(u8("ÒÏ ê ñåáå")) then
				lua_thread.create(function()
					sampSendChat('/reoff')
					wait(500)
			...
			if imgui.Button(u8("Âûäàòü HP")) then
				sampSetChatInputEnabled(true)
			...
			if imgui.Button(u8("Íàêàçàíèÿ")) then
			...
			if imgui.Button(fa.ICON_FA_FORWARD .. u8 "" .. fa.ICON_FA_FORWARD, imgui.ImVec2(50, 20)) then
				lua_thread.create(function()
					if spec_id + 1 > 999 then
						sampAddChatMessage("NEXT ID = 1000 | Äîñòóïíî òîëüêî BACK", 16711680)
			...
			if imgui.Button(u8("Âû òóò?")) then
				sampSendChat('/pm ' .. spec_id .. ' 0 Óâàæàåìûé ' .. nick .. ', Âû òóò? Îòâåòüòå "Äà" â /b (/n) ÷àò.')
			end
			...
			if imgui.Button(u8("Ôëèï"), imgui.ImVec2(50, 21)) then
				sampSendChat("/flip " .. spec_id)
			end

			imgui.SameLine()

			if imgui.Button(u8("ÒÏ ê èãðîêó")) then
				lua_thread.create(function()
					sampSendChat('/reoff')
					wait(500)
		...
		if style_controlpanel.v == 2 then
			...
			imgui.Begin(u8("##reconPanel2"), _,
				imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove +
				imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
			if imgui.Button(fa.ICON_FA_BACKWARD .. u8 ' Íàçàä', imgui.ImVec2(130, 20)) then
				lua_thread.create(function()
					if spec_id - 1 < 0 then
						sampAddChatMessage("BACK ID = -1 | Äîñòóïíî òîëüêî NEXT", 16711680)
						return
			...
			if imgui.Button(fa.ICON_FA_TH_LIST .. u8 ' Ñòàòèñòèêà', imgui.ImVec2(130, 20)) then
				sampSendChat('/check ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_REGISTERED .. u8 ' Ðåã. äàííûå', imgui.ImVec2(130, 20)) then
				sampSendChat('/getip ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_HOME .. u8 ' Çàñïàâíèòü', imgui.ImVec2(130, 20)) then
				sampSendChat('/spplayer ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_ARROW_UP .. u8 ' Ñëàï ââåðõ', imgui.ImVec2(130, 20)) then
				sampSendChat('/slap ' .. spec_id .. ' 1')
			end
			if imgui.Button(fa.ICON_FA_ARROW_DOWN .. u8 ' Ñëàï âíèç', imgui.ImVec2(130, 20)) then
				sampSendChat('/slap ' .. spec_id .. ' 2')
			end
			if imgui.Button(fa.ICON_FA_SNOWFLAKE .. u8 ' Çàìîðîçèòü', imgui.ImVec2(130, 20)) then
				sampSendChat('/freeze ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_SNOWFLAKE .. u8 ' Ðàçìîðîçèòü', imgui.ImVec2(130, 20)) then
				sampSendChat('/unfreeze ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_LEVEL_DOWN_ALT .. u8 ' ÒÏ ê ñåáå', imgui.ImVec2(130, 20)) then
				lua_thread.create(function()
					sampSendChat('/reoff')
					wait(500)
					sampSendChat('/gethere ' .. spec_id)
			...
			if imgui.Button(fa.ICON_FA_MEDKIT .. u8 ' Âûäàòü HP', imgui.ImVec2(130, 20)) then
				sampSendChat('/sethp ' .. spec_id .. ' 100')
			end
			if imgui.Button(fa.ICON_FA_FIRE .. u8 ' Çàáðàòü îðóæèå', imgui.ImVec2(130, 20)) then
				sampSendChat('/weap ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_BAN .. u8 ' Íàêàçàíèÿ', imgui.ImVec2(130, 20)) then
				sampSendChat('/checkpunish ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_DUMBBELL .. u8 ' Íàâûêè', imgui.ImVec2(130, 20)) then
				sampSendChat('/checkskills ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_USER_SHIELD .. u8 ' Îðóæèå', imgui.ImVec2(130, 20)) then
				sampSendChat('/iwep ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_QUESTION .. u8 ' Âû òóò?', imgui.ImVec2(130, 20)) then
				sampSendChat('/pm ' .. spec_id .. ' 0 Óâàæàåìûé ' .. nick .. ', Âû òóò? Îòâåòüòå "Äà" â /b (/n) ÷àò.')
			end
			if imgui.Button(fa.ICON_FA_CAR .. u8 ' Âðåìåííûé ÒÑ', imgui.ImVec2(130, 20)) then
				sampSetChatInputEnabled(true)
				cursorotstup = 8
				if tonumber(spec_id) < 10 then
					cursorotstup = cursorotstup + 1
			...
			if imgui.Button(fa.ICON_FA_USER_FRIENDS .. u8 ' Ìóëüòèàêêàóíò', imgui.ImVec2(130, 20)) then
				sampSendChat('/pgetip ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_CAR_CRASH .. u8 ' Ôëèï', imgui.ImVec2(130, 20)) then
				sampSendChat('/flip ' .. spec_id)
			end
			if imgui.Button(fa.ICON_FA_LEVEL_UP_ALT .. u8 ' ÒÏ ê èãðîêó', imgui.ImVec2(130, 20)) then
				lua_thread.create(function()
					sampSendChat('/reoff')
					wait(500)
					sampSendChat('/g ' .. spec_id)
			...
			if imgui.Button(fa.ICON_FA_USER_LOCK .. u8 ' ÒÏ â AZ', imgui.ImVec2(130, 20)) then
				lua_thread.create(function()
					sampSendChat('/reoff')
					wait(500)
					sampSendChat('/az ' .. spec_id)
			...
			if imgui.Button(fa.ICON_FA_FORWARD .. u8 ' Âïåðåä', imgui.ImVec2(130, 20)) then
				lua_thread.create(function()
					if spec_id + 1 > 999 then
						sampAddChatMessage("NEXT ID = 1000 | Äîñòóïíî òîëüêî BACK", 16711680)
			...
			if imgui.Button(fa.ICON_FA_SYNC_ALT .. u8 ' Îáíîâèòü', imgui.ImVec2(130, 20)) then
				sampSendChat('/re ' .. spec_id .. '')
				printStringNow('UPDATE RECON', 1000)
			end
			if imgui.Button(fa.ICON_FA_EYE_SLASH .. u8 ' Âûéòè', imgui.ImVec2(130, 20)) then
				sampSendChat("/reoff")
				ReconWindow.v = false
				PunishWindow.v = false
				InfoWindow.v = false
	...
	if PunishWindow.v and cfg.main.menurecon then
		...
		imgui.Begin(u8("Áûñòðûå íàêàçàíèÿ"), _,
			imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove +
			imgui.WindowFlags.NoBringToFrontOnFocus + imgui.WindowFlags.NoSavedSettings)
		if imgui.Button(u8 'Âûäàòü KICK', imgui.ImVec2(-1, 20)) then
			imgui.OpenPopup(u8 'Íàêàçàíèå KICK')
		end
		if imgui.BeginPopupModal(u8 'Íàêàçàíèå KICK', PunishWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
			if imgui.Button(u8 'Óêàçàòü ñâîþ ïðè÷èíó', imgui.ImVec2(-1, 20)) then
				sampSetChatInputEnabled(true)
				sampSetChatInputText("/kick " .. spec_id .. " ")
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Òèõèé êèê', imgui.ImVec2(140, 20)) then
				sampSendChat('/skick ' .. spec_id)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Ïîìåõà', imgui.ImVec2(140, 20)) then
				sampSendChat('/kick ' .. spec_id .. ' ïîìåõà')
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Ïåðåçàéäèòå (ëàãè)', imgui.ImVec2(140, 20)) then
				sampSendChat('/kick ' .. spec_id .. ' ïåðåçàéäèòå (ëàãè)')
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'ÀÔÊ 50+', imgui.ImVec2(140, 20)) then
				sampSendChat('/kick ' .. spec_id .. ' AFK 50+')
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Çàêðûòü', imgui.ImVec2(140, 20)) then
				imgui.CloseCurrentPopup()
			end
			imgui.EndPopup()
		end
		if imgui.Button(u8 'Âûäàòü JAIL', imgui.ImVec2(-1, 20)) then
			imgui.OpenPopup(u8 'Íàêàçàíèå JAIL')
		end
		if imgui.BeginPopupModal(u8 'Íàêàçàíèå JAIL', PunishWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
			if imgui.Button(u8 'Óêàçàòü ñâîþ ïðè÷èíó', imgui.ImVec2(140, 20)) then
				sampSetChatInputEnabled(true)
				sampSetChatInputText("/jail " .. spec_id .. " ")
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'ÄÌ', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/jail " .. spec_id .. "  ÄÌ")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'ÄÌ ÇÇ', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/jail " .. spec_id .. "  ÄÌ ÇÇ")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'ÄÁ', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/jail " .. spec_id .. "  ÄÁ")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'ÄÌ ñëåò', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/jail " .. spec_id .. "  ÄÌ ñëåò")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'ÄÁ ñëåò', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/jail " .. spec_id .. "  ÄÁ ñëåò")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Çàêðûòü', imgui.ImVec2(140, 20)) then
		...
		if imgui.Button(u8 'Âûäàòü MUTE', imgui.ImVec2(-1, 20)) then
			imgui.OpenPopup(u8 'Íàêàçàíèå MUTE')
		end
		if imgui.BeginPopupModal(u8 'Íàêàçàíèå MUTE', PunishWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
			if imgui.Button(u8 'Óêàçàòü ñâîþ ïðè÷èíó', imgui.ImVec2(140, 20)) then
				sampSetChatInputEnabled(true)
				sampSetChatInputText("/mute " .. spec_id .. " ")
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Îñêîðáëåíèå', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/mute " .. spec_id .. "  îñêîðáëåíèå")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Íåàäåêâàò', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/mute " .. spec_id .. "  íåàäåêâàòíîå ïîâåäåíèå")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Òðàíñëèò', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/mute " .. spec_id .. "  òðàíñëèò")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Óïîì. ðîäíûõ', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/mute " .. spec_id .. "  óïîì. ðîäíûõ")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Îñê. ðîäíûõ', imgui.ImVec2(140, 20)) then
				otstupcursor()
				sampSetChatInputText("/mute " .. spec_id .. "  îñê. ðîäíûõ")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Çàêðûòü', imgui.ImVec2(140, 20)) then
		...
		if imgui.Button(u8 'Âûäàòü WARN', imgui.ImVec2(-1, 20)) then
			imgui.OpenPopup(u8 'Íàêàçàíèå WARN')
		end
		if imgui.BeginPopupModal(u8 'Íàêàçàíèå WARN', PunishWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
			if imgui.Button(u8 'Óêàçàòü ñâîþ ïðè÷èíó', imgui.ImVec2(-1, 20)) then
				sampSetChatInputEnabled(true)
				sampSetChatInputText("/warn " .. spec_id .. " ")
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'ÄÌ ÇÇ', imgui.ImVec2(-1, 20)) then
				sampSendChat('/warn ' .. spec_id .. ' ÄÌ ZZ')
			end
			if imgui.Button(u8 'nRP êîï', imgui.ImVec2(-1, 20)) then
				sampSendChat('/warn ' .. spec_id .. ' nRP cop')
			end
			if imgui.Button(u8 '+Ñ', imgui.ImVec2(-1, 20)) then
				sampSendChat('/warn ' .. spec_id .. ' +Ñ')
			end
			if imgui.Button(u8 'ÒÊ', imgui.ImVec2(-1, 20)) then
				sampSendChat('/warn ' .. spec_id .. ' ÒÊ')
			end
			if imgui.Button(u8 'ÑÊ', imgui.ImVec2(-1, 20)) then
		...
		if imgui.Button(u8 'Âûäàòü BAN', imgui.ImVec2(-1, 20)) then
			imgui.OpenPopup(u8 'Íàêàçàíèå BAN')
		end
		if imgui.BeginPopupModal(u8 'Íàêàçàíèå BAN', PunishWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
			if imgui.Button(u8 'Óêàçàòü ñâîþ ïðè÷èíó', imgui.ImVec2(-1, 20)) then
				sampSetChatInputEnabled(true)
				sampSetChatInputText("/ban " .. spec_id .. " ")
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Ðàêáîò', imgui.ImVec2(-1, 20)) then
				otstupcursor1()
				sampSetChatInputText("/ban " .. spec_id .. "  ðàêáîò")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Ðåêëàìà', imgui.ImVec2(-1, 20)) then
				otstupcursor1()
				sampSetChatInputText("/ban " .. spec_id .. "  ðåêëàìà")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'ÀÕÊ', imgui.ImVec2(-1, 20)) then
				otstupcursor1()
				sampSetChatInputText("/ban " .. spec_id .. "  ÀÕÊ")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Áîò', imgui.ImVec2(-1, 20)) then
				otstupcursor1()
				sampSetChatInputText("/ban " .. spec_id .. "  áîò")
				sampSetChatInputCursor(cursorotstup, cursorotstup)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Çàêðûòü', imgui.ImVec2(140, 20)) then
		...
		if imgui.Button(u8 'Âûäàòü ÒÑÐ', imgui.ImVec2(-1, 20)) then
			imgui.OpenPopup(u8 'Íàêàçàíèå ÒÑÐ')
		end
		if imgui.BeginPopupModal(u8 'Íàêàçàíèå ÒÑÐ', PunishWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
			if imgui.Button(u8 'Óêàçàòü ñâîþ ïðè÷èíó', imgui.ImVec2(-1, 20)) then
				sampSetChatInputEnabled(true)
				sampAddChatMessage(tag .. 'Íà Arizona Arena äàííàÿ ôóíêöèÿ íå ðàáîòàåò!', -1)
				imgui.CloseCurrentPopup()
			end
			if imgui.Button(u8 'Çàêðûòü', imgui.ImVec2(140, 20)) then
				imgui.CloseCurrentPopup()
	...
	if ChangeLogWindow.v then
		...
		imgui.Begin('##ChangeLogWindow', ChangeLogWindow,
			imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar +
			imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.ShowBorders)
		imgui.PushFont(infoFs)
		imgui.CenterTextColoredRGB('{E78284}Íîâîå îáíîâëåíèå!')
		imgui.PopFont()
		imgui.SameLine()
		local p = imgui.GetCursorScreenPos()
		local s = imgui.CalcTextSize('new')
		imgui.GetWindowDrawList():AddRectFilled(imgui.ImVec2(p.x - 4, p.y + 2),
			imgui.ImVec2(p.x + s.x + 2, p.y + s.y + 2), 0xFF0000FF, 10, 15)
		imgui.Text('new')
		imgui.CenterTextColoredRGB('{606060}Âåðñèÿ: {868686}' .. thisScript().version)
		...
		for count = 0, #changelog do
			...
			if #patches > 0 then
				imgui.TextColoredRGB(sc .. '>> Ñïèñîê èñïðàâëåíèé (' .. #patches .. ')')
				imgui.Hint('show_patches:' .. count,
					u8('Êëèêíèòå, ÷òî áû ' .. (changelog[count]['patches']['show'] and 'ñâåðíóòü' or 'ðàçâåðíóòü')))
				if imgui.IsItemClicked() then
				...
				if changelog[count]['patches']['show'] then
					...
					imgui.GetWindowDrawList():AddLine(imgui.ImVec2(p1.x + 20.5, p1.y + 2),
						imgui.ImVec2(p2.x + 20, p2.y - 2), 0xFF00C235, 2)
				end
			end
	...
	if ReportWindow.v then
		...
		imgui.Begin(u8 'Æàëîáà/Âîïðîñ', ReportWindow,
			imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove +
			imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize)
		imgui.Text(u8 "Æàëîáà îò: " .. nick_rep .. "[" .. report_id .. "]")
		imgui.SameLine()
		...
		imgui.Hint('slejkare', u8 'Ïåðåéòè â ñëåæêó çà àâòîðîì ðåïîðòà')
		imgui.SameLine(435)
		imgui.Text(fa.ICON_FA_WRENCH .. '')
		imgui.Hint('helpcur',
			u8 'Åñëè ó âàñ çàáàãàëñÿ êóðñîð, òî ïîôèêñèòü ïðîáëåìó\nìîæíî ïåðåâåäÿ ðàñêëàäêó íà àíãë. ÿç., ïîñëå îòêðîéòå ÷àò,\näàëåå çàêðîéòå åãî, è ïîëå ââîäà ñíîâà ñòàíåò äîñòóïíî!')
		imgui.Separator()
		imgui.Text(u8:encode(report_text))
		imgui.Separator()
		local otvet = text_buffer_rep.v
		if len(text_buffer_rep.v) > 55 then
			imgui.CenterTextColoredRGB('{fe4e4e} Îòâåò äîëæåí ñîäåðæàòü íå áîëåå 70 ñèìâîëîâ [' ..
				len(text_buffer_rep.v) .. '/70]')
		end
		...
		if len(text_buffer_rep.v) >= 70 then
			if imgui.Button(u8 'Ñáðîñèòü ïîëå ââîäà', imgui.ImVec2(444, 20)) then
				text_buffer_rep.v = ''
			end
		end
		imgui.Separator()
		if imgui.Button(fa.ICON_FA_ROBOT .. u8 ' Ñëåæó çà íàðóø', imgui.ImVec2(145, 20)) then
			if report_text:match("(%d+)") ~= nil then
				if sampIsPlayerConnected(report_text:gmatch("(%d+)")) then
					...
					sampAddChatMessage(
						"{808080}[Ñëåæêà] Âû íå ìîæåòå íà÷àòü ñëåäèòü çà íàðóøèòåëåì,ò.ê íå óêàçàí àéäè â ðåïîðòå èëè èãðîê îôôëàéí.",
						-1)
				end
			end
		end
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_PEOPLE_CARRY .. u8 ' Ïîìî÷ü àâòîðó', imgui.ImVec2(145, 20)) then
			local template = u8:decode(reason_settings_report[2].v)
			...
			ReportWindow.v = false
			sampSendChat('/re ' .. report_id)
			sampAddChatMessage("[Àâòîìàòè÷åñêàÿ ñëåæêà] Âû óøëè â ñëåæêó çà " .. nick_rep .. "[" .. report_id .. "]",
				14628149)
		end
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_COMMENTS .. u8 ' Ïåðåñëàòü â /a ÷àò', imgui.ImVec2(145, 20)) then
			sampSendChat("/a >>> [ÐÅÏÎÐÒ] " .. report_name .. "[" .. report_id .. "]: " .. report_text .. " ")
		end
		if imgui.Button(fa.ICON_FA_MAP_MARKED_ALT .. u8 ' Ïîìîùü ïî GPS', imgui.ImVec2(145, 20)) then
			if not doesFileExist(getGameDirectory() .. '//moonloader//AdminToolsGromov//gps.json') then
				sampAddChatMessage(tag .. 'Ó âàñ îòñóòñòâóåò ôàéë ñ íàâèãàöèåé äëÿ GPS!', -1)
			else
				imgui.OpenPopup(u8 'Ïîìîùü ïî GPS')
			end
		end
		if imgui.BeginPopupModal(u8 'Ïîìîùü ïî GPS', ReportWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar) then
			local gps = io.open('moonloader\\AdminToolsGromov\\gps.json', 'a+')
			...
			imgui.PopItemWidth()
			if imgui.Button(u8 'Çàêðûòü', imgui.ImVec2(510, 25)) then
				imgui.CloseCurrentPopup()
			end
			lua_thread.create(function()
		...
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_LEVEL_UP_ALT .. u8 ' ÒÏ ê èãðîêó', imgui.ImVec2(145, 20)) then
			sampSendDialogResponse(6370, 1, _, 'Óâàæàåìûé èãðîê, ñåé÷àñ ïîïðîáóþ âàì ïîìî÷ü!')
			text_buffer_rep.v = ''
			ReportWindow.v = false
			sampSendChat('/g ' .. report_id)
		end
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_TABLET_ALT .. u8 ' Ñïèñîê êîìàíä', imgui.ImVec2(145, 20)) then
			if not doesFileExist(getGameDirectory() .. '//moonloader//AdminToolsGromov//commands.json') then
				sampAddChatMessage(tag .. 'Ó âàñ îòñóòñòâóåò ôàéë ñî ñïèñêîì êîìàíä!', -1)
			else
				imgui.OpenPopup(u8 'Ñïèñîê êîìàíä')
			end
		end
		if imgui.BeginPopupModal(u8 'Ñïèñîê êîìàíä', ReportWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar) then
			local commands = io.open('moonloader\\AdminToolsGromov\\commands.json', 'a+')
		...
		if imgui.Button(fa.ICON_FA_PALETTE .. u8 ' Òàáëèöà öâåòîâ', imgui.ImVec2(145, 20)) then
			ColorsWindow.v = not ColorsWindow.v
			imgui.Process = ColorsWindow.v
		end
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_CAR_CRASH .. u8 ' Ôëèïíóòü', imgui.ImVec2(145, 20)) then
			sampSendChat('/flip ' .. report_id)
			sampSendDialogResponse(6370, 1, _, 'Óâàæàåìûé ' .. nick_rep .. ', ôëèïíóë Âàñ.')
			text_buffer_rep.v = ''
			ReportWindow.v = false
		end
		imgui.SameLine()
		peredam()
		if imgui.Button(fa.ICON_FA_MOTORCYCLE .. u8 ' Âûäàòü ÍÐÃ', imgui.ImVec2(145, 20)) then
		...
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_USER .. u8 ' Çàñïàâíèòü', imgui.ImVec2(145, 20)) then
		...
		imgui.SameLine()
		if imgui.Button(fa.ICON_FA_MEDKIT .. u8 ' Âûäàòü HP', imgui.ImVec2(145, 20)) then
			...
			if imgui.Button(u8(er), imgui.ImVec2(145, 20)) then
				if cfg.reportstyle[qw] == 0 then
					if cfg.reportotvet[qw] ~= "" then
						...
						sampAddChatMessage("[Îøèáêà] Ââåäèòå îòâåò. Ëèáî çàêðîéòå ðåïîðò.", 14628149)
					end
		...
		if imgui.Button(u8 'Îòïðàâèòü', imgui.ImVec2(145, 20)) then
			sampSendDialogResponse(6370, 1, _, u8:decode(otvet))
			text_buffer_rep.v = ''
			ReportWindow.v = false
		end
		...
		if imgui.Button(u8 'Çàêðûòü', imgui.ImVec2(145, 20)) then
			imgui.OpenPopup(u8 'Ïîäòâåðæäåíèå çàêðûòèÿ')
		end
		imgui.PopStyleColor()
		if imgui.BeginPopupModal(u8 'Ïîäòâåðæäåíèå çàêðûòèÿ', ReportWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
			local size = imgui.GetWindowSize()
			btns_sizeX = size.x / 2 - 10
			imgui.Text(u8 'Âû äåéñòâèòåëüíî õîòèòå çàêðûòü ðåïîðò?')
			imgui.Text(u8 'Èç-çà ýòîãî ìîæíî ïîòåðÿòü ðåïóòàöèþ.')
			if imgui.Button(u8 ' Äà', imgui.ImVec2(btns_sizeX, 20)) then
				sampSendDialogResponse(6370, 1, _, '>> Skipped >>')
				ReportWindow.v = false
				imgui.CloseCurrentPopup()
			end
			imgui.SameLine()
			if imgui.Button(u8 ' Íåò', imgui.ImVec2(btns_sizeX, 20)) then
				imgui.CloseCurrentPopup()
			end
			imgui.EndPopup()
		end
	...
	if myOnline.v then
		...
		imgui.Begin(u8 '#WeekOnline', _,
			imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.ShowBorders +
		...
		if imgui.Button(u8 'Çàêðûòü', imgui.ImVec2(200, 25)) then myOnline.v = false end
		imgui.End()
	end
end
...
function flood(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /flood {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
				tag .. "Èãðîê íå â ñåòè {E78284}& {84A6E7}äàííûé èãðîê àäìèíèñòðàòîð. Âîçìîæíî Âû îøèáëèñü {E78284}ID'îì",
				-1)
		else
			local nick = sampGetPlayerNickname(id)
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typeflood .. ' ' .. nick .. ', ' .. cfg.main.flood .. ' Ôëóä')
		end
	end
end
...
function dm(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /dm {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typedm .. ' ' .. nick .. ' ' .. cfg.main.dm .. ' ÄÌ')
		end
	end
end
...
function db(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /db {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typedb .. ' ' .. nick .. ', ' .. cfg.main.db .. ' ÄÁ')
		end
	end
end
...
function oskrod(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /oskrod {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typeosk .. ' ' .. nick .. ', ' .. cfg.main.oskrod .. ' Îñêîðáëåíèå ðîäíûõ')
		end
	end
end
...
function sbiv(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /sbiv {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
...
function dbk(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /dbk {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typedbk .. ' ' .. nick .. ', ' .. cfg.main.dbk .. ' ÄÁ êîâøîì')
		end
	end
end
...
function upomrod(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /upomrod {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typeupom .. ' ' .. nick .. ', ' .. cfg.main.upomrod .. ' Óïîìèíàíèå ðîäíûõ')
		end
	end
end
...
function ncop(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /ncop {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typencop .. ' ' .. nick .. ', ' .. cfg.main.ncop .. ' íÐÏ êîï')
		end
	end
end
...
function cheat(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /cheat {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typecheat .. ' ' .. nick .. ', ' .. cfg.main.cheat .. ' ÈÇÏ')
		end
	end
end
...
function ntune(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /ntune {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typentune .. ' ' .. nick .. ', ' .. cfg.main.ntune .. ' íÐÏ òþíèíã')
		end
	end
end
...
function nead(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /nead {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typenead .. ' ' .. nick .. ', ' .. cfg.main.nead .. ' íåàäåêâàòíîå ïîâåäåíèå')
		end
	end
end
...
function oskadm(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /oskadm {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typeoskadm .. ' ' ..
				nick .. ', ' .. cfg.main.oskadm .. ' îñêîðáëåíèå àäìè[í]èñòðàöèè')
		end
	end
end
...
function desc(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /desc {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			local nick = sampGetPlayerNickname(id)
			sampSendChat('/pm ' .. nick .. ', 0 Îïèñàíèå âàøåãî ïåðñîíàæà áûëî óäàëåíî. Ïðè÷èíà: nRP îïèñàíèå', -1)
			sampSendChat('/adeldesc ' .. nick .. '')
		end
	end
end
...
function pc(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /pc {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
...
function mdm(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /mdm {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
			...
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish))
			sampSendChat('/pm ' .. id .. ' 0 ' .. u8:decode(cfg.main.infopunish1))
			sampSendChat('/' .. cfg.main.typemdm .. ' ' .. nick .. ', ' .. cfg.main.mdm .. ' Ìàññîâûé ÄÌ')
		end
	end
end
...
function sk(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /sk {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
...
function tk(id)
	if id == nil or id == " " or id == "" then
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå: /tk {E78284}[ID]', -1)
	else
		local result = sampIsPlayerConnected(id)
		if not result then
			sampAddChatMessage(
...
function spcarall(arg)
	if tonumber(arg) and tonumber(arg) >= 5 and tonumber(arg) <= 60 then
		lua_thread.create(function()
			sampSendChat('/ao [Ñïàâí Òðàíñïîðòà] Óâàæàåìûå èãðîêè, ÷åðåç ' ..
				arg .. ' ñåêóíä ïðîèçîéäåò ðåñïàâí âñåãî òðàíñïîðòà.')
			...
			spcars = true
			sampSendChat('/apanel')
			sampSendChat('/ao [Ñïàâí Òðàíñïîðòà] Ñïàâí òðàíñïîðòà ïðîèçîøåë óñïåøíî | Ïðèÿòíîé èãðû!')
		end)
	else
		sampAddChatMessage(tag .. "Âû óêàçàëè íåâåðíîå âðåìÿ, äèàïàçîí [5-60] & ñïàâí òðàíñïîðòà óæå èäåò.", -1)
	end
end
...
function tpcssss()
	_, x, y, z = SearchMarker()
	if not _ then
		sampAddChatMessage(tag .. "Ìàðêåð äëÿ òåëåïîðòà {E78284}íå íàéäåí{84A6E7}.", -1)
	else
		setCharCoordinates(PLAYER_PED, x, y, z)
	end
end
...
function ev.onShowDialog(id, dialogStyle, dialogTitle, okButtonText, cancelButtonText, dialogText)
	...
	if cd and id == 9991 and dialogTitle:find('Âûáîð ïðèçà') then
		sampSendDialogResponse(9991, 1, 1, nil)
	end
	if cd and id == 9992 and dialogTitle:find("Âûáîð ïðèçà") then
		sampSendDialogResponse(9992, 1, 0, prize)
		sampCloseCurrentDialogWithButton(0)
		cd = false
	end
	if id == 15316 and arena_type.v == 0 and dialogTitle:find("Âûáîð ïðèçà") then
		sampSendDialogResponse(15316, 1, 0, nil)
	end
	if id == 15316 and arena_type.v == 1 and dialogTitle:find("Âûáîð ïðèçà") then
		sampSendDialogResponse(15316, 1, 1, nil)
	end
	if id == 15316 and arena_type.v == 2 and dialogTitle:find("Âûáîð ïðèçà") then
		sampSendDialogResponse(15316, 1, 2, nil)
	end
	if id == 15316 and arena_type.v == 3 and dialogTitle:find("Âûáîð ïðèçà") then
		sampSendDialogResponse(15316, 1, 3, nil)
	end
	if id == 15317 and dialogTitle:find("Êîëè÷åñòâî") then
		sampSendDialogResponse(15317, 1, 0, arena_amount.v .. "," .. arena_people.v .. "," .. arena_time.v)
		sampCloseCurrentDialogWithButton(0)
	end
	...
	if id == 235 then
		if dialogText:find("\n{FFFFFF}Îðãàíèçàöèÿ: {B83434}%[(.*)%]\n{FFFFFF}Óðîâåíü ðîçûñêà: {B83434}%[(%d+)%]") then
			org = dialogText:match("\n{FFFFFF}Îðãàíèçàöèÿ: {B83434}%[(.*)%]\n{FFFFFF}Óðîâåíü ðîçûñêà: {B83434}%[(%d+)%]")
			rank = 0
		end
		if dialogText:find("\n{FFFFFF}Îðãàíèçàöèÿ: {B83434}%[(.*)%]\n{FFFFFF}Äîëæíîñòü: {B83434}(.*)%((%d+)%)\n\n{FFFFFF}Óðîâåíü ðîçûñêà: {B83434}%[(%d+)%] ") then
			org, namerank, rank = dialogText:match(
				"\n{FFFFFF}Îðãàíèçàöèÿ: {B83434}%[(.*)%]\n{FFFFFF}Äîëæíîñòü: {B83434}(.*)%((%d+)%)\n\n{FFFFFF}Óðîâåíü ðîçûñêà: {B83434}%[(%d+)%] ")
		end
		if dialogText:find("\n{ffffff}Åâðî: {B83434}%[(%d+)%] \n{ffffff}BTC: {B83434}%[(%d+)%]\n") then
			euro, btc = dialogText:match("\n{ffffff}Åâðî: {B83434}%[(%d+)%] \n{ffffff}BTC: {B83434}%[(%d+)%]\n")
		end
		if dialogText:find("\n{FFFFFF}Ñòàòóñ: {B83434}%[(.*)%]\n{FFFFFF}Òåëåôîííàÿ êíèæêà: ") then
			vipstatus = dialogText:match("\n{FFFFFF}Ñòàòóñ: {B83434}%[(.*)%]\n{FFFFFF}Òåëåôîííàÿ êíèæêà: ")
		end
		if dialogText:find("\n\n{FFFFFF}Ïðåäóïðåæäåíèÿ: {B83434}%[(.*)%]\n{FFFFFF}Ñåìåéíûå òàëîíû: ") then
			warns = dialogText:match("\n\n{FFFFFF}Ïðåäóïðåæäåíèÿ: {B83434}%[(.*)%]\n{FFFFFF}Ñåìåéíûå òàëîíû: ")
		end
		if dialogText:find("\n{FFFFFF}Óâàæåíèå: {B83434}%[(.*)/(.*)%] \n{FFFFFF}Äåíüãè:") then
	...
	if id == 6370 then
		...
		report_name, report_id, report_text = dialogText:match("Æàëîáà/Âîïðîñ îò: (.*)%[(%d+)%]\n\n{......}(.*)\n\n")
		nick_rep = report_name:gsub('_', ' ')
		id_rep = report_id
		mynick = sampGetPlayerNickname(myid)
	...
	if dialogText:find("Âû áûëè êèêíóòû") and cfg.main.deleteac then
		return false
	end
	...
	if id == 2 then
		if cfg.main.autologin then
			if dialogText:find('Íåâåðíûé ïàðîëü') then
				sampAddChatMessage(tag .. 'Âû ââåëè íåâåðíûé ïàðîëü â íàñòðîéêàõ, àâòîëîãèí âûêëþ÷åí àâòîìàòè÷åñêè.', -1)
...
function ev.onSendCommand(param)
	if param:find('reoff') then
		...
		sampAddChatMessage("[Recon] Âû ïîêèíóëè ñëåæêó.", 14628149)
	end
...
function onScriptTerminate(script, quitGame)
	if script == thisScript() then
		...
		if ErrorMessage and not quitGame then
			...
			local sf = isSampfuncsLoaded() and "{E78284}Åñòü" or "{fe4e4e}Íåò"
			local cleo = isCleoLoaded() and "{E78284}Åñòü" or "{fe4e4e}Íåò"
			local date = os.date("%d.%m.%Y %H:%M:%S", os.time())
			local libs = {
				(bImgui and "{E78284}Ïðèñóòñòâóåò (" .. imgui._VERSION .. ")" or "{fe4e4e}Îòñóòñòâóåò"),
				(bEvents and "{E78284}Ïðèñóòñòâóåò" or "{fe4e4e}Îòñóòñòâóåò")
			}
		end
	end
end
...
function ev.onServerMessage(color, text)
	if not isGamePaused() then
		if text:find("%[(%d+)%] (.*) | Óðîâåíü: (%d+) | UID: (%d+) | packetloss: (%d+)%.(%d+) %((.*)%)") then
			idspec, nickspec, lvlspec, uidspec, pl, statespec = text:match(
				"%[(%d+)%] (.*) | Óðîâåíü: (%d+) | UID: (%d+) | packetloss: (.*) %((.*)%)")
			if statespec == 'ñ ëàóí÷åðà' then
				statespec = '{00ff00}Ëàóí÷åð'
			elseif statespec == 'áåç ëàóí÷åðà' then
				statespec = '{fe4e4e}Êëèåíò'
			end
			if showid then
				showid = false
				return false
		...
		if cfg.main.underb then
			if text:find("%(%( Àäìèíèñòðàòîð (.*)%[(%d+)%]: {AFAFAF}(.*) {FFE6E6}%)%)") then
				local nickt, idt, t = text:match("%(%( Àäìèíèñòðàòîð (.*)%[(%d+)%]: {AFAFAF}(.*) {FFE6E6}%)%)")
			...
			if cfg.main.formaplus then
				if text:find("Ýòîò èãðîê óæå â ÄÅÌÎÐÃÀÍÅ!") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", ýòîò èãðîê óæå â äåìîðãàíå!")
					acceptforma = false
				end

				if text:find("Ýòîò èãðîê íå çàáàíåí") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", ýòîò èãðîê íå â áàíå!")
					acceptforma = false
				end

				if text:find("Íåâàëèäíàÿ ìîäåëü") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", òàêîãî ID òðàíñïîðòà íå ñóùåñòâóåò!")
					acceptforma = false
				end

				if text:find("Èãðîê çà ðóëåì!") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", èãðîê çà ðóëåì!")
					acceptforma = false
				end

				if text:find("Ó âàñ íåò äîñòóïà ê ýòîé êîìàíäå%!") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", íå ìîãó ïðèíÿòü âàøó ôîðìó!")
					acceptforma = false
				end

				if text:find("Èãðîê îíëàéí%!") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", èãðîê îíëàéí!")
					acceptforma = false
				end

				if text:find("Íå áîëüøå 30 ñèìâîëîâ%!") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", â ïðè÷èíå áîëüøå 30 ñèìâîëîâ!")
					acceptforma = false
				end

				if text:find("Èñïîëüçóé: (.*)") and acceptforma then
					wait(1000)
					usefull = text:match("Èñïîëüçóé: (.*)")
					sampSendChat("/a [Forma] " .. admin_nick .. ", èñïîëüçóéòå: " .. usefull)
					acceptforma = false
				end

				if text:find("Èñïîëüçóéòå: (.*)") and acceptforma then
					wait(1000)
					usefull1 = text:match("Èñïîëüçóé: (.*)")
					sampSendChat("/a [Forma] " .. admin_nick .. ", èñïîëüçóéòå: " .. usefull1)
					acceptforma = false
				end

				if text:find("%[Èíôîðìàöèÿ%] {FFFFFF}Èñïîëüçóéòå: (.*)") and acceptforma then
					wait(1000)
					usefull12 = text:match("%[Èíôîðìàöèÿ%] {FFFFFF}Èñïîëüçóéòå: (.*)")
					sampSendChat("/a [Forma] " .. admin_nick .. ", èñïîëüçóéòå: " .. usefull12)
					acceptforma = false
				end

				if text:find("/muteoff %[nick%] %[time%] %[reason%]") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", èñïîëüçóéòå: /muteoff [nick] [time] [reason]")
					acceptforma = false
				end

				if text:find("%[Îøèáêà%] {FFFFFF}Èãðîê íå â ìàøèíå%!") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", èãðîê íå â ìàøèíå!")
					acceptforma = false
				end

				if text:find("Ó ýòîãî èãðîêà óæå åñòü áàí ÷àòà%!") and acceptforma then
					wait(1000)
					sampSendChat("/a [Forma] " .. admin_nick .. ", ó ýòîãî èãðîêà åñòü ìóò!")
					acceptforma = false
				end
				acceptforma = false
			end

			if text:find('%[A%] (.*)%[(%d+)%] èñïîëüçîâàë êîä%: (.*) Ïðèç%: (.*) %(Êîëè÷åñòâî%: (%d+)%)') then
				nickname, playerid, code, prize, amount = text:match(
					'%[A%] (.*)%[(%d+)%] èñïîëüçîâàë êîä%: (.*) Ïðèç%: (.*) %(Êîëè÷åñòâî%: (%d+)%)')
			end

			if text:find('%[Îøèáêà%] {......}Âíèìàíèÿ, ñóíäóê óæå ñòîèò, %/settings %> Âêëþ÷èòü ÒèêÒîê ñóíäóê , äëÿ òîãî ÷òîáû óâèäåòü åãî!') then
				sampAddChatMessage(tag .. 'TikTok ñóíäóê óæå çàïóùåí.', -1)
				return false
			end

			if text:find("{......}%[TIK%-TOK SUNDUK%] {......}Ïî îêîí÷àíèþ òàéìåðà, ó âàñ ïîÿâèòüñÿ 'Êóðñîð' äëÿ îòêðûòèÿ") then
				lua_thread.create(function()
					sampAddChatMessage(tag .. 'Êòî-òî ïîñòàâèë TikTok ñóíäóê!', -1)
					wait(140000)
					sampAddChatMessage(tag .. 'Âû ñíîâà ìîæåòå ïîñòàâèòü TikTok ñóíäóê!', -1)
					if cfg.main.autosunduk then
						arena_type.v = "1"
						arena_amount.v = "300"
						arena_time.v = "2"
						arena_people.v = "10"
						sampSendChat('/sunduk')
						sampAddChatMessage(tag .. 'Ñêðèïò àâòîìàòè÷åñêè ïîñòàâèë TikTok ñóíäóê!', -1)
					end
				end)
			end
	...
	if cfg.main.logging then
		if text:find("Àäìèíèñòðàòîð (.*) âûïóñòèë èãðîêà (.*), ïðè÷èíà: (.*)") and cfg.main.logunjail then
			adminnick, playernick, reason = text:match("Àäìèíèñòðàòîð (.*) âûïóñòèë èãðîêà (.*), ïðè÷èíà: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnjailsLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Àäìèíèñòðàòîð ' .. adminnick .. ' âûïóñòèë èãðîêà ' .. playernick .. '. Ïðè÷èíà: ' .. reason .. '\n')
			f:close()
		end

		if text:find("Àäìèíèñòðàòîð (.*) ñíÿë çàãëóøêó ñ èãðîêà (.*), ïðè÷èíà: (.*)") and cfg.main.logunmute then
			adminnick, playernick, reason = text:match("Àäìèíèñòðàòîð (.*) ñíÿë çàãëóøêó ñ èãðîêà (.*), ïðè÷èíà: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnmutesLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Àäìèíèñòðàòîð ' .. adminnick .. ' ñíÿë çàãëóøêó ñ èãðîêà ' .. playernick .. '. Ïðè÷èíà: ' .. reason ..
				'\n')
			f:close()
		end

		if text:find("Àäìèíèñòðàòîð (.*) ðàçáàíèë èãðîêà (.*), ïðè÷èíà: (.*)") and cfg.main.logunban then
			adminnick, playernick, reason = text:match("Àäìèíèñòðàòîð (.*) ðàçáàíèë èãðîêà (.*), ïðè÷èíà: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnbanLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Àäìèíèñòðàòîð ' .. adminnick .. ' ðàçáàíèë èãðîêà ' .. playernick .. '. Ïðè÷èíà: ' .. reason .. '\n')
			f:close()
		end

		if text:find("%[A%] (.*)%[(%d+)%] ðàçáëîêèðîâàë IP: (.*)") and cfg.main.logunbanip then
			adminnick, adminid, unbanip = text:match("%[A%] (.*)%[(%d+)%] ðàçáëîêèðîâàë IP: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnbanipLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) .. '] Àäìèíèñòðàòîð ' .. adminnick .. ' ðàçáëîêèðîâàë IP: ' ..
				unbanip .. '\n')
			f:close()
		end

		if text:find("Àäìèíèñòðàòîð (.*) çàáàíèë â îôôëàéíå èãðîêà (.*)%. Ïðè÷èíà: (.*)") and cfg.main.logbanoff then
			adminnick, playernick, reason = text:match(
				"Àäìèíèñòðàòîð (.*) çàáàíèë â îôôëàéíå èãðîêà (.*)%. Ïðè÷èíà: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//BanoffLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Àäìèíèñòðàòîð ' .. adminnick ..
				' çàáëîêèðîâàë â îôôëàéíå èãðîêà ' .. playernick .. '. Ïðè÷èíà: ' .. reason .. '\n')
			f:close()
		end

		if text:find("Àäìèíèñòðàòîð (.*) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò òþðüìû èãðîêó (.*)%. Ïðè÷èíà: (.*)") and cfg.main.logjailoff then
			adminnick, srok, playernick, reason = text:match(
				"Àäìèíèñòðàòîð (.*) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò òþðüìû èãðîêó (.*)%. Ïðè÷èíà: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//JailoffLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Àäìèíèñòðàòîð ' ..
				adminnick .. ' óñòàíîâèë â îôôëàéíå ' ..
				srok .. ' ìèíóò òþðüìû èãðîêó ' .. playernick .. '. Ïðè÷èíà: ' .. reason .. '\n')
			f:close()
		end

		if text:find("Àäìèíèñòðàòîð (.*) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò ìîë÷àíêè èãðîêó (.*)%. Ïðè÷èíà: (.*)") and cfg.main.logmuteoff then
			adminnick, srok, playernick, reason = text:match(
				"Àäìèíèñòðàòîð (.*) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò ìîë÷àíêè èãðîêó (.*)%. Ïðè÷èíà: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//MuteoffLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Àäìèíèñòðàòîð ' ..
				adminnick ..
				' óñòàíîâèë â îôôëàéíå ' .. srok .. ' ìèíóò ìîë÷àíêè èãðîêó ' .. playernick .. '. Ïðè÷èíà: ' .. reason ..
				'\n')
			f:close()
		end

		if text:find("Àäìèíèñòðàòîð (.*) çàáàíèë ip (.*)%. Ïðè÷èíà: (.*)") and cfg.main.logbanipoff then
			adminnick, banipoffip, reason = text:match("Àäìèíèñòðàòîð (.*) çàáàíèë ip (.*)%. Ïðè÷èíà: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//BanipoffLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Àäìèíèñòðàòîð ' .. adminnick .. ' çàáàíèë IP ' .. banipoffip .. '. Ïðè÷èíà: ' .. reason .. '\n')
			f:close()
		end
	end
	if text:find("Ïðèâåòñòâóåì íîâîãî èãðîêà íàøåãî ñåðâåðà: {FF9900}(.*) {FFFFFF}%(ID: (%d+)%)  {cccccc}IP: (%d+)%.(%d+)%.(%d+)%.(%d+)") then
		regnick, regid, ip1, ip2, ip3, ip4 = text:match(
			"Ïðèâåòñòâóåì íîâîãî èãðîêà íàøåãî ñåðâåðà: {FF9900}(.*) {FFFFFF}%(ID: (%d+)%)  {cccccc}IP: (%d+)%.(%d+)%.(%d+)%.(%d+)")
		if cfg.main.regChecker then
		...
		if cfg.main.removeip then
			sampAddChatMessage(
				'Ïðèâåòñòâóåì íîâîãî èãðîêà íàøåãî ñåðâåðà: {FF9900}' ..
				regnick .. ' {ffffff}(ID: ' .. regid .. ') {cccccc}IP: ' .. ip1 .. '.xx.xx.' .. ip4, -1)
			return false
		end
	end
	if text:find("Èãðîê (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] ïåðåâåë èãðîêó (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] íà ñ÷åò $(.*) %[$(.*)%]") then
		nickplayer, idplayer, ip1, ip2, ip3, ip4, nickget, idget, ip5, ip6, ip7, ip8, summ, allmoney = text:match(
			"Èãðîê (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] ïåðåâåë èãðîêó (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] íà ñ÷åò $(.*) %[$(.*)%]")
		if cfg.main.removeip then
			sampAddChatMessage(
				'{BA2B29}Èãðîê ' ..
				nickplayer ..
				...
				ip4 ..
				'] ïåðåâåë èãðîêó ' ..
				nickget .. '[' .. idget .. '] [IP: ' .. ip5 .. '.xx.xx.' .. ip8 .. '] íà ñ÷åò $' ..
				summ .. ' [$' .. allmoney .. ']', -1)
			return false
		end
	end

	if text:find("%- %- %- %- Ñîâïàäåíèå ñ: (%d+)%.(%d+)%.(%d+)%.(%d+)%. %- %- %- %-") and cfg.main.removeip then
		ip1, ip2, ip3, ip4 = text:match("%- %- %- %- Ñîâïàäåíèå ñ: (%d+)%.(%d+)%.(%d+)%.(%d+)%. %- %- %- %-")
		sampAddChatMessage('{99CC00}- - - - Ñîâïàäåíèå ñ ' .. ip1 .. '.xx.xxx.' .. ip4 .. ' - - - -', -1)
		return false
	end


	if text:find("Èãðîê/àäìèí (.*)%[(%d+)%] çàøåë %[reg ip:(%d+).(%d+).(%d+).(%d+)%] %-%- %[norm ip: (%d+).(%d+).(%d+).(%d+)%]") then
		nickadm, idadm, ip1, ip2, ip3, ip4, ip5, ip6, ip7, ip8 = text:match(
			"Èãðîê/àäìèí (.*)%[(%d+)%] çàøåë %[reg ip:(%d+).(%d+).(%d+).(%d+)%] %-%- %[norm ip: (%d+).(%d+).(%d+).(%d+)%]")
		if cfg.main.removeip then
			sampAddChatMessage(
				'{BA2B29}Ëèäåð/àäìèíèñòðàòîð ' ..
				nickadm ..
				'[' .. idadm ..
				'] çàøåë íà ñåðâåð. [REG IP: ' ..
				ip1 .. '.xx.xx.' .. ip4 .. '] - [LAST IP: ' .. ip5 .. '.xx.xx.' .. ip7 ..
				']', -1)
			return false
		end
	end
	if text:find("Niñk %[(.*)%]  R%-IP %[(.*)%]  IP | A%-IP %[{FF6A78}(.*) | (.*){6ab1ff}%]") then
		nickplayer, regip, lastip, xzip = text:match(
			"Niñk %[(.*)%]  R%-IP %[(.*)%]  IP | A%-IP %[{FF6A78}(.*) | (.*){6ab1ff}%]")
		if cfg.main.regi then
			sampAddChatMessage(tag .. 'Íà÷èíàþ ïðîâåðÿòü äàííûå, íå ñïåøè!', -1)
			ips = {}
			...
			if #ips > 0 then
				...
				asyncHttpRequest(
								...
								if style_reg.v == 1 then
									if style_admin.v == 1 then
										if cmd == 'getip' then
											lua_thread.create(function()
												wait(700)
												sampSendChat('/a [Tools] Ïðîâåðÿþ IP: ' .. rdata[1]["query"] .. ' | ¹1')
												wait(700)
												sampSendChat('/a [1 IP] Ñòðàíà: ' ..
													rdata[1]["country"] ..
													' | Ãîðîä: ' ..
													rdata[1]["city"] .. ' | Ïðîâàéäåð: ' .. rdata[1]["isp"])
												wait(700)
												sampSendChat('/a [1 IP] Ðàññòîÿíèå îò ðåãèñòðàöèîííîãî: ' ..
													distances .. 'êì')
												wait(700)
												sampSendChat('/A [Tools] Ïðîâåðÿþ IP: ' .. rdata[2]["query"] .. ' | ¹2')
												wait(700)
												sampSendChat('/a [2 IP] Ñòðàíà: ' ..
													rdata[2]["country"] ..
													' | Ãîðîä: ' ..
													rdata[2]["city"] .. ' | Ïðîâàéäåð: ' .. rdata[2]["isp"])
												wait(700)
												sampSendChat('/a [2 IP] Ðàññòîÿíèå îò ðåãèñòðàöèîííîãî: ' ..
													distances .. 'êì')
											end)
										end
									else
										lua_thread.create(function()
											...
											sampSendChat('/a [1 IP] Ñòðàíà: ' ..
												rdata[1]["country"] ..
												' | Ãîðîä: ' .. rdata[1]["city"] .. ' | Ïðîâàéäåð: ' .. rdata[1]["isp"])
											wait(700)
											sampSendChat('/a [1 IP] Ðàññòîÿíèå îò ðåãèñòðàöèîííîãî: ' .. distances ..
												'êì')
											wait(700)
											sampSendChat('/a [Tools] Ïðîâåðÿþ IP: ' .. rdata[2]["query"] .. ' | ¹2')
											wait(700)
											sampSendChat('/a [2 IP] Ñòðàíà: ' ..
												rdata[2]["country"] ..
												' | Ãîðîä: ' .. rdata[2]["city"] .. ' | Ïðîâàéäåð: ' .. rdata[2]["isp"])
											wait(700)
											sampSendChat('/a [2 IP] Ðàññòîÿíèå îò ðåãèñòðàöèîííîãî: ' .. distances ..
												'êì')
										end)
									end
								else
									if cfg.main.removeip then
										sampAddChatMessage(tag .. 'Ïðîâåðÿþ IP: {E78284}ñêðûòî {84A6E7}| {E78284}¹' .. i,
											-1)
									else
										sampAddChatMessage(
											tag ..
											'Ïðîâåðÿþ IP: {E78284}' .. rdata[i]["query"] .. ' {84A6E7}| {E78284}¹' .. i,
											-1)
									end
									sampAddChatMessage(
										tag ..
										...
										' IP] Ñòðàíà: ' ..
										rdata[i]["country"] ..
										' | Ãîðîä: ' .. rdata[i]["city"] .. ' | Ïðîâàéäåð: ' .. rdata[i]["isp"], -1)
									sampAddChatMessage(
										tag .. '[' .. i .. ' IP] Ðàññòîÿíèå îò ðåãèñòðàöèîííîãî: ' .. distances .. 'êì',
										-1)
								end
							end
						end
					...
					function(err)
						sampAddChatMessage(tag .. 'Ïðîèçîøëà îøèáêà ' .. err, -1)
					end
				)
			end
			if cfg.main.removeip then
				sampAddChatMessage(
					tag ..
					'Èãðîâîé íèêíåéì: {E78284}' ..
					nickplayer .. '{84A6E7}. REG-IP: {E78284}ñêðûòî {84A6E7}| LAST-IP: {E78284}ñêðûòî', -1)
				return false
			end
		end
	end
	...
	--[[if text:find("(.*) %[(%d+)%] ïîäîçðåâàåòñÿ âî çëîìå, ïðîâåðüòå åãî è àâòîðèçèðóéòå /acceptadmin %[6%+ lvl%]") and cfg.main.geniumaccept then
nickadmina, idadmina = text:match("(.*) %[(%d+)%] ïîäîçðåâàåòñÿ âî çëîìå, ïðîâåðüòå åãî è àâòîðèçèðóéòå /acceptadmin %[6%+ lvl%]")
if nickadmina == mynick then
sampSendChat('/acceptadmin '..idadmina)
end
end]]

	if text:find("%[Ïîäîçðåíèå íà ðåêëàìó%]") and cfg.main.deletereklama then
		return false
	end

	if text:find("%[Îøèáêà%]{FFFFFF} Èñïîëüçóéòå: /tpcor %[id%] %[x%], %[y%], %[z%]") then
		sampAddChatMessage('{E78284}[AdminTools] {FFFFFF}Èñïîëüçóéòå: /tpc [id] [x] [y] [z]', -1)
		return false
	end

	local reasons = { "deladmtag", "rmute", "unrmute", "slap", "flip", "freeze", "unfreeze", "pm", "spplayer", "sethp",
		"unjail", "jail", "weap", "unmute", "mute", "spcar", "kick",
		"getip", "pgetip", "plveh", "unban", "ban", "unwarn", "warn", "spcars", "givegun", "removetune", "delbname",
		"delhname", "warnoff", "setgangzone", "makeleader", "sban", "banip", "unbanip",
		"jailoff", "muteoff", "skick", "setskin", "uval", "ao", "banoff", "agl", "setname", "veh", "agiveskin",
		"setadmtag", "giveitem", "acceptadmin", "awarn", "unjailoff", "asellbiz", "asellhouse", "setarmour", "unmuteoff" }


	for k, v in ipairs(reasons) do
		if not isGamePaused() then
			if text:match('%[A%] (.*)%[(%d+)%]%: /' .. v .. '%s') and cfg.main.trueform then
				admin_nick, admin_id, other = text:match("%[A%] (.*)%[(%d+)%]%: /" .. v .. "%s(.*)")
				cmd = v
				paramssss = other
				local _, myid = sampGetPlayerIdByCharHandle(playerPed)
				if not isGamePaused() then
					if cmd == "mute" and cfg.formssettings.autoforma_mute or cmd == "rmute" and cfg.formssettings.autoforma_rmute or cmd == "slap" and cfg.formssettings.autoforma_slap or cmd == "jail" and cfg.formssettings.autoforma_jail or cmd == "kick" and cfg.formssettings.autoforma_kick
						...
						lua_thread.create(function()
							sampAddChatMessage(
								...
								"}Ôîðìà {" ..
								cfg.main.colorforms ..
								"}/" .. cmd .. " " .. paramssss .. " {" .. cfg.main.twocolorforms ..
								"} ïðèíÿòà àâòîìàòè÷åñêè!", "0xFF" .. cfg.main.colorforms)
							local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//AcceptForms.txt",
								'a')
							f:write('[' ..
								os.date("%d.%m.%Y %X", os.time()) ..
								'] Ôîðìà: /' ..
								cmd ..
								' ' .. paramssss .. ' | Îòïðàâèòåëü: ' ..
								admin_nick .. ' | Òèï ïðèíÿòèÿ ôîðìû: àâòîìàòè÷åñêè\n')
							f:close()
							cfg.main.formsday = cfg.main.formsday + 1
							cfg.main.formsvse = cfg.main.formsvse + 1
							sampSendChat("/" .. cmd .. " " .. paramssss)
	...
	for k, v in ipairs(reasons) do
		if not isGamePaused() then
			if text:match('%[A | {......}(.*){......}%] (.*)%[(%d+)%]: /' .. v .. '%s') and cfg.main.trueform then
				aprefix, admin_nick, admin_id, other = text:match("%[A | {......}(.*){......}%] (.*)%[(%d+)%]: /" ..
					v .. "%s(.*)")
				cmd = v
				paramssss = other
				local _, myid = sampGetPlayerIdByCharHandle(playerPed)
	...
	if text:find("Ñåé÷àñ íåò âîïðîñîâ â ðåïîðò") and cfg.main.deleteot then
		return false
	end

	if xuesos20 == "true" then
		for k, v in ipairs(reasons) do
			if text:match("^%[(.-)%] (.-)%[(%d+)%]: /" .. v .. "%s") and cfg.main.trueform then
				started = started + 1
				if started < 2 then
					prikoll = "true"
					admin_tag, admin_nick, admin_id, other = text:match("^%[(.-)%] (.-)%[(%d+)%]: /" .. v .. "%s(.*)")
					cmd = v
					paramssss = other
					if stop == 0 then
						if cmd == "mute" and cfg.formssettings.forma_mute or cmd == "jail" and cfg.formssettings.forma_jail or cmd == "rmute" and cfg.formssettings.forma_rmute or cmd == "unrmute" and cfg.formssettings.forma_unrmute
							...
							if not isGamePaused() then
								sampAddChatMessage(
									...
									" {" ..
									cfg.main.twocolorforms ..
									"}<< Îòïðàâèòåëü: {" .. cfg.main.colorforms .. "}" ..
									"[" ..
									admin_tag ..
									"{" .. cfg.main.colorforms .. "}]" .. admin_nick .. "[" .. admin_id .. "]",
									"0xFF" .. cfg.main.colorforms)
								sampAddChatMessage(
									"[AdminTools] {" ..
									cfg.main.twocolorforms ..
									"}Íàæìèòå êëàâèøó >> {" ..
									cfg.main.colorforms ..
									"}" .. klavisha .. " {" .. cfg.main.twocolorforms .. "}<< ÷òîáû ïðèíÿòü ôîðìó!",
									"0xFF" .. cfg.main.colorforms)
								lua_thread.create(function()
									for i = 0, 5 do
									...
									if prikoll == "true" then
										sampAddChatMessage(
											...
											cfg.main.colorforms ..
											"}/" .. cmd .. " " .. paramssss ..
											" {" .. cfg.main.twocolorforms .. "}<< íå ïðèíÿòà!", "0xFF" ..
											cfg.main.colorforms)
						...
						if cmd == "mute" and cfg.formssettings.forma_mute or cmd == "jail" and cfg.formssettings.forma_jail or cmd == "rmute" and cfg.formssettings.forma_rmute or cmd == "unrmute" and cfg.formssettings.forma_unrmute
							...
							if not isGamePaused() then
								sampAddChatMessage(
									...
									cfg.main.twocolorforms ..
									"}<< Îòïðàâèòåëü: {" .. cfg.main.colorforms .. "}" ..
									admin_nick .. "[" .. admin_id .. "]", "0xFF" .. cfg.main.colorforms)
								sampAddChatMessage(
									"[AdminTools] {" ..
									cfg.main.twocolorforms ..
									"}Íàæìèòå êëàâèøó >> {" ..
	...
	if not isGamePaused() then
		if text:find("%[A%] Àäìèíèñòðàòîð (.*)%[(%d+)%] äàë ïîäæîïíèê (.*)%[(%d+)%]") and cfg.main.deleteslap and cfg.main.deleteactions then
			return false
		end

		if text:find("%[Æàëîáà%] îò (.*)%[(%d+)%]: {ffffff}(.*)%. Óæå {BE2D2D}(%d+){FFFFFF} æàëîá!!!") then
			nickp, idp, sute, kolvo = text:match(
				"%[Æàëîáà%] îò (.*)%[(%d+)%]: {ffffff}(.*)%. Óæå {BE2D2D}(%d+){FFFFFF} æàëîá!!!")
			sampAddChatMessage(
				'[Ðåïîðò] îò ' ..
				nickp ..
				'[' ..
				idp ..
				']: {' ..
				cfg.main.twocolorreport ..
				'}' .. sute .. '. Óæå {' .. cfg.main.colorreport .. '}' .. kolvo ..
				' {' .. cfg.main.twocolorreport .. '}ðåïîðòîâ!', '0xFF' .. cfg.main.colorreport)
		...
		if text:find("(.*) %[(.*)%] êóïèë áèçíåñ ID: (.*) ïî ãîñ. öåíå çà (.*) ms! Êàï÷à: %((%d+) | (%d+)%)") and cfg.autoopra.settingbiz then
			bnickp, bidp, bidbiz, bvremya, bcaptcha1, bcaptcha2 = text:match(
				"(.*) %[(.*)%] êóïèë áèçíåñ ID: (.*) ïî ãîñ. öåíå çà (.*) ms! Êàï÷à: %((%d+) | (%d+)%)")
			nick_cutch = bnickp
			idp_cutch = bidp
			id_cutch = bidbiz
			time_cutch = bvremya
			...
			if not cfg.autoopra.status then
				for _, nick in pairs(cfg.ignore) do
					if bnickp == nick then
						if cfg.main.notfinchat then
							sampAddChatMessage(tag .. "Áèçíåñ áûë êóïëåí èãðîêîì èç áåëîãî ñïèñêà", -1)
						end
						return false
					end
				end
				sampAddChatMessage(
					tag ..
					'Èñïîëüçóéòå êîìàíäó {E78284}/' ..
					cmdBusiness.v .. ' {84A6E7}äëÿ çàïðîñà îïðîâåðæåíèÿ íà áèçíåñ {E78284}¹' .. bidbiz, -1)
			end
			local f = io.open(getGameDirectory() .. "\\moonloader\\AdminToolsGromov\\Ëîãè\\AutoOpraLogs.txt", 'a')
			f:write(string.format(
				'%s | Èãðîê ' ..
				bnickp .. ' êóïèë áèçíåñ ID: ' .. bidbiz .. ' ïî ãîñ. öåíå çà ' .. bvremya .. ' ms! Êàï÷à: ' ..
			...
			if cfg.autoopra.status then
				...
				if cfg.autoopra.myreasonbiz then
					...
					local result = sampIsPlayerConnected(bidp)
					if not result then
						send_rpc_command('/jailoff ' .. bnickp .. ' ' .. cfg.main.timeopra .. ' îïðà áèçíåñ ¹' .. bidbiz)
					else
						send_rpc_command('/jail ' .. bidp .. ' ' .. cfg.main.timeopra .. ' îïðà áèçíåñ ¹' .. bidbiz)
					end
				end
			end
		end

		local admin, victim, tj = text:match(
			"Àäìèíèñòðàòîð ([a-zA-Z_]+).* ïîñàäèë èãðîêà ([a-zA-Z_]+).* â äåìîðãàí íà %d+ ìèíóò")
		...
		local adminoff, tjoff, victimoff = text:match(
			"Àäìèíèñòðàòîð ([A-z_]+) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò äåìîðãàíà èãðîêó ([0-9A-z_]+)%.")
		for k, box in pairs(tLog) do
		...
		local adminunj, victimunj = text:match("Àäìèíèñòðàòîð ([a-zA-Z_]+) âûïóñòèë èãðîêà ([a-zA-Z_]+)")
		...
		local adminunjoff, victimunjoff = text:match(
			"Àäìèíèñòðàòîð ([a-zA-Z_]+) âûïóñòèë â îôôëàéíå ñ äåìîðãàíà ([a-zA-Z_]+)")
		...
		if text:find("(.*) %[(.*)%] êóïèë äîì ID: (.*) ïî ãîñ. öåíå çà (.*) ms! Êàï÷à: %((%d+) | (%d+)%)") and cfg.autoopra.settinghouse then
			nickp, idp, idhouse, vremya, captcha1, captcha2 = text:match(
				"(.*) %[(.*)%] êóïèë äîì ID: (.*) ïî ãîñ. öåíå çà (.*) ms! Êàï÷à: %((%d+) | (%d+)%)")
			nick_cutch = nickp
			idp_cutch = idp
			id_cutch = idhouse
			time_cutch = vremya
			...
			if not cfg.autoopra.status then
				for _, nick in pairs(cfg.ignore) do
					if nickp == nick then
						if cfg.main.notfinchat then
							sampAddChatMessage(tag .. "Äîì áûë êóïëåí èãðîêîì èç áåëîãî ñïèñêà", -1)
						end
						return false
					end
				end
				sampAddChatMessage(
					tag ..
					'Èñïîëüçóéòå êîìàíäó {E78284}/' ..
					cmdHouse.v .. ' {84A6E7}äëÿ çàïðîñà îïðîâåðæåíèÿ íà äîì {E78284}¹' .. idhouse, -1)
			end
			local f = io.open(getGameDirectory() .. "\\moonloader\\AdminToolsGromov\\Ëîãè\\AutoOpraLogs.txt", 'a')
			f:write(string.format(
				'%s | Èãðîê ' ..
				nickp ..
				' êóïèë äîì ID: ' .. idhouse .. ' ïî ãîñ. öåíå çà ' .. vremya .. ' ms! Êàï÷à: ' .. captcha1 .. '\n',
				os.date('%d.%m %H:%M', os.time())))
			f:close()
		...
		if text:find("%[MakeAdmin%] .*%[%d+%] óñòàíîâèë (%d+) %[.*%] óðîâåíü àäìèíèñòðàòîðà èãðîêó (.*) %(áûë.*") then
			lvladmin, nickchela = text:match(
				"%[MakeAdmin%] .*%[%d+%] óñòàíîâèë (%d+) %[.*%] óðîâåíü àäìèíèñòðàòîðà èãðîêó (.*) %(áûë.*")

			if MyName() == nickchela then
				sampAddChatMessage("[Âíèìàíèå] Âàì óñòàíîâëåí [" .. lvladmin .. "] óðîâåíü àäìèíèñòðèðîâàíèÿ!", 14628149)
		...
		local _, myid = sampGetPlayerIdByCharHandle(playerPed)
		mynick = sampGetPlayerNickname(myid)
		if text:find("Àäìèíèñòðàòîð " .. mynick .. "%[(%d+)%] îòâåòèë èãðîêó (.*)%[(%d+)%]:{ffffff} (.*)") and cfg.main.inforeport then
			local _, myid = sampGetPlayerIdByCharHandle(playerPed)
			mynick = sampGetPlayerNickname(myid)
			myidplayer, nickplayer, idplayer, textplayer = text:match("Àäìèíèñòðàòîð " ..
				mynick .. "%[(%d+)%] îòâåòèë èãðîêó (.*)%[(%d+)%]:{ffffff} (.*)")
		...
		local _, myid = sampGetPlayerIdByCharHandle(playerPed)
		mynick = sampGetPlayerNickname(myid)
		if text:find("Àäìèíèñòðàòîð " .. mynick .. "%[(%d+)%] çàáàíèë èãðîêà (.*)%[(%d+)%] íà (%d+) äíåé%. Ïðè÷èíà:") then
			cfg.main.nakazaniyaday = cfg.main.nakazaniyaday + 1
			cfg.main.nakazaniyavse = cfg.main.nakazaniyavse + 1
		end

		if text:find("Àäìèíèñòðàòîð (.*)%[(%d+)%] çàáàíèë èãðîêà (.*)%[(%d+)%] íà (%d+) äíåé%. Ïðè÷èíà: (.*)") then
			nickadmin, idadmin, nickp, idp, days, reason = text:match(
				"Àäìèíèñòðàòîð (.*)%[(%d+)%] çàáàíèë èãðîêà (.*)%[(%d+)%] íà (%d+) äíåé%. Ïðè÷èíà: (.*)")
		end
		...
		if text:find("%[A%] Àäìèíèñòðàòîð (.*)%[(%d+)%] âûäàë âðåìåííóþ ìàøèíó èãðîêó (.*)%[(%d+)%] %[(%d+)%]") and cfg.main.deleteplveh and cfg.main.deleteactions then
			return false
		end
		...
		if text:find("Àäìèíèñòðàòîð " .. mynick .. "%[(%d+)%] çàãëóøèë èãðîêà (.*)%[(%d+)%] íà (%d+) ìèíóò%. Ïðè÷èíà:") then
			cfg.main.nakazaniyaday = cfg.main.nakazaniyaday + 1
			cfg.main.nakazaniyavse = cfg.main.nakazaniyavse + 1
		end

		if text:find("Àäìèíèñòðàòîð (.*)%[(%d+)%] çàãëóøèë èãðîêà (.*)%[(%d+)%] íà (%d+) ìèíóò%. Ïðè÷èíà:") then
			if cfg.main.deletemute and cfg.main.deleteactions then return false end
		end

		if text:find("Àäìèíèñòðàòîð " .. mynick .. "%[(%d+)%] âûäàë ïðåäóïðåæäåíèå èãðîêó (.*)%[(%d+)%] %[(%d+)/(%d+)%] Ïðè÷èíà:") then
			cfg.main.nakazaniyaday = cfg.main.nakazaniyaday + 1
			cfg.main.nakazaniyavse = cfg.main.nakazaniyavse + 1
		end

		if text:find("Àäìèíèñòðàòîð (.*)%[(%d+)%] âûäàë ïðåäóïðåæäåíèå èãðîêó (.*)%[(%d+)%] %[(%d+)/(%d+)%] Ïðè÷èíà:") then
			if cfg.main.deletewarn and cfg.main.deleteactions then return false end
		end

		if text:find("%[A%] (.*)%[(%d+)%] íàïèñàë èãðîêó (.*)%[(%d+)%]:") and cfg.main.deletepm and cfg.main.deleteactions then
			return false
		end

		if text:find("Àäìèíèñòðàòîð " .. mynick .. "%[(%d+)%] ïîñàäèë èãðîêà (.*)%[(%d+)%] â äåìîðãàí íà (%d+) ìèíóò%. Ïðè÷èíà:") then
			cfg.main.nakazaniyaday = cfg.main.nakazaniyaday + 1
			cfg.main.nakazaniyavse = cfg.main.nakazaniyavse + 1
		end

		if text:find("Àäìèíèñòðàòîð (.*)%[(%d+)%] ïîñàäèë èãðîêà (.*)%[(%d+)%] â äåìîðãàí íà (%d+) ìèíóò%. Ïðè÷èíà:") then
			if cfg.main.deletejail and cfg.main.deleteactions then return false end
		end

		if text:find("Àäìèíèñòðàòîð " .. mynick .. "%[(%d+)%] êèêíóë èãðîêà (.*)%[(%d+)%]%. Ïðè÷èíà:") then
			cfg.main.nakazaniyaday = cfg.main.nakazaniyaday + 1
			cfg.main.nakazaniyavse = cfg.main.nakazaniyavse + 1
		end

		if cfg.main.deleteclad then
			if text:find("<deletewarn> (.*)%[(%d+)%] îòêðûë") or text:find("Ïðåäûäóùèé êëàä áûë îòêðûò") then
				return false
			end
		end
	end
	...
	if text:find("%[Àíòè÷èò%]") and cfg.main.deleteanticheat and cfg.main.deleteactions then
		return false
	end

	if text:find("%[A%] (.*)%[(%d+)%] íà÷àë ñëåäèòü çà (.*)%[(%d+)%]") and cfg.main.deletespec and cfg.main.deleteactions then
		return false
	end


	if text:find("Àäìèíèñòðàòîð (.*)%[(%d+)%] îòâåòèë èãðîêó (.*)%[(%d+)%]:{ffffff} (.*)") then
		nickadmina, idadmina, nickplayer, idplayer, textadmin = text:match(
			"Àäìèíèñòðàòîð (.*)%[(%d+)%] îòâåòèë èãðîêó (.*)%[(%d+)%]:{ffffff} (.*)")
		sampAddChatMessage(
			'[A] ' .. nickadmina .. '[' .. idadmina ..
			'] îòâåòèë èãðîêó ' .. nickplayer .. '[' .. idplayer .. ']: {ffffff}' .. textadmin, 0xFFFAAC58)
		return false
	end


	--if text:find("%[A%] {......}(.*) {......}(%w+_?%w+)%[(%d+)%]: (.*)") then
	--	dolzhka, nickadmina, idadmina, textadmina = text:match("%[A%] {......}(.*) {......}(%w+_?%w+)%[(%d+)%]: (.*)")
	--	sampAddChatMessage('['..dolzhka..'] {'..cfg.main.twocoloradminchat..'}'..nickadmina..'['..idadmina..']{'..cfg.main.coloradminchat..'}: '..textadmina, '0xFF'..cfg.main.coloradminchat)
	--	return false
	--end
	...
	if text:find("%[A%] {......}(%w+_?%w+)%[(%d+)%]: {......}(.*)") then
		...
		sampAddChatMessage(
			'[Áåç äîëæíîñòè] {' ..
			cfg.main.twocoloradminchat .. '}' .. nickadmina ..
			'[' .. idadmina .. ']{' .. cfg.main.coloradminchat .. '}: ' .. textadmina, '0xFF' .. cfg.main.coloradminchat)
		return false
	end
	...
	if not isGamePaused() then
		...
		if text:find("%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {......}(.*)") then
			nameadmin = text:match("%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {......}(.*)")
			if cfg.main.mylvladmin == 8 then
				sampAddChatMessage(
					'%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {fe4e4e}Íà÷àëüíèê [LVL: ' ..
					cfg.main.mylvladmin .. ' | FullDostup: ' .. cfg.main.mylvlfd .. ']', -1)
			else
				sampAddChatMessage('%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {00ff00}' .. nameadmin, -1)
			end
...
function imgui.GetOpraStatus(count, tInfoJail)
	...
	if not tInfoJail[1] and tInfoJail[2] == nil and tInfoJail[3] == nil then
		...
		imgui.Hint("status_ajail:" .. count, u8("Îïðîâåðæåíèå íå çàïðîøåíî"))
	elseif tInfoJail[1] and tInfoJail[2] and tInfoJail[3] then
		imgui.TextColored(imgui.ImVec4(1.0, 1.0, 0.3, 1.0), fa.ICON_FA_CLOCK)
		imgui.Hint("status_ajail:" .. count,
			u8("Îïðîâåðæåíèå çàïðîøåíî\nÇàïðîñèë: " .. tInfoJail[2] .. "\nÂðåìÿ: " .. os.date("%H:%M:%S", tInfoJail[3])))
	elseif not tInfoJail[1] and tInfoJail[2] and tInfoJail[3] then
		...
		imgui.Hint("status_ajail:" .. count,
			u8("Îïðîâåðæåíèå ïðîâåðåíî\nÂûïóñòèë: " .. tInfoJail[2] .. "\nÂðåìÿ: " .. os.date("%H:%M:%S", tInfoJail[3])))
	else
		imgui.TextColored(imgui.ImVec4(1.0, 0.3, 0.3, 1.0), fa.ICON_FA_QUESTION_CIRCLE)
		imgui.Hint("status_ajail:" .. count, u8 "Íåèçâåñòíàÿ îøèáêà")
	end
end
...
function tpcar(arg)
	if arg:find('(.+) (.+)') then
		...
		lua_thread.create(function()
			...
			sampSendChat("/pm " .. arg1 .. " 0 Òðàíñïîðò [ID" .. arg2 .. "] òåëåïîðòèðîâàí.")
		end)
	else
		sampAddChatMessage(tag .. "Èñïîëüçóéòå: {E78284}/tpcar [ID player] [ID car]", -1)
	end
end
...
function giveskin(arg)
	if arg:find('(.+) (.+)') then
		...
		lua_thread.create(function()
			...
			sampSendChat("/pm " .. arg1 .. " 0 Ñïàñèáî çà ó÷àñòèå â ðàçäà÷å!")
		end)
	else
		sampAddChatMessage(tag .. "Èñïîëüçóéòå: {E78284}/ss [ID player] [ID skin]", -1)
	end
end
...
function isPos()
	if changePositionRecon then
		...
		if isKeyJustPressed(49) then
			...
			sampAddChatMessage(tag .. 'Íîâîå ìåñòîïîëîæåíèå ñîõðàíåíî.', -1)
		...
		if isKeyJustPressed(50) then
			showCursor(false, false)
			changePositionRecon = false
			sampAddChatMessage(tag .. 'Âû îòìåíèëè èçìåíåíèå ïîçèöèè.', -1)
			MainWindow.v = true
		end
	end
end
...
function kotofeev()
	if imgui.ColorEdit3(u8 'Îñíîâíîé öâåò àäìèí-÷àòà', coloradmchat, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		color_adminchat()
	end
	if imgui.ColorEdit3(u8 'Äîïîëíèòåëüíûé öâåò àäìèí-÷àòà', twocoloradmchat, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		twocolor_adminchat()
	end
	if imgui.ColorEdit3(u8 'Îñíîâíîé öâåò ðåïîðòà', colorreport, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		color_report()
	end
	if imgui.ColorEdit3(u8 'Äîïîëíèòåëüíûé öâåò ðåïîðòà', twocolorreport, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		twocolor_report()
	end
	if imgui.ColorEdit3(u8 'Îñíîâíîé öâåò ôîðì èç àäìèí-÷àòà', colorforms, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		color_forms()
	end
	if imgui.ColorEdit3(u8 'Äîïîëíèòåëüíûé öâåò ôîðì èç àäìèí-÷àòà', twocolorforms, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		twocolor_forms()
	end
end
...
function imgui.Hint(str_id, hint, delay, width)
	...
	local showHint = function(text, alpha, max_width)
		...
		imgui.TextColored(imgui.ImVec4(col.x, col.y, col.z, 1.00), fa.ICON_FA_INFO_CIRCLE .. u8 ' Ïîäñêàçêà:')
...
function kotofeev3()
	...
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü ïàíåëü àäìèíèñòðàòîðà')
	imadd.HotKey('##123123123', facceptform, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Ïðèíÿòü ôîðìó èç àäìèí-÷àòà')
	imadd.HotKey('##1231231123523', fopentools, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü ìåíþ íàñòðîåê ñêðèïòà')
	imadd.HotKey('##openmenutp', fopenmenutp, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü ìåíþ òåëåïîðòà')
	imadd.HotKey('##openme45689nutp', fgm, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Àêòèâèðîâàòü/äåàêòèâèðîâàòü GodMode')
	imadd.HotKey('##openme45689nvbncvcutp', freoff, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Âûéòè èç ñëåæêè')
	imadd.HotKey('##opbncvcutp', fvizualrep, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Âûâåñòè ïîñëåäíèé ðåïîðò â âèçóàëüíûé ÷àò')
	imadd.HotKey('##opbncvcmnnutp', farep, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Âûâåñòè ïîñëåäíèé ðåïîðò â àäìèí-÷àò')
	imadd.HotKey('##openreport', fopenreport, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü ðåïîðò')
	imadd.HotKey('##lastreport', flastreport, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Âûâåñòè /pm íà ID àâòîðà ïîñëåäíåãî ðåïîðòà')
	imadd.HotKey('##lasddtreport', fnorules, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Íàïèñàòü àâòîðó ðåïîðòà î òîì, ÷òî èãðîê íå íàðóøàåò')
	imadd.HotKey('##lasddnbtreport', fspecrep, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Âñòóïèòü â ñëåæêó çà âîçìîæíûì íàðóøèòåëåì [ðåïîðò]')
	imadd.HotKey('##lasddnbtrephhhhort', fspecauthor, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Âñòóïèòü â ñëåæêó çà àâòîðîì [ðåïîðò]')
	imadd.HotKey('##lasddnbtrephzzzzzzzzzzzzzzzhhhort', fperedam, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Ïåðåäàòü àäìèíèñòðàöèè [ðåïîðò]')
	imadd.HotKey('##slapnut', fslap, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Ñëàïíóòü ñåáÿ')
	imadd.HotKey('##vidatjp', fjetpack, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Âûäàòü ñåáå JetPack')
	imadd.HotKey('##famem', famember, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü äèàëîã äëÿ âñòóïëåíèÿ âî ôðàêöèþ')
	imadd.HotKey('##speclastrep', fspeclast, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Ñëåäèòü çà àâòîðîì ïîñëåäíåãî ðåïîðòà')
	imadd.HotKey('##spec12lastrep', fopenmap, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü êàðòó')
	imadd.HotKey('##flovlyareporta', flovlyareporta, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Àêòèâèðîâàòü/äåàêòèâèðîâàòü àâòîëîâëþ ðåïîðòà')
	imadd.HotKey('##adsads', fopenpunish, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü ìåíþ âûäà÷è íàêàçàíèé')
	imadd.HotKey('##fairbrake', fairbrake, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Àêòèâèðîâàòü/äåàêòèâèðîâàòü AirBrake')
	imadd.HotKey('##upair', upair, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Ïîâûñèòü ñêîðîñòü AirBrake')
	imadd.HotKey('##fspeeddown', fspeeddown, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Ïîíèçèòü ñêîðîñòü AirBrake')
	imadd.HotKey('##fvzaim', fvzaim, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Âçàèìîäåéñòâèå ñ èãðîêàìè â ðàäèóñå')
	imadd.HotKey('##ftpaz', ftpaz, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Òåëåïîðòèðîâàòüñÿ â àäìèí-çîíó')
	imadd.HotKey('##fjspeedup', fjspeedup, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Ïîâûñèòü ñêîðîñòü JetPack')
	imadd.HotKey('##fjspeeddown', fjspeeddown, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Ïîíèçèòü ñêîðîñòü JetPack')
	imadd.HotKey('##fopencbiz', fopencbiz, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü îòäåë êîììåðöèè')
	imadd.HotKey('##fopenchome', fopenchome, {}, 100)
	imgui.SameLine()
	imgui.Text(u8 'Îòêðûòü îòäåë íåäâèæèìîñòè')
end
...
function peredam()
	if imgui.Button(fa.ICON_FA_COMMENTS .. u8 ' Ïåðåäàòü àäì ðåï', imgui.ImVec2(145, 20)) then
		local template = u8:decode(reason_settings_report[9].v)
		local template = template:gsub('{nick_rep}', nick_rep)
		local template = template:gsub('{my_id}', my_id)
		local template = template:gsub('{id_rep}', id_rep)
...
function imgui.CogButton()
	imgui.SameLine()
	imgui.TextDisabled(u8 '<< Ñëåäèòü ' .. fa.ICON_FA_ROBOT)
...
function imgui.SettingsButton()
	...
	imgui.PopFont()
	imgui.HintTwo(u8 'Íàñòðîéêè ôóíêöèè')
	return imgui.IsItemClicked()
end
...
function twocolor_adminchat()
	...
	cfg.twocoloradmchat = {
		r = twocoloradmchat.v[1] * 255,
		g = twocoloradmchat.v[2] * 255,
		b = twocoloradmchat.v[3] *
			255,
	}
...
function stock_colorchecker()
	...
	cfg.stockcolorchecker = {
		r = stockcolorchecker.v[1] * 255,
		g = stockcolorchecker.v[2] * 255,
		b = stockcolorchecker
			.v[3] * 255,
	}
...
function afk_colorchecker()
	...
	cfg.afkcolorchecker = {
		r = afkcolorchecker.v[1] * 255,
		g = afkcolorchecker.v[2] * 255,
		b = afkcolorchecker.v[3] *
			255,
	}
...
function kotofeevformi()
	...
	imgui.Separator()
	imgui.TextColoredRGB(
		'{E78284}*Ð {84A6E7}- ïðèíèìàòü âðó÷íóþ\n{E78284}*À{84A6E7} - ïðèíèìàòü àâòîìàòè÷åñêè\n{E78284}*Ç{84A6E7} - çâóêîâîå îïîâåùåíèå')
	imgui.BeginChild("ChildWin1231233dow", imgui.ImVec2(140, 1400), true)
	imgui.CenterText(u8 'Êîìàíäà')
	imgui.Separator()
	...
	imgui.CenterText(u8 'Ç')
	imgui.Separator()
	if imgui.Checkbox('##èäèíàõóé122123', notf_slap) then
...
function kotofeevstatsadm()
	imgui.Text(u8 'Ìåñòîïîëîæåíèå îêíà ñòàòèñòèêè:')
	imgui.SameLine()
	if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '', imgui.ImVec2(-1, 25)) then
		if to.v then
			ChangePosWindow.v = true
		else
			sampAddChatMessage(tag .. 'Ïåðåìåùåíèå íåâîçìîæíî ïîêà âûêëþ÷åíà ïàíåëü èíôîðìàöèè', -1)
		end
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü îíëàéí (òåêóùàÿ ñåññèÿ)', Radio['sesOnline']) then
		Radio['sesOnline'] = not Radio['sesOnline']; cfg.statTimers.sesOnline = Radio['sesOnline']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü îíëàéí çà äåíü', Radio['dayOnline']) then
		Radio['dayOnline'] = not Radio['dayOnline']; cfg.statTimers.dayOnline = Radio['dayOnline']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü îíëàéí â àôê', Radio['dayAfk']) then
		Radio['dayAfk'] = not Radio['dayAfk']; cfg.statTimers.dayAfk = Radio['dayAfk']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü îíëàéí çà íåäåëþ (áåç ó÷åòà àôê)', Radio['weekOnline']) then
		Radio['weekOnline'] = not Radio['weekOnline']; cfg.statTimers.weekOnline = Radio['weekOnline']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü îíëàéí àôê çà íåäåëþ', Radio['weekAfk']) then
		Radio['weekAfk'] = not Radio['weekAfk']; cfg.statTimers.weekAfk = Radio['weekAfk']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü îáùèé îíëàéí çà íåäåëþ (ñ ó÷åòîì àôê)', Radio['weekFull']) then
		Radio['weekFull'] = not Radio['weekFull']; cfg.statTimers.weekFull = Radio['weekFull']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü âûäàííûå íàêàçàíèÿ çà äåíü', Radio['nakazaniyaDay']) then
		Radio['nakazaniyaDay'] = not Radio['nakazaniyaDay']; cfg.statTimers.nakazaniyaDay = Radio['nakazaniyaDay']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü âûäàííûå íàêàçàíèÿ çà âñå âðåìÿ', Radio['nakazaniyaVse']) then
		Radio['nakazaniyaVse'] = not Radio['nakazaniyaVse']; cfg.statTimers.nakazaniyaVse = Radio['nakazaniyaVse']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü âûäàííûå ôîðì çà äåíü', Radio['formsDay']) then
		Radio['formsDay'] = not Radio['formsDay']; cfg.statTimers.formsDay = Radio['formsDay']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü âûäàííûå ôîðì çà âñå âðåìÿ', Radio['formsVse']) then
		Radio['formsVse'] = not Radio['formsVse']; cfg.statTimers.formsVse = Radio['formsVse']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü ðåïîðòîâ îòâå÷åíî çà äåíü', Radio['reportsDay']) then
		Radio['reportsDay'] = not Radio['reportsDay']; cfg.statTimers.reportsDay = Radio['reportsDay']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü ðåïîðòîâ îòâå÷åíî çà âñå âðåìÿ', Radio['reportsVse']) then
		Radio['reportsVse'] = not Radio['reportsVse']; cfg.statTimers.reportsVse = Radio['reportsVse']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü êîë-âî ðåïîðòîâ íà äàííûé ìîìåíò', Radio['reportnow']) then
		Radio['reportnow'] = not Radio['reportnow']; cfg.statTimers.reportnow = Radio['reportnow']
	end
	if imgui.RadioButton(u8 'Îòîáðàæàòü òåêóùåå âðåìÿ', Radio['nowTime']) then
	...
	imgui.PopFont()
	if sampGetGamestate() ~= 3 then
		imgui.TextColoredRGB("Ïîäêëþ÷åíèå ê ñåðâåðó:\n" .. get_clock(connectingTime))
	else
		if cfg.statTimers.sesOnline then imgui.TextColoredRGB("Òåêóùàÿ ñåññèÿ: " .. get_clock(sesOnline.v)) end
		if cfg.statTimers.dayOnline then imgui.TextColoredRGB("Îíëàéí çà äåíü: " .. get_clock(cfg.onDay.online)) end
		if cfg.statTimers.dayAfk then imgui.TextColoredRGB("Îíëàéí â àôê: " .. get_clock(cfg.onDay.afk)) end
		if cfg.statTimers.reportsDay then imgui.TextColoredRGB("Ðåïîðòîâ çà äåíü: " .. cfg.main.reportzaday) end
		if cfg.statTimers.reportsVse then imgui.TextColoredRGB("Ðåïîðòîâ çà âñå âðåìÿ: " .. cfg.main.reportzavse) end
		if cfg.statTimers.nakazaniyaDay then imgui.TextColoredRGB("Íàêàçàíèé çà äåíü: " .. cfg.main.nakazaniyaday) end
		if cfg.statTimers.nakazaniyaVse then imgui.TextColoredRGB("Íàêàçàíèé çà âñå âðåìÿ: " .. cfg.main.nakazaniyavse) end
		if cfg.statTimers.formsDay then imgui.TextColoredRGB("Ôîðì çà äåíü: " .. cfg.main.formsday) end
		if cfg.statTimers.formsVse then imgui.TextColoredRGB("Ôîðì çà âñå âðåìÿ: " .. cfg.main.formsvse) end
		if cfg.statTimers.nowTime then imgui.TextColoredRGB("Òåêóùåå âðåìÿ: " .. nowTime123) end
		if cfg.statTimers.weekOnline then imgui.TextColoredRGB("Çà íåäåëþ (÷èñòûé): " .. get_clock(cfg.onWeek.online)) end
		if cfg.statTimers.weekAfk then imgui.TextColoredRGB("ÀÔÊ çà íåäåëþ: " .. get_clock(cfg.onWeek.afk)) end
		if cfg.statTimers.weekFull then imgui.TextColoredRGB("Îíëàéí çà íåäåëþ: " .. get_clock(cfg.onWeek.full)) end
		if cfg.statTimers.reportnow then imgui.TextColoredRGB("Ðåïîðòà ñåé÷àñ: " .. reportnow) end
	end
end
...
function yuiop()
	...
	if sampGetGamestate() ~= 3 then
		imgui.TextColoredRGB("Ïîäêëþ÷åíèå: " .. get_clock(connectingTime))
	else
		if cfg.statTimers.sesOnline then imgui.TextColoredRGB("Òåêóùàÿ ñåññèÿ: " .. get_clock(sesOnline.v)) end
		if cfg.statTimers.dayOnline then imgui.TextColoredRGB("Îíëàéí çà äåíü: " .. get_clock(cfg.onDay.online)) end
		if cfg.statTimers.dayAfk then imgui.TextColoredRGB("Îíëàéí â àôê: " .. get_clock(cfg.onDay.afk)) end
...
function lyalyalyalya2()
	...
	if changePositionStats then
		if isKeyDown(vkeys.VK_LBUTTON) then
			cfg.pos.x = posX123.v
			cfg.pos.y = posY123.v
			sampAddChatMessage(tag .. 'Ïîçèöèÿ ñòàòèñòèêè óñïåøíî ñîõðàíåíà!', -1)
			imgui.ShowCursor = false
...
function lyalyalyalya5()
	...
	if changePositionStatss then
		if isKeyDown(vkeys.VK_LBUTTON) then
			...
			sampAddChatMessage(tag .. 'Ïîçèöèÿ îêíà óñïåøíî ñîõðàíåíà!', -1)
			imgui.ShowCursor = false
...
function lyalyalyalya6()
	...
	if changePositionStatsss then
		if isKeyDown(vkeys.VK_LBUTTON) then
			...
			sampAddChatMessage(tag .. 'Ïîçèöèÿ îêíà èíôîðìàöèè óñïåøíî ñîõðàíåíà!', -1)
			imgui.ShowCursor = false
...
function lyalyalyalya4()
	...
	if changePositionStat1s then
		if isKeyDown(vkeys.VK_LBUTTON) then
			...
			sampAddChatMessage(tag .. 'Ïîçèöèÿ ÷åêåðà èãðîêîâ óñïåøíî ñîõðàíåíà!', -1)
			imgui.ShowCursor = false
...
function kotofeevautoopra()
	if logBut["selected"] == 1 then
		imgui.Text(u8 'Âðåìÿ, íà êîòîðîå áóäåò ñàäèòüñÿ èãðîê â äåìîðãàí ïðè çàïðîñå îïðîâåðæåíèÿ:')
		imgui.SameLine()
		imgui.PushItemWidth(100)
		...
		imgui.Text(u8 'Êîìàíäà äëÿ ðó÷íîãî çàïðîñà îïðîâåðæåíèÿ íà ïîñëåäíèé ñëîâëåííûé äîì:')
		imgui.SameLine()
		imgui.PushItemWidth(100)
		...
		imgui.PopItemWidth()
		imgui.Text(u8 'Êîìàíäà äëÿ ðó÷íîãî çàïðîñà îïðîâåðæåíèÿ íà ïîñëåäíèé ñëîâëåííûé áèçíåñ:')
		imgui.SameLine()
		imgui.PushItemWidth(100)
		...
		imgui.PopItemWidth()
		imgui.Separator()
		imgui.TextColoredRGB('{E78284}Óêàæèòå ïðè÷èíó äåìîðãàíà ïðè çàïðîñå îïðîâåðæåíèÿ íà äîì, äîñòóïíûå òýãè:')
		imgui.TextColoredRGB('{E78284}{idhouse} {84A6E7}- ID ñëîâëåííîãî äîìà')
		imgui.TextColoredRGB('{E78284}{time} {84A6E7}- âðåìÿ ââîäà êàï÷è')
		imgui.TextColoredRGB('{E78284}{captcha} {84A6E7}- ââåä¸ííàÿ êàï÷à')
		if imgui.InputText('##reasonhouse', reason_autoopra[1]) then
			cfg.autoopra.myreasonhouse = tostring(u8:decode(reason_autoopra[1].v))
		end
		imgui.TextColoredRGB('{E78284}Óêàæèòå ïðè÷èíó äåìîðãàíà ïðè çàïðîñå îïðîâåðæåíèÿ íà áèçíåñ, äîñòóïíûå òýãè:')
		imgui.TextColoredRGB('{E78284}{idbiz} {84A6E7}- ID ñëîâëåííîãî áèçíåñà')
		imgui.TextColoredRGB('{E78284}{time} {84A6E7}- âðåìÿ ââîäà êàï÷è')
		imgui.TextColoredRGB('{E78284}{captcha} {84A6E7}- ââåä¸ííàÿ êàï÷à')
	...
	elseif logBut["selected"] == 5 then
		imgui.Text(u8 'Àêòèâàöèÿ àâòî-îïðîâåðæåíèÿ íà äîìà è áèçíåñû: /autoopra, /aopra')
		imgui.Separator()
		if imgui.ToggleButton(u8 'Çàïðàøèâàòü îïðîâåðæåíèå íà ëîâëþ áèçíåñîâ', checkautooprabiz) then
			cfg.autoopra.settingbiz = not cfg.autoopra.settingbiz
		end
		imgui.SameLine()
		imgui.Text(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
		imgui.Hint('bizopra',
			u8 'Ïðè ââîäå êîìàíäû /autoopra áóäåò âêëþ÷åíà àâòî-îïðà íà áèçíåñû (ò.å. åñëè ÷åëîâåê ïîéìàë áèçíåñ - åãî ïîñàäèò â äåìîðãàí)')
		if imgui.ToggleButton(u8 'Çàïðàøèâàòü îïðîâåðæåíèå íà ëîâëþ äîìîâ', checkautooprahouse) then
			cfg.autoopra.settinghouse = not cfg.autoopra.settinghouse
		end
		imgui.SameLine()
		imgui.Text(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
		imgui.Hint('houseopra',
			u8 'Ïðè ââîäå êîìàíäû /autoopra áóäåò âêëþ÷åíà àâòî-îïðà íà äîìà (ò.å. åñëè ÷åëîâåê ïîéìàë äîì - åãî ïîñàäèò â äåìîðãàí)')
	elseif logBut["selected"] == 3 then
		...
		if #log_full == 0 then
			...
			imgui.CenterTextColoredRGB("{ffff00}Ëîã ïîêóïêè èìóùåñòâà ïóñòîé")
		else
	...
	elseif logBut["selected"] == 2 then
		if #tLog == 0 then
			imgui.SetCursorPosY((420 - 20) / 2)
			imgui.CenterTextColoredRGB(lc .. "Ëîã ïóñòîé")
		end
		...
		for k, box in pairs(tLog) do
			if box[1] == 1 then
				...
				imgui.SameLine(30)
				imgui.Text(string.format(u8 "| Èãðîê %s êóïèë äîì ID: %s ïî ãîñ. öåíå çà %s! Êàï÷à: %s", box[4][1],
					box[4][2], box[4][3], box[4][4]))
				imgui.Hint("plInt:" .. k, u8(os.date("%H:%M:%S", box[2]) .. "\nÏÊÌ - Âçàèìîäåéñòâîâàòü"))
				if imgui.IsItemClicked(1) then
					contK = k; imgui.OpenPopup("##toolContext")
				end
			...
			if box[1] == 3 then
				...
				imgui.SameLine(30)
				imgui.Text(string.format(u8 "| Èãðîê %s êóïèë áèçíåñ ID: %s ïî ãîñ. öåíå çà %s! Êàï÷à: %s", box[4][1],
					box[4][2], box[4][3], box[4][4]))
				imgui.Hint("plInt:" .. k, u8(os.date("%H:%M:%S", box[2]) .. "\nÏÊÌ - Âçàèìîäåéñòâîâàòü"))
				if imgui.IsItemClicked(1) then
					contK = k; imgui.OpenPopup("##toolContext")
		...
		if contK ~= nil and imgui.BeginPopupContextItem("##toolContext") then
			...
			imgui.CenterTextColoredRGB(lc ..
				tLog[contK][4][1] ..
				(plOnline and mc .. " [Online]" or ec .. " [Offline]") ..
				"\n" .. lc .. "Âðåìÿ ïîêóïêè: " .. os.date("%H:%M:%S", tLog[contK][2]))
			if not tLog[contK][5][1] then
				if imgui.Button(fa.ICON_FA_BED .. u8(" Çàïðîñèòü îïðó" .. (plOnline and "" or " â îôôëàéíå")), imgui.ImVec2(220, 30)) then
					local jtype = tLog[contK][1]
					sampSendChat(string.format("/jail%s %s %s îïðà [¹%s | %s]", (plOnline and "" or "off"),
						tLog[contK][4][1], cfg.main.timeopra, tLog[contK][4][2], tLog[contK][4][3]))
					imgui.CloseCurrentPopup()
				end
			else
				if imgui.Button(fa.ICON_FA_CHECK .. u8(" Îäîáðèòü"), imgui.ImVec2(107.5, 30)) then
					sampSendChat(string.format("/unjail%s %s îïðà ïðèíÿòà", (plOnline and "" or "off"), tLog[contK][4]
						[1]))
					imgui.CloseCurrentPopup()
				end
				imgui.SameLine()
				if imgui.Button(fa.ICON_FA_TIMES .. u8(" Îòêëîíèòü"), imgui.ImVec2(107.5, 30)) then
					sampSendChat(string.format("/unjail%s %s îïðà îòêàçàíà", (plOnline and "" or "off"),
						tLog[contK][4][1]))
					imgui.CloseCurrentPopup(); MainWindow.v = false
				end
			end
			if imgui.Button(fa.ICON_FA_PRINT .. u8 " Ñòðîêà ïîêóïêè â ÷àò", imgui.ImVec2(220, 30)) then
				sampSetChatInputEnabled(true)
				sampSetChatInputText(tLog[contK][3])
				imgui.CloseCurrentPopup()
				MainWindow.v = false
			end
			if imgui.Button(fa.ICON_FA_ADDRESS_CARD .. u8 " Èñòîðèÿ íàêàçàíèé", imgui.ImVec2(220, 30)) then
				sampSendChat("/checkpunish " .. tLog[contK][4][1])
				imgui.CloseCurrentPopup()
				MainWindow.v = false
			end
			if imgui.ButtonHex(fa.ICON_FA_TRASH .. u8 " Óäàëèòü ýòîò ëîã", 0xFFBB2020, imgui.ImVec2(220, 30)) then
	...
	elseif logBut["selected"] == 4 then
		if imgui.ToggleButton(u8 'Óâåäîìëåíèå â ÷àò î ïîêóïêå èìóùåñòâà èãðîêîì èç áåëîãî ñïèñêà', imgui.ImBool(cfg.main.notfinchat)) then
		...
		imgui.PopFont()
		imgui.Hint('helplist',
			u8 'Äîïîëíèòåëüíîå îïîâåùåíèå â ÷àò â ñëó÷àå åñëè èãðîê, ïîéìàâøèé\nèìóùåñòâî, íàõîäèëñÿ â áåëîì ñïèñêå')
		if imgui.ToggleButton(u8 'Óäàëåíèå ñîîáùåíèÿ î ïîêóïêå èìóùåñòâà', imgui.ImBool(cfg.main.delinchat)) then
		...
		imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
		imgui.PopFont()
		imgui.Hint('helplis1t',
			u8 'Ñêðèïò áóäåò óäàëÿòü ñåðâåðíîå ñîîáùåíèå î ïîêóïêå èìóùåñòâà èãðîêîì èç áåëîãî ñïèñêà')
		...
		if #addIgnore.v > 0 then
			...
			imgui.DisableButton(u8 "Äîáàâèòü", imgui.ImVec2(-1, 20))
		end
		if #cfg.ignore == 0 then
			imgui.SetCursorPosY((imgui.GetWindowHeight() - 10) / 2)
			imgui.CenterTextColoredRGB("{ffff00}Ñïèñîê èãðîêîâ â áåëîì ñïèñêå ïóñò")
		end
...
function spawnset()
	...
	sampAddChatMessage(
		'{E78284}[AdminTools] {84A6E7}Âû óñòàíîâèëè íîâóþ òî÷êó ñïàâíà {E78284}[' ..
		myCoordinates[1] .. ' | ' .. myCoordinates[2] .. ' | ' .. myCoordinates[3] .. ']', -1)
end
...
function rek_off()
	...
	sampAddChatMessage(
		tag ..
		'Îòîáðàæåíèå ñîîáùåíèé î ïîäîçðåíèè íà ðåêëàìó ' ..
		(cfg.main.deletereklama and '{ff4040}âûêëþ÷åíî{84A6E7}.' or '{00ff00}âêëþ÷åíî{84A6E7}.'), -1)
end
...
function playVolume()
	...
	if not doesFileExist(notification) then
		sampAddChatMessage('[AdminTools] Çâóêîâîé ôàéë îòñóòñòâóåò.', 14628149)
	else
...
function arep()
	if isCharInAnyCar(PLAYER_PED) then
		repcar = true
		sampSendChat('/apanel')
		sampAddChatMessage('[AdminTools] Òðàíñïîðò ïî÷èíåí.', 14628149)
	else
		sampAddChatMessage('[AdminTools] Âû íå íàõîäèòåñü â òðàíñïîðòå.', 14628149)
	end
end
...
function ammo()
	addammo = true
	sampSendChat('/apanel')
	sampAddChatMessage('[AdminTools] Òîâàð â àììóíèöèÿõ óñïåøíî ïîïîëíåí.', 14628149)
end
...
function ohome()
	if nickp == nil or idp == nil or idhouse == nil or vremya == nil or captcha1 == nil or captcha2 == nil then
		sampAddChatMessage(tag .. 'Íèêòî íå ëîâèë äîìîâ â ïîñëåäíåå âðåìÿ :(', -1)
	else
		if cfg.autoopra.myreasonhouse then
			local template = u8:decode(reason_autoopra[1].v)
			local template = template:gsub('{idhouse}', idhouse)
...
function SettingsTraicer()
	...
	imgui.PushFont(fs16)
	imgui.CenterTextColoredRGB('{E3C47F}Íàñòðîéêè ñâîèõ ïóëü')
	imgui.PopFont()
	imgui.Separator()
	if imgui.ToggleButton(u8 "Îòðèñîâêà ñâîèõ ïóëü", elements.checkbox.drawMyBullets) then
		cfg.config.drawMyBullets = elements.checkbox.drawMyBullets.v
		inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
	end
	...
	imgui.SameLine()
	imgui.Text(u8 "Âðåìÿ çàäåðæêè òðåéñåðà")
	...
	imgui.SameLine()
	imgui.Text(u8 "Ìàêñèìàëüíîå êîëè÷åñòâî ëèíèé")

	imgui.Separator()

	if imgui.ToggleButton(u8 "Îêîí÷àíèÿ ó ëèíèé##1", elements.checkbox.cbEndMy) then
	...
	imgui.SameLine()
	imgui.Text(u8 "Êîëè÷åñòâî óãëîâ íà îêîí÷àíèÿõ")
	...
	imgui.SameLine()
	imgui.Text(u8 "Ãðàäóñ ïîâîðîòà îêîí÷àíèÿ")
	...
	imgui.SameLine()
	imgui.Text(u8 "Ðàçìåð îêîí÷àíèÿ òðåéñåðà")

	imgui.Separator()

	imgui.PopItemWidth()
	imgui.Text(u8 "Óêàæèòå öâåò òðåéñåðà, åñëè: ")
	...
	if imgui.ColorEdit4("##dinamicObjectMy", dinamicObjectMy, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		cfg.config.dinamicObjectMy = join_argb(dinamicObjectMy.v[1] * 255, dinamicObjectMy.v[2] * 255,
			dinamicObjectMy.v[3] * 255, dinamicObjectMy.v[4] * 255)
		inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
	end
	imgui.SameLine()
	imgui.Text(u8 "Âû ïîïàëè â äèíàìè÷åñêèé îáúåêò")
	if imgui.ColorEdit4("##staticObjectMy", staticObjectMy, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		cfg.config.staticObjectMy = join_argb(staticObjectMy.v[1] * 255, staticObjectMy.v[2] * 255,
			staticObjectMy.v[3] * 255, staticObjectMy.v[4] * 255)
		inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
	end
	imgui.SameLine()
	imgui.Text(u8 "Âû ïîïàëè â ñòàòè÷åñêèé îáúåêò")
	...
	imgui.SameLine()
	imgui.Text(u8 "Âû ïîïàëè â èãðîêà")
	...
	imgui.SameLine()
	imgui.Text(u8 "Âû ïîïàëè â ìàøèíó")
	imgui.PopItemWidth()
	imgui.Separator()
	imgui.PushFont(fs16)
	imgui.CenterTextColoredRGB('{E3C47F}Íàñòðîéêè ÷óæèõ ïóëü')
	imgui.PopFont()
	imgui.Separator()
	if imgui.ToggleButton(u8 "Îòðèñîâêà ÷óæèõ ïóëü", elements.checkbox.drawBullets) then
		cfg.config.drawBullets = elements.checkbox.drawBullets.v
		inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
	end
	imgui.PushItemWidth(175)
	...
	if imgui.ToggleButton(u8 "Îêîí÷àíèÿ ó ëèíèé##2", elements.checkbox.cbEnd) then
	...
	if imgui.ToggleButton(u8 "ÈÄ è íèêíåéì èãðîêà ïðè âûñòðåëå", elements.checkbox.showPlayerInfo) then
		cfg.config.showPlayerInfo = elements.checkbox.showPlayerInfo.v
		inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
	end

	if imgui.Button(u8 "Íàñòðîèòü èíôîðìàöèþ î ñòðåëêå", imgui.ImVec2(325, 0)) then
		imgui.OpenPopup(u8 "Íàñòðîéêà èíôîðìàöèè î ñòðåëêå")
	end

	if imgui.BeginPopup(u8 "Íàñòðîéêà èíôîðìàöèè î ñòðåëêå") then
		if imgui.ToggleButton(u8 "Ïîêàçûâàòü ÈÄ", elements.checkbox.onlyId) then
			cfg.config.onlyId = elements.checkbox.onlyId.v
			inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
		end
		if imgui.ToggleButton(u8 "Ïîêàçûâàòü íèêíåéì", elements.checkbox.onlyNick) then
	...
	if imgui.ColorEdit4("##infoNickId", colorPlayerI, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		cfg.config.colorPlayerI = join_argb(colorPlayerI.v[1] * 255, colorPlayerI.v[2] * 255, colorPlayerI.v[3] * 255,
			colorPlayerI.v[4] * 255)
		inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
	end
	imgui.SameLine()
	imgui.Text(u8 "Óêàæèòå öâåò íèêíåéìà ñòðåëêà")
	...
	if imgui.ColorEdit4("##dinamicObject", dinamicObject, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		cfg.config.dinamicObject = join_argb(dinamicObject.v[1] * 255, dinamicObject.v[2] * 255, dinamicObject.v[3] * 255,
			dinamicObject.v[4] * 255)
		inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
	end
	imgui.SameLine()
	imgui.Text(u8 "Èãðîê ïîïàë â äèíàìè÷åñêèé îáúåêò")
	if imgui.ColorEdit4("##staticObject", staticObject, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
		cfg.config.staticObject = join_argb(staticObject.v[1] * 255, staticObject.v[2] * 255, staticObject.v[3] * 255,
			staticObject.v[4] * 255)
		inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
	end
	imgui.SameLine()
	imgui.Text(u8 "Èãðîê ïîïàë â ñòàòè÷åñêèé îáúåêò")
	...
	imgui.SameLine()
	imgui.Text(u8 "Èãðîê ïîïàë â èãðîêà")
	...
	imgui.SameLine()
	imgui.Text(u8 "Èãðîê ïîïàë â ìàøèíó")
	imgui.PopItemWidth()
	imgui.Separator()
	imgui.EndChild()
end
...
function obiz()
	if nickp == nil or idp == nil or idbiz == nil or vremya == nil or captcha1 == nil or captcha2 == nil then
		sampAddChatMessage(tag .. 'Íèêòî íå ëîâèë áèçíåñîâ â ïîñëåäíåå âðåìÿ :(', -1)
	else
		if cfg.autoopra.myreasonbiz then
			local template = u8:decode(reason_autoopra[2].v)
			...
			local result = sampIsPlayerConnected(idp)
			if not result then
				send_rpc_command('/jailoff ' .. nickp .. ' ' .. cfg.main.timeopra .. ' îïðà áèçíåñ ¹' .. idbiz)
			else
				send_rpc_command('/jail ' .. idp .. ' ' .. cfg.main.timeopra .. ' îïðà áèçíåñ ¹' .. idbiz)
			end
		end
	end
end
...
function clr(arg)
	if tonumber(arg) and tonumber(arg) >= 5 and tonumber(arg) <= 100 then
		...
		sampAddChatMessage(tag .. "Óêàæèòå âåðíîå êîëè÷åñòâî ñòðîê [5-100]", -1)
	end
end
...
function calc(params)
	if params == '' then
		sampAddChatMessage(tag .. "Èñïîëüçóéòå: /cal [ïðèìåð]", -1)
	else
		local func = load('return ' .. params)
		if func == nil then
			sampAddChatMessage(tag .. "Îøèáêà.", -1)
		else
			local bool, res = pcall(func)
			if bool == false or type(res) ~= 'number' then
				sampAddChatMessage(tag .. "Îøèáêà.", -1)
			else
				sampAddChatMessage(tag .. "Ðåçóëüòàò: " .. res, -1)
			end
		end
	end
end
...
function ajailThread()
	while true do
		...
		if isGamePaused() and cfg.autoopra.status then
			...
			cfg.autoopra.status = false
			sampShowDialog(5454, tag .. 'Ïðåäóïðåæäåíèå',
				'{488cff}Àâòîìàòè÷åñêèé ðåæèì áûë âûêëþ÷åí èç-çà ñâîðà÷èâàíèÿ èãðû', 'Ïîíÿë', _, 0)
		end
	end
end
...
function tpmetka(arg)
	if not tonumber(arg) then
		sampAddChatMessage(tag .. 'Âû òåëåïîðòèðîâàíû íà ìåòêó íà êàðòå.', -1)
		_, x, y, z = getTargetBlipCoordinatesFixed()
		if not _ then
			sampAddChatMessage(tag .. "Ìåòêà äëÿ òåëåïîðòà {E78284}íå íàéäåíà{84A6E7}.", -1)
		else
	...
	elseif tonumber(arg) == 1 then
		sampAddChatMessage(tag .. 'Âû òåëåïîðòèðîâàíû íà ñåðâåðíóþ ìåòêó.', -1)
		tpcssss()
	end
end
...
function proccessInfo(vehHandle)
	...
	if getDistanceBetweenCoords3d(pedX, pedY, pedZ, carX, carY, carZ) < tonumber(cfg.Settings.distance) then
		...
		if not carRenderText then
			carRenderText = 'Íåèçâåñòíàÿ ìîäåëü ' .. model
		end
...
if cfg.main.theme == 'pink' then
	pinkTheme()
	result = imgui.ImInt(0)
elseif cfg.main.theme == 'darkgreen' then
	DarkGreen()
	result = imgui.ImInt(1)
elseif cfg.main.theme == 'red' then
	Red()
	result = imgui.ImInt(2)
elseif cfg.main.theme == 'purple' then
	Purple()
	result = imgui.ImInt(3)
elseif cfg.main.theme == 'dark' then
	Dark()
	result = imgui.ImInt(4)
else
	cfg.main.theme = 'pink'
	pinkTheme()
	result = imgui.ImInt(0)
...
if cfg.main.numberbind == 0 then
	result1 = imgui.ImInt(0)
elseif cfg.main.numberbind == 1 then
	result1 = imgui.ImInt(1)
elseif cfg.main.numberbind == 2 then
	result1 = imgui.ImInt(2)
elseif cfg.main.numberbind == 3 then
	result1 = imgui.ImInt(3)
elseif cfg.main.numberbind == 4 then
	result1 = imgui.ImInt(4)
elseif cfg.main.numberbind == 5 then
	result1 = imgui.ImInt(5)
elseif cfg.main.numberbind == 6 then
	result1 = imgui.ImInt(6)
elseif cfg.main.numberbind == 7 then
	result1 = imgui.ImInt(7)
elseif cfg.main.numberbind == 8 then
	result1 = imgui.ImInt(8)
else
	cfg.main.numberbind = 0
	result1 = imgui.ImInt(0)
end
...
function imgui.ToggleButton(str_id, bool)
	...
	local col = bool.v and imgui.ColorConvertFloat4ToU32(imgui.ImVec4(imgui.GetStyle().Colors[imgui.Col.Button])) or
		0xFF606060

	draw_list:AddRectFilled(imgui.ImVec2(p.x, p.y + (height / 6)),
		imgui.ImVec2(p.x + width - 1.0, p.y + (height - (height / 6))), col, 5.0)
	draw_list:AddCircleFilled(imgui.ImVec2(p.x + radius + t * (width - radius * 2.0), p.y + radius), radius - 0.75,
		col_static)
...
function regcommands()
	sampRegisterChatCommand('tpp', function()
		TeleportPlayerWindow.v = not TeleportPlayerWindow.v
	end)
	...
	sampRegisterChatCommand('autoopra', function()
		...
		sampAddChatMessage(tag ..
			'Àâòîîïðîâåðæåíèå ' .. (cfg.autoopra.status and '{E78284}âêëþ÷åíî' or '{E78284}âûêëþ÷åíî'), -1)
	end)
	...
	sampRegisterChatCommand("toolsoff", function()
		sampAddChatMessage(tag .. 'Ñêðèïò óñïåøíî âûãðóæåí, äëÿ çàïóñêà èñïîëüçóéòå {E78284}CTRL + R.', -1)
		thisScript():unload()
	end)
	sampRegisterChatCommand("toolsreload", function()
		sampAddChatMessage(tag .. 'Ñêðèïò óñïåøíî ïåðåçàãðóæåí.', -1)
		thisScript():reload()
	end)
end
...
function cccc()
	if cfg.main.playersChecker then
		...
		renderFontDrawTextAlign(fonts.playersChecker, "Èãðîêè îíëàéí: ", cfg.checkerPosition.x, cfg.checkerPosition.y, -1,
			cfg.main.align)
		for k, v in ipairs(J_.PLAYERS_CHECKER[1]) do
						...
						if cfg.main.afkinchecker then
							if sampIsPlayerPaused(id) then
								if fColor == 'fffdfcfc' then
									renderFontDrawTextAlign(fonts.playersChecker,
										"{" ..
										cfg.main.stockcolorchecker ..
										"}" .. v .. " [" .. id .. "] - {" .. cfg.main.afkcolorchecker .. "}AFK", x, y,
										color, cfg.main.align)
								else
									renderFontDrawTextAlign(fonts.playersChecker,
										"{" .. fColor .. "}" .. v .. " [" ..
										id .. "] - {" .. cfg.main.afkcolorchecker .. "}AFK", x, y, color, cfg.main.align)
								end
							else
								if fColor == 'fffdfcfc' then
									renderFontDrawTextAlign(fonts.playersChecker,
										"{" .. cfg.main.stockcolorchecker .. "}" .. v .. " [" .. id .. "]", x, y, color,
										cfg.main.align)
								else
									renderFontDrawTextAlign(fonts.playersChecker, "{" .. fColor .. "}" ..
										v .. " [" .. id .. "]", x, y, color, cfg.main.align)
								end
							end
						else
							if fColor == 'fffdfcfc' then
								...
								renderFontDrawTextAlign(fonts.playersChecker, "{" .. fColor .. "}" .. v .. " [" ..
									id .. "]", x, y, color, cfg.main.align)
							end
						end
					end
				end
...
function loadbinders()
	if not doesDirectoryExist(getGameDirectory() .. '//moonloader//AdminToolsGromov') then
		createDirectory(getGameDirectory() .. '//moonloader//AdminToolsGromov')
		sampAddChatMessage(tag .. 'Äèðåêòîðèÿ äëÿ íóæíûõ ôàéëîâ ñêðèïòà óñïåøíî ñîçäàíà', -1)
	end
	if not doesDirectoryExist(getGameDirectory() .. '//moonloader//AdminToolsGromov//Ëîãè//') then
		createDirectory(getGameDirectory() .. '//moonloader//AdminToolsGromov//Ëîãè//')
		sampAddChatMessage(tag .. 'Äèðåêòîðèÿ äëÿ ëîãîâ óñïåøíî ñîçäàíà', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder1.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder1.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Ñîáñòâåííûé ¹1" óñïåøíî ñîçäàí', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder2.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder2.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Ñîáñòâåííûé ¹2" óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder3.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder3.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Ñîáñòâåííûé ¹3" óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder4.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder4.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Ñîáñòâåííûé ¹4" óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//AcceptForms.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//AcceptForms.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè ïðèíÿòûõ ôîðì óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//GiveWarnsTK.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//GiveWarnsTK.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè âûäàííûõ âàðíîâ çà ÒÊ óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//zawitabinder.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//zawitabinder.txt", 'w')
		if not f then return False end
		f:write(u8 '/ao Óâàæàåìûå èãðîêè, áåðåãèòå ñâîè àêêàóíòû. Óñòàíàâëèâàéòå ïðèâÿçêó ïî VK (vk-guard)!\n/ao Íå äàâàéòå ïàðîëü îò ñâîåãî àêêàóíòà íèêîìó, íå ñêà÷èâàéòå ìîäèôèêàöèè ñ íåèçâåñòíûõ ñàéòîâ.\n/ao Íàïîìíþ, àäìèíèñòðàöèÿ íå íåñåò îòâåòñòâåííîñòè çà âàøè àêêàóíòû. Âñå â âàøèõ ðóêàõ!')
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Çàùèòà àêêàóíòîâ" óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//donatebinder.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//donatebinder.txt", 'w')
		if not f then return False end
		f:write(u8 '/ao Óâàæàåìûå èãðîêè, íà äàííûé ìîìåíò íà ñåðâåðå äåéñòâóåò àêöèÿ X2 äîíàò!\n/ao Âñÿ ïîïîëíåííàÿ ñóììà óâåëè÷èâàåòñÿ â 2 ðàçà, íå óïóñòèòå âîçìîæíîñòü ïðèîáðåñòè ïðèâèëåãèè ïî íèçêîé öåíå!\n/aoÏîïîëíåíèå ñ÷åòà íà íàøåì ñàéòå - *** ññûëêà ***')
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Ìíîæèòåëü äîíàòà" óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//rulesbinder.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//rulesbinder.txt", 'w')
		if not f then return False end
		f:write(u8 '/ao Óâàæàåìûå èãðîêè, ñîâåòóþ îçíàêîìèòüñÿ ñ ïðàâèëàìè ñåðâåðà íà íàøåì ôîðóìå - *** ññûëêà ***\n/ao Çà íàðóøåíèå ïðàâèë ñåðâåðà Âû áóäåòå ñòðîãî íàêàçàíû, ïðèÿòíîé èãðû!')
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Ïðàâèëà ñåðâåðà" óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//discordbinder.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//discordbinder.txt", 'w')
		if not f then return False end
		f:write(u8 '/ao Íå õâàòàåò îáùåíèÿ? Õî÷åøü ïðîäàòü èìóùåñòâî? Ïîñëå ñëîæíîãî ó÷åáíîãî äíÿ õî÷åòñÿ îòäîõíóòü?\n/ao Òîãäà ïðèñîåäèíÿéñÿ ê Äèñêîðäó íàøåãî ñåðâåðà! Àäåêâàòíûå èãðîêè è ìîäåðàöèÿ.\n/ao Ññûëêà íà íàø Äèñêîðä-êàíàë: discord.gg/QusdeJnGeM')
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Äèñêîðä" óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//groupbinder.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//groupbinder.txt", 'w')
		if not f then return False end
		f:write(u8 '/ao Óâàæàåìûå èãðîêè, õîòèòå óçíàâàòü àêòóàëüíóþ èíôîðìàöèþ ñàìûì ïåðâûì?\n/ao Äëÿ òàêîãî ñëó÷àÿ ó íàñ åñòü ïàáëèê ÂÊ - vk.com/arp_666, óçíàâàéòå íîâîñòè ñàìûì ïåðâûì!')
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ áèíäåðà "Ïàáëèê VK" óñïåøíî ñîçäàí', -1)
	end

	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//AdminVipChatLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//AdminVipChatLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè ñîîáùåíèé îò àäìèíèñòðàòîðîâ â VIP ÷àòå ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnjailsLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnjailsLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè âûïóñêîâ èç äåìîðãàíà ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnmutesLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnmutesLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè ñíÿòèé ìóòîâ ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnbanLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnbanLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè ðàçáëîêèðîâîê ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnbanipLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//UnbanipLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè ðàçáëîêèðîâîê IP ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//BanoffLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//BanoffLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè áëîêèðîâîê â îôôëàéíå ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//JailoffLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//JailoffLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè âûäà÷è äåìîðãàíîâ â îôôëàéíå ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//MuteoffLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//MuteoffLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè âûäà÷è ìóòîâ â îôôëàéíå ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//BanipoffLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//BanipoffLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè âûäà÷è áëîêèðîâîê IP â îôôëàéíå ñîçäàí.', -1)
	end

	if not doesFileExist(getWorkingDirectory() .. "\\AdminToolsGromov\\Ëîãè\\AutoOpraLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Ëîãè//AutoOpraLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Ôàéë äëÿ çàïèñè ïîêóïîê èìóùåñòâà ñîçäàí.', -1)
	end

	mybind1 = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder1.txt", "r")
	mybinder1.v = mybind1:read('*a')
	mybind1:close()

	mybind2 = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder2.txt", "r")
	mybinder2.v = mybind2:read('*a')
	mybind2:close()

	mybind3 = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder3.txt", "r")
	mybinder3.v = mybind3:read('*a')
	mybind3:close()

	mybind4 = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//binder4.txt", "r")
	mybinder4.v = mybind4:read('*a')
	mybind4:close()

	zawitabind = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//zawitabinder.txt", "r")
	zawitabinder.v = zawitabind:read('*a')
	zawitabind:close()

	donatebind = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//donatebinder.txt", "r")
	donatebinder.v = donatebind:read('*a')
	donatebind:close()

	rulesbind = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//rulesbinder.txt", "r")
	rulesbinder.v = rulesbind:read('*a')
	rulesbind:close()

	discordbind = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//discordbinder.txt", "r")
	discordbinder.v = discordbind:read('*a')
	discordbind:close()

	groupbind = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//groupbinder.txt", "r")
		...
		if data.center.x ~= 0 then
			if data.center.y ~= 0 then
				if data.center.z ~= 0 then
					...
					bulletSyncMy[bulletSyncMy.lastId].my.o.x, bulletSyncMy[bulletSyncMy.lastId].my.o.y, bulletSyncMy[bulletSyncMy.lastId].my.o.z =
						data.origin.x, data.origin.y, data.origin.z
					bulletSyncMy[bulletSyncMy.lastId].my.t.x, bulletSyncMy[bulletSyncMy.lastId].my.t.y, bulletSyncMy[bulletSyncMy.lastId].my.t.z =
						data.target.x, data.target.y, data.target.z
					if data.targetType == 0 then
						bulletSyncMy[bulletSyncMy.lastId].my.color = join_argb(255, staticObjectMy.v[1] * 255,
							staticObjectMy.v[2] * 255, staticObjectMy.v[3] * 255)
					elseif data.targetType == 1 then
						bulletSyncMy[bulletSyncMy.lastId].my.color = join_argb(255, pedPMy.v[1] * 255, pedPMy.v[2] * 255,
							pedPMy.v[3] * 255)
					elseif data.targetType == 2 then
						bulletSyncMy[bulletSyncMy.lastId].my.color = join_argb(255, carPMy.v[1] * 255, carPMy.v[2] * 255,
							carPMy.v[3] * 255)
					elseif data.targetType == 3 then
						bulletSyncMy[bulletSyncMy.lastId].my.color = join_argb(255, dinamicObjectMy.v[1] * 255,
							dinamicObjectMy.v[2] * 255, dinamicObjectMy.v[3] * 255)
					end
				end
			end
		end
			...
			if data.center.y ~= 0 then
				if data.center.z ~= 0 then
					...
					if elements.checkbox.showPlayerInfo.v then
						...
						bulletSync[bulletSync.lastId].other.colorText = join_argb(255, colorPlayerI.v[1] * 255,
							colorPlayerI.v[2] * 255, colorPlayerI.v[3] * 255)
					end
					bulletSync[bulletSync.lastId].other.time = os.time() + elements.int.timeRenderBullets.v
					bulletSync[bulletSync.lastId].other.o.x, bulletSync[bulletSync.lastId].other.o.y, bulletSync[bulletSync.lastId].other.o.z =
						data.origin.x, data.origin.y, data.origin.z
					bulletSync[bulletSync.lastId].other.t.x, bulletSync[bulletSync.lastId].other.t.y, bulletSync[bulletSync.lastId].other.t.z =
						data.target.x, data.target.y, data.target.z
					bulletSync[bulletSync.lastId].other.type = data.targetType
					if data.targetType == 0 then
						bulletSync[bulletSync.lastId].other.color = join_argb(255, staticObject.v[1] * 255,
							staticObject.v[2] * 255, staticObject.v[3] * 255)
					elseif data.targetType == 1 then
						bulletSync[bulletSync.lastId].other.color = join_argb(255, pedP.v[1] * 255, pedP.v[2] * 255,
							pedP.v[3] * 255)
					elseif data.targetType == 2 then
						bulletSync[bulletSync.lastId].other.color = join_argb(255, carP.v[1] * 255, carP.v[2] * 255,
							carP.v[3] * 255)
					elseif data.targetType == 3 then
						bulletSync[bulletSync.lastId].other.color = join_argb(255, dinamicObject.v[1] * 255,
							dinamicObject.v[2] * 255, dinamicObject.v[3] * 255)
					end
				end
			end
		end
...
function rlvl(arg)
	if tonumber(arg) and tonumber(arg) >= 3 and tonumber(arg) <= 100 then
		...
		for _, v in pairs(peds) do
			...
			if result and id ~= sampGetPlayerIdByCharHandle(PLAYER_PED) and distance < tonumber(arg) then
				if id ~= myid then
					...
					if lvl < 15 then
						sampAddChatMessage(
							tag .. 'Íèêíåéì: {fe4e4e}' .. nick ..
							' [' .. id .. '] {84A6E7}| Óðîâåíü: {fe4e4e}' .. lvl .. ' [ÏÓÑÒÛØÊÀ]', -1)
					else
						sampAddChatMessage(
							tag .. 'Íèêíåéì: {E78284}' .. nick .. ' [' .. id .. '] {84A6E7}| Óðîâåíü: {E78284}' .. lvl,
							-1)
					end
				end
			end
		...
		sampAddChatMessage(tag .. 'Óêàæèòå ðàäèóñ ïðîâåðêè óðîâíåé [3-100]', -1)
	end
end

function settingswallhackoncar()
	if imgui.BeginPopupModal(u8 'Íàñòðîéêè | WallHack íà òðàíñïîðò', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
		imgui.CenterText(u8 'Òèï íàçâàíèÿ àâòî:')
		if imgui.RadioButton(u8 "Íàçâàíèå òðàíñïîðòà", style_wallhackoncar, 1) then
			cfg.Settings.mode = 1
		end
		if imgui.RadioButton(u8 'Íàçâàíèå òåêñòóðû ôàéëà', style_wallhackoncar, 2) then
			cfg.Settings.mode = 2
		end
		imgui.Text(u8 'Äàëüíîñòü ïðîðèñîâêè:')
		imgui.SameLine()
		imgui.PushItemWidth(100)
		...
		if imgui.InputText('##fontwh', fontwh) then
			...
			fontForRender = renderCreateFont(cfg.Settings.font, cfg.Settings.size, cfg.Settings.style,
				FCR_BOLD + FCR_BORDER)
		end
		imgui.PopItemWidth()
		imgui.Text(u8 'Ðàçìåð øðèôòà:')
...
function connectwithplayer()
	if check_input_available() and isKeysZazhat(fvzaim.v) then
		...
		local mltCorner = math.floor(cfg.main.circleRadius / 100) == 0 and 1 or math.floor(cfg.main.circleRadius / 100)
		for i = 1, 2 do
			renderFigure2D(cx, cy, 25 * mltCorner, cfg.main.circleRadius + i,
				(cfg.main.targetState == "player" and PLAYER_COLOR or ACTIVE_COLOR))
		end
		renderFontDrawTextCenter(fonts1.circle, "Ðåæèì: " ..
			(cfg.main.targetState == "player" and "Èãðîêè" or "Òðàíñïîðò"), cx, windowCoordinates[2] - 40, 0xFF84A6E7)
		renderFontDrawTextCenter(fonts1.circle,
			"Ðàçìåð: " ..
			(cfg.main.circleRadius < maxRad and cfg.main.circleRadius > 30 and cfg.main.circleRadius or (cfg.main.circleRadius <= 30 and "Ìèíèìóì" or (cfg.main.circleRadius >= maxRad and "Ìàêñèìóì"))),
			windowCoordinates[1] / 2, windowCoordinates[2] - 29, 0xFF84A6E7)
		renderFontDrawTextCenter(fonts1.circle, "Èçìåíåíèå ðàäèóñà: êîë¸ñèêî ìûøè", cx, windowCoordinates[2] - 18,
			0xFF84A6E7)
			...
			if doesCharExist(nearPed) then
				...
				if isCharOnScreen(nearPed) then
					...
					if not isNameTagVisible(id) then
						if plPause then
							renderFontDrawTextCenter(fonts1.circle, 'AFK', fPlCoordinates[1],
								fPlCoordinates[2] - 35, 0xFFAA3333)
						end
						renderFontDrawTextCenter(fonts1.circle, plName, fPlCoordinates[1], fPlCoordinates[2] - 20,
							plColor)
					end
					local dist = math.floor(getDistanceBetweenCoords3d(plCoordinates[1], plCoordinates[2],
						plCoordinates[3], myCoordinates[1], myCoordinates[2], myCoordinates[3])) .. ' ìåòðîâ'
					renderFontDrawTextCenter(fonts1.circle, dist, cx, cy + cfg.main.circleRadius + 10, ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '1 - Òåëåïîðòèðîâàòüñÿ', cx + cfg.main.circleRadius + 10, cy - 30,
						ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '2 - Òåëåïîðòèðîâàòü ê ñåáå', cx + cfg.main.circleRadius + 10,
						cy - 15, ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '3 - Óéòè â ðåêîí çà èãðîêîì', cx + cfg.main.circleRadius + 10, cy,
						ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '4 - Âûäàòü 100 HP', cx + cfg.main.circleRadius + 10, cy + 15,
						ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '5 - Çàñïàâíèòü èãðîêà', cx + cfg.main.circleRadius + 10, cy + 30,
						ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '6 - Îòêðûòü ìåíþ âçàèìîäåéñòâèÿ ñ èãðîêîì',
						cx + cfg.main.circleRadius + 10, cy + 45, ACTIVE_COLOR)
					...
					if isKeyJustPressed(VK_2) then
						if cfg.main.mylvladmin >= 2 then
							...
							sampAddChatMessage(tag .. 'Âàø àäìèí-óðîâåíü ñëèøêîì íèçîê äëÿ âûïîëíåíèÿ äàííîãî äåéñòâèÿ.',
								-1)
						end
					end
...
function nicknames(arg)
	if tonumber(arg) then
		...
		if not result then
			sampAddChatMessage(tag .. 'Óêàçàííûé âàìè èãðîê íàõîäèòñÿ íå â ñåòè!', -1)
		else
			send_rpc_command('/pm ' .. arg .. ' 0 Óâàæàåìûé èãðîê, âàø íèêíåéì ñîñòàâëåí íå ïî òðåáóåìîìó ôîðìàòó.')
			send_rpc_command('/pm ' .. arg .. ' 0 Ñäåëàéòå ñâîé íèêíåéì ïî ôîðìå: Nick_Name | Ïðèìåð: Requiem_Devil')
			send_rpc_command('/kick ' .. arg .. ' NickName [Èìÿ_Ôàìèëèÿ]')
		end
	else
		sampAddChatMessage(tag .. 'Óêàæèòå âåðíûé ID èãðîêà!', -1)
	end
end
...
function rvanka(arg)
	if tonumber(arg) then
		...
		if not result then
			...
			send_rpc_command('/banip ' .. arg .. ' ÈÇÏ [Ðâàíêà]')
		end
	else
		sampAddChatMessage(tag .. 'Óêàæèòå âåðíûé ID èãðîêà!', -1)
	end
...
function rekb(arg)
	if tonumber(arg) then
		...
		if not result then
			...
			send_rpc_command('/banip ' .. arg .. ' Ðåêëàìà ïîñòîð. ðåñóðñîâ')
		end
	else
		sampAddChatMessage(tag .. 'Óêàæèòå âåðíûé ID èãðîêà!', -1)
	end
...
function opraplus(arg)
	if arg == nil or arg == " " or arg == "" then
		sampAddChatMessage(tag .. 'Óêàæèòå âåðíûé NickName èãðîêà!', -1)
	else
		local id = sampGetPlayerIdByNickname(arg)
		local result = sampIsPlayerConnected(id)
		if not result then
			send_rpc_command('/unjailoff ' .. arg .. ' îïðîâåðæåíèå ïðèíÿòî')
		else
			send_rpc_command('/unjail ' .. id .. ' îïðîâåðæåíèå ïðèíÿòî')
		end
	end
end
...
function opraminus(arg)
	if arg == nil or arg == " " or arg == "" then
		...
		local result = sampIsPlayerConnected(id)
		if not result then
			send_rpc_command('/unjailoff ' .. arg .. ' îïðîâåðæåíèå îòêëîíåíî')
		else
			send_rpc_command('/unjail ' .. id .. ' îïðîâåðæåíèå îòêëîíåíî')
		end
	end
end
...
function chip(cl)
	...
	if #ips > 0 then
			...
			function(response)
				...
				for i = 1, #rdata do
					if rdata[i]["status"] == "success" then
						...
						if cfg.main.removeip then
							sampAddChatMessage(tag .. 'Ïðîâåðÿþ IP: {E78284}ñêðûòî {84A6E7}| {E78284}¹' .. i, -1)
						else
							sampAddChatMessage(
								tag .. 'Ïðîâåðÿþ IP: {E78284}' .. rdata[i]["query"] .. ' {84A6E7}| {E78284}¹' .. i, -1)
						end
						sampAddChatMessage(
							tag ..
							'[' ..
							...
							' IP] Ñòðàíà: ' ..
							rdata[i]["country"] ..
							' | Ãîðîä: ' .. rdata[i]["city"] .. ' | Ïðîâàéäåð: ' .. rdata[i]["isp"],
							-1)
						sampAddChatMessage(tag .. '[' .. i .. ' IP] Ðàññòîÿíèå îò ðåãèñòðàöèîííîãî: ' .. distances ..
							'êì', -1)
					end
				end
				if text == "" then
					text = " \n\t{FFF500}Íè÷åãî íå íàéäåíî"
				end
			end,
			function(err)
				sampAddChatMessage(tag .. "Ïðîèçîøëà îøèáêà " .. err, -1)
		...
		sampAddChatMessage(tag .. 'Èñïîëüçóéòå /checkip [IP àäðåñ(-û)]', -1)
	end
end
...
function CodeList()
	ev.onPlayerDeathNotification = function(killerId, killedId, reason)
		...
		lua_thread.create(function()
			local line = {
				killer = (not cfg.main.showIDKiller) and sampGetPlayerNickname(killerId) or
					string.format('%s[%d]', sampGetPlayerNickname(killerId), killerId),
				killerColor = sampGetPlayerColor(killerId),
				killed = (not cfg.main.showIDKilled) and sampGetPlayerNickname(killedId) or
					string.format('%s[%d]', sampGetPlayerNickname(killedId), killedId),
				...
				if sampGetPlayerColor(killedId) < 368966908 or sampGetPlayerColor(killedId) > 368966908 then
					...
					if not isGamePaused() then
						sampAddChatMessage(
							...
							' [' ..
							killerId ..
							'] {84A6E7}óáèë ñâîåãî íàïàðíèêà ïî îðãàíèçàöèè {' ..
							color1 .. '}' .. sampGetPlayerNickname(killedId) .. ' [' .. killedId .. ']', -1)
					end
				end
			end
		end
...
function CodeList2()
	if cfg.main.killlist then
		if cfg.main.stylekilllist == 2 then
			for i = 1, #killInfo do
				local y = cfg.killListPosition.y +
					math.pow(-1, 0) * (i - 1) * (cfg.main.indentKillListY + renderGetFontDrawHeight(fonts123.killList));
				if drawClickableText(fonts123.killList, string.format('{a0a0a0}[%s]{ffffff} {%06X}%s', killInfo[i].timeKill, argb_to_rgb_bare(killInfo[i].killerColor), killInfo[i].killer), cfg.killListPosition.x, y, 0xFFFFFFFF, 0xFFFFFFFF) then
					if cfg.main.spectatekill and not editPositionKillList then
						sampSendChat('/re ' .. killInfo[i].killer_id)
					end
				end
				renderDrawTexture(LOAD_IMAGES.KILL_LIST[tonumber(killInfo[i].reason)],
					cfg.killListPosition.x +
					renderGetFontDrawTextLength(fonts123.killList, '[' .. killInfo[i].timeKill .. '] ') +
					renderGetFontDrawTextLength(fonts123.killList, killInfo[i].killer) + cfg.main.indentKillListX,
					y - (cfg.main.iconSizeKillListX - renderGetFontDrawHeight(fonts123.killList)) / 2.0,
					cfg.main.iconSizeKillListX, cfg.main.iconSizeKillListY, 0, 0xFFFFFFFF)
				if drawClickableText(fonts123.killList, string.format('{%06X}%s', argb_to_rgb_bare(killInfo[i].killedColor), killInfo[i].killed), cfg.killListPosition.x + renderGetFontDrawTextLength(fonts123.killList, '[' .. killInfo[i].timeKill .. '] ') + renderGetFontDrawTextLength(fonts123.killList, killInfo[i].killer) + 2 * cfg.main.indentKillListX + cfg.main.iconSizeKillListX, y, 0xFFFFFFFF, 0xFFFFFFFF) then
					if cfg.main.spectatekill then
						sampSendChat('/re ' .. killInfo[i].killed_id)
					end
				end
			end
		elseif cfg.main.stylekilllist == 3 then
			for i = 1, #killInfo do
				local y = cfg.killListPosition.y +
					math.pow(-1, 0) * (i - 1) * (cfg.main.indentKillListY + renderGetFontDrawHeight(fonts123.killList));
				if drawClickableText(fonts123.killList, string.format('{a0a0a0}[%s]{ffffff} {%06X}%s', killInfo[i].timeKill, argb_to_rgb_bare(killInfo[i].killerColor), killInfo[i].killer), cfg.killListPosition.x, y, 0xFFFFFFFF, 0x257E1CFF) then
					if cfg.main.spectatekill and not editPositionKillList then
						sampSendChat('/re ' .. killInfo[i].killer_id)
					end
				end
				renderFontDrawText(fonts123.killList,
					string.format(' {a0a0a0}=> ' .. weapons.get_name(tonumber(killInfo[i].reason)) .. ' => '),
					cfg.killListPosition.x +
					renderGetFontDrawTextLength(fonts123.killList, '[' .. killInfo[i].timeKill .. '] ') +
					renderGetFontDrawTextLength(fonts123.killList, killInfo[i].killer) + 2 * cfg.main.indentKillListX, y,
					0xFFFFFFFF)
				if drawClickableText(fonts123.killList, string.format('{%06X}%s', argb_to_rgb_bare(killInfo[i].killedColor), killInfo[i].killed), cfg.killListPosition.x + renderGetFontDrawTextLength(fonts123.killList, '[' .. killInfo[i].timeKill .. '] ') + renderGetFontDrawTextLength(fonts123.killList, killInfo[i].killer) + 4 * cfg.main.indentKillListX + renderGetFontDrawTextLength(fonts123.killList, ' {a0a0a0}=> ' .. weapons.get_name(tonumber(killInfo[i].reason)) .. ' => '), y, 0xFFFFFFFF, 0xFFFFFFFF) then
					if cfg.main.spectatekill then
						sampSendChat('/re ' .. killInfo[i].killed_id)
					end
				end
...
IMAGES = {
	KILL_LIST = {
		[0] = "moonloader/AdminToolsGromov/KillList/weap_0.png",
		[1] = "moonloader/AdminToolsGromov/KillList/weap_1.png",
		[2] = "moonloader/AdminToolsGromov/KillList/weap_2.png",
		[3] = "moonloader/AdminToolsGromov/KillList/weap_3.png",
		[4] = "moonloader/AdminToolsGromov/KillList/weap_4.png",
		[5] = "moonloader/AdminToolsGromov/KillList/weap_5.png",
		[6] = "moonloader/AdminToolsGromov/KillList/weap_6.png",
		[7] = "moonloader/AdminToolsGromov/KillList/weap_7.png",
		[8] = "moonloader/AdminToolsGromov/KillList/weap_8.png",
		[9] = "moonloader/AdminToolsGromov/KillList/weap_9.png",
		[10] = "moonloader/AdminToolsGromov/KillList/weap_10.png",
		[11] = "moonloader/AdminToolsGromov/KillList/weap_11.png",
		[12] = "moonloader/AdminToolsGromov/KillList/weap_12.png",
		[13] = "moonloader/AdminToolsGromov/KillList/weap_13.png",
		[14] = "moonloader/AdminToolsGromov/KillList/weap_14.png",
		[15] = "moonloader/AdminToolsGromov/KillList/weap_15.png",
		[16] = "moonloader/AdminToolsGromov/KillList/weap_16.png",
		[17] = "moonloader/AdminToolsGromov/KillList/weap_17.png",
		[18] = "moonloader/AdminToolsGromov/KillList/weap_18.png",
		[19] = "moonloader/AdminToolsGromov/KillList/weap_19.png",
		[20] = "moonloader/AdminToolsGromov/KillList/weap_20.png",
		[21] = "moonloader/AdminToolsGromov/KillList/weap_21.png",
		[22] = "moonloader/AdminToolsGromov/KillList/weap_22.png",
		[23] = "moonloader/AdminToolsGromov/KillList/weap_23.png",
		[24] = "moonloader/AdminToolsGromov/KillList/weap_24.png",
		[25] = "moonloader/AdminToolsGromov/KillList/weap_25.png",
		[26] = "moonloader/AdminToolsGromov/KillList/weap_26.png",
		[27] = "moonloader/AdminToolsGromov/KillList/weap_27.png",
		[28] = "moonloader/AdminToolsGromov/KillList/weap_28.png",
		[29] = "moonloader/AdminToolsGromov/KillList/weap_29.png",
		[30] = "moonloader/AdminToolsGromov/KillList/weap_30.png",
		[31] = "moonloader/AdminToolsGromov/KillList/weap_31.png",
		[32] = "moonloader/AdminToolsGromov/KillList/weap_32.png",
		[33] = "moonloader/AdminToolsGromov/KillList/weap_33.png",
		[34] = "moonloader/AdminToolsGromov/KillList/weap_34.png",
		[35] = "moonloader/AdminToolsGromov/KillList/weap_35.png",
		[36] = "moonloader/AdminToolsGromov/KillList/weap_36.png",
		[37] = "moonloader/AdminToolsGromov/KillList/weap_37.png",
		[38] = "moonloader/AdminToolsGromov/KillList/weap_38.png",
		[39] = "moonloader/AdminToolsGromov/KillList/weap_39.png",
		[40] = "moonloader/AdminToolsGromov/KillList/weap_40.png",
		[41] = "moonloader/AdminToolsGromov/KillList/weap_41.png",
		[42] = "moonloader/AdminToolsGromov/KillList/weap_42.png",
		[43] = "moonloader/AdminToolsGromov/KillList/weap_43.png",
		[44] = "moonloader/AdminToolsGromov/KillList/weap_44.png",
		[45] = "moonloader/AdminToolsGromov/KillList/weap_45.png",
		[46] = "moonloader/AdminToolsGromov/KillList/weap_46.png",
		[49] = "moonloader/AdminToolsGromov/KillList/weap_49.png",
		[50] = "moonloader/AdminToolsGromov/KillList/weap_50.png",
		[51] = "moonloader/AdminToolsGromov/KillList/weap_51.png",
		[53] = "moonloader/AdminToolsGromov/KillList/weap_53.png",
		[54] = "moonloader/AdminToolsGromov/KillList/weap_54.png",
		[255] = "moonloader/AdminToolsGromov/KillList/weap_255.png"
...
function SettingsKill()
	imgui.CenterText(u8 'Âûáåðèòå òèï êèëë ëèñòà')
	if imgui.RadioButton(u8 "Ñòàíäàðòíûé", style_killlist, 1) then
		cfg.main.stylekilllist = 1
		kill_selected.v = 1
	end
	imgui.SameLine(250)
	if imgui.RadioButton(u8 'Ýêñêëþçèâíûé ¹1', style_killlist, 2) then
		cfg.main.stylekilllist = 2
		kill_selected.v = 2
	end
	imgui.SameLine(500)
	if imgui.RadioButton(u8 'Ýêñêëþçèâíûé ¹2', style_killlist, 3) then
	...
	elseif cfg.main.stylekilllist == 2 then
		if imgui.ToggleButton(u8 'Îòîáðàæàòü ID óáèéöû', imgui.ImBool(cfg.main.showIDKiller)) then
			cfg.main.showIDKiller = not cfg.main.showIDKiller
		end
		if imgui.ToggleButton(u8 'Îòîáðàæàòü ID óáèòîãî', imgui.ImBool(cfg.main.showIDKilled)) then
		...
		if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '') then
			if not editPositionKillList then
				sampAddChatMessage(tag .. 'Èñïîëüçóéòå {E78284}ËÊÌ {84A6E7}äëÿ ñîõðàíåíèÿ ïîçèöèè ñïèñêà óáèéñòâ.', -1)
				local lastX, lastY = cfg.killListPosition.x, cfg.killListPosition.y
				local open = cfg.main.regChecker
				editPositionKillList = true
				MainWindow.v = false
				lua_thread.create(function()
					while editPositionKillList do
						wait(0)
						sampSetCursorMode(CMODE_LOCKCAM)
						local x, y = getCursorPos()
						cfg.killListPosition.x, cfg.killListPosition.y = x, y
						if isKeyDown(VK_LBUTTON) then
							sampAddChatMessage(tag .. 'Ïîçèöèÿ ñïèñêà óáèéñòâ óñïåøíî ñîõðàíåíà.', -1)
							inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
		...
		imgui.Separator()
		imgui.Text(u8 'Ãîðèçîíòàëüíûé îòñòóï ìåæäó ñòðîêàìè:')
		imgui.SameLine()
		imgui.PushItemWidth(100)
		...
		imgui.PushItemWidth(100)
		imgui.Text(u8 'Âåðòèêàëüíûé îòñòóï ìåæäó ñòðîêàìè:')
		imgui.SameLine()
		...
		imgui.Text(u8 'Ðàçìåð èêîíêè îðóæèÿ ïî øèðèíå:')
		imgui.SameLine()
		imgui.PushItemWidth(100)
		...
		imgui.PushItemWidth(100)
		imgui.Text(u8 'Ðàçìåð èêîíêè îðóæèÿ ïî âûñîòå:')
		imgui.SameLine()
		...
		imgui.Separator()
		imgui.Text(u8 'Øðèôò êèëë ëèñòà:')
		imgui.SameLine()
		imgui.PushItemWidth(100)
...
function imgui.CustomMenu(labels, selected, size, speed, centering)
	...
	for i, v in ipairs(labels) do
		...
		local col_button = imgui.GetColorU32(selected.v == i and imgui.GetStyle().Colors[imgui.Col.ButtonActive] or
			imgui.ImVec4(0, 0, 0, 0))
		local col_box = imgui.GetColorU32(selected.v == i and imgui.GetStyle().Colors[imgui.Col.ButtonHovered] or
			imgui.ImVec4(0, 0, 0, 0))
		local col_hovered = imgui.GetStyle().Colors[imgui.Col.ButtonActive]
		local col_hovered = imgui.GetColorU32(imgui.ImVec4(col_hovered.x, col_hovered.y, col_hovered.z,
			(imgui.IsItemHovered() and 0.2 or 0)))
		draw_list:AddRectFilled(imgui.ImVec2(p.x - size.x / 6, p.y),
			imgui.ImVec2(p.x + (radius * 0.65) + t * size.x, p.y + size.y), col_button, 0.0)
		draw_list:AddRectFilled(imgui.ImVec2(p.x - size.x / 6, p.y),
			imgui.ImVec2(p.x + (radius * 0.65) + size.x, p.y + size.y), col_hovered, 0.0)
		imgui.SetCursorPos(imgui.ImVec2(c.x + (centering and (size.x - imgui.CalcTextSize(v).x) / 2 or 15),
...
function infostate()
	...
	if cfg.main.styleinformation == 1 then
		...
		if _ and char then
			...
			imgui.Columns(4)
			imgui.SetColumnWidth(-1, 87); imgui.Text(u8 'Óðîâåíü'); imgui.NextColumn()
			imgui.SetColumnWidth(-1, 87); imgui.Text(tostring(sampGetPlayerScore(spec_id))); imgui.NextColumn()
			imgui.SetColumnWidth(-1, 87); imgui.Text(u8 'Îïûò'); imgui.NextColumn()
			...
			imgui.Text(u8 'ÀÔÊ')
			imgui.NextColumn()
			if afkplayer then imgui.Text(u8 'Îòîøåë') else imgui.Text('0') end
			imgui.Separator()
			...
			imgui.NextColumn()
			imgui.Text(u8 'Îðóæèå/ÏÒ')
			imgui.NextColumn()
			if _ then
				imgui.Text(tostring('' ..
					tonumber(getCurrentCharWeapon(char)) ..
					':' .. getAmmoInCharWeapon(char, tonumber(getCurrentCharWeapon(char))) .. ''))
			else
				imgui.Text('0')
			end
			imgui.NextColumn()
			imgui.Text(u8 'Shot îáùåå')
			imgui.NextColumn()
			imgui.Text('0/0')
			imgui.Separator()
			...
			imgui.Text(tostring('' .. shotre .. '/' .. shotre))
			imgui.Separator()

			imgui.NextColumn()
			imgui.Text(u8 'Ôðàêöèÿ')
			imgui.NextColumn()
			if org ~= "Íå èìååòñÿ" then imgui.Text(u8:decode(tostring(org))) else imgui.Text(u8 'Ãðàæäàíèí') end
			imgui.NextColumn()
			imgui.Text(u8 'Ðàíã')
			imgui.NextColumn()
			...
			imgui.NextColumn()
			if _ then
				imgui.Text(tostring('' .. math.ceil(getCharSpeed(char)) .. ' / 257.0'))
			else
				imgui.Text('0 / 257.0')
			end
			imgui.NextColumn()
		...
		imgui.SetColumnWidth(-1, 87); imgui.Text(tostring(sampGetPlayerScore(spec_id))); imgui.NextColumn()
		imgui.SetColumnWidth(-1, 87); imgui.Text(u8 'Îïûò'); imgui.NextColumn()
		imgui.SetColumnWidth(-1, 87); imgui.Text(tostring(respect))
		imgui.NextColumn()
		imgui.Separator()
		...
		imgui.Text(u8 'Ôðàêöèÿ')
		imgui.NextColumn()
		if org ~= "Íå èìååòñÿ" then imgui.Text(tostring(org)) else imgui.Text(u8 'Ãðàæäàíèí') end
		imgui.NextColumn()
		imgui.Text(u8 'Ðàíã')
		imgui.NextColumn()
		imgui.Text(rank)
...
function reconusers()
	...
	imgui.Begin(fa.ICON_FA_EYE .. u8 ' Ñëåæêà', reconusers_state,
		imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove +
		imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
	imgui.ShowCursor = true
	imgui.CenterTextColoredRGB('{fe4e4e}Î÷åðåäü ñëåæêè')
	for idjb in report_text:gmatch("(%d+)") do
		if idjb then
			if sampIsPlayerConnected(idjb) then
				nickrecon = sampGetPlayerNickname(idjb)
				if imgui.Button(u8 '' .. nickrecon .. "[" .. idjb .. "]", imgui.ImVec2(200, 25)) then
					sampSendChat(
						'/re ' .. idjb)
				end
			end
...
function imgui.CustomBarLogs()
	...
	local renderButton = function(n, color)
		if n == 1 then
			drawList:AddRectFilled(imgui.ImVec2(pos.x + (135 * n) - 135, pos.y),
				imgui.ImVec2(pos.x + (135 * n), pos.y + 35), color, 5, 9)
		elseif n == count then
			drawList:AddRectFilled(imgui.ImVec2(pos.x + (135 * n) - 135, pos.y),
				imgui.ImVec2(pos.x + (135 * n), pos.y + 35), color, 5, 6)
		else
			drawList:AddRectFilled(imgui.ImVec2(pos.x + (135 * n) - 135, pos.y),
				imgui.ImVec2(pos.x + (135 * n), pos.y + 35), color)
		end
	end
...
function tponroad(arg)
	if tonumber(arg) then
		local result, x, y, z = getNearestRoadCoordinates()
		if result then
			local dist = getDistanceBetweenCoords3d(x, y, z, getCharCoordinates(PLAYER_PED))
			local name = sampGetPlayerNickname(arg)
			lua_thread.create(function()
				setCharCoordinates(PLAYER_PED, x, y, z + 1)
				sampAddChatMessage(
					("[AdminTools] Âû òåëåïîðòèðîâàëè èãðîêà " .. name .. " íà áëèæàéøóþ îò Âàñ äîðîãó (%dm.)"):format(
						dist),
					0xAAFFAA)
				wait(500)
				sampSendChat('/gethere ' .. arg)
				wait(500)
				sampSendChat('/a [AdminTools] Òåëåïîðòèðîâàë èãðîêà ' .. name .. ' íà êîîðäèíàòû ' .. x .. ' ' ..
					y .. ' ' .. z)
			end)
		else
			sampAddChatMessage("[AdminTools] Íå íàøëîñü íè îäíîé äîðîãè ïîáëèçîñòè", 0xFFAAAA)
		end
	else
		sampAddChatMessage('[AdminTools] /tpr [id]', 0xAAFFAA)
	end
end
...
--[[function checkKey()
        ...
        if not response.text:match("<body>(.*)</body>"):find("podpiska otsutstvuet") then
            if not response.text:match("<body>(.*)</body>"):find("podpiska istekla") then
                sampAddChatMessage(tag..'Âû óñïåøíî àâòîðèçîâàëèñü â ñêðèïòå!', -1)
                sampAddChatMessage(tag..'Ïîäïèñêà íà òåêóùèé ñåðâåð àêòèâíà äî'..response.text:match("<body>(.*)</body>"), -1)
				sampAddChatMessage('', -1)
				sampAddChatMessage(tag..'/amenu & '..klavisha..' {E78284}- ìåíþ íàñòðîåê ñêðèïòà.', -1)
				sampAddChatMessage(tag..'/helpa {E78284}- êîìàíäû ñêðèïòà.', -1)
				sampfuncsLog(pinktag..'Ñêðèïò çàãðóæåí, ïîäïèñêà àêòèâíà äî '..response.text:match("<body>(.*)</body>"))
            else
                sampAddChatMessage(pinktag..'Ó âàñ îòñóòñòâóåò ïîäïèñêà íà AdminTools, ñêðèïò âûêëþ÷àåòñÿ.', -1)
				sampAddChatMessage(pinktag..'Âàø óíèêàëüíûé êëþ÷: '..getserial(), -1)
				sampfuncsLog(pinktag..'Âàø óíèêàëüíûé êëþ÷: '..getserial())
				sampAddChatMessage(pinktag..'Äëÿ ïîëó÷åíèÿ/ïîêóïêè äîñòóïà ê èñïîëüçîâàíèþ ñêðèïòà îáðàòèòåñü ê  vk.com/gromovboss.', -1)
				sampfuncsLog(pinktag..'Ñêðèïò îòêëþ÷åí, ïîäïèñêà íåàêòèâíà.')
				sampShowDialog(6666, '{E78284}Ïëàòíàÿ ïîäïèñêà AdminTools', '{84A6E7}×òîáû êóïèòü ïîäïèñêó íà ñêðèïò îáðàòèòåñü ê  {E78284}vk.com/gromovboss\n\n{84A6E7}Ïî÷åìó íå ñòîèò èñïîëüçîâàòü îáõîäû?\n âû íå áóäåòå ïîëó÷àòü ðåãóëÿðíûå îáíîâëåíèÿ;\n ñêðèïò áóäåò ðàáîòàòü íåñòàáèëüíî, áóäóò âîçíèêàòü îøèáêè;\n êàæäûé ðàç íóæíî ÷òî-òî çàìåíÿòü ðóêàìè/ïåðåçàãðóæàòü;\n âàì ñïîêîéíî ìîãóò ïîäãðóçèòü âèðóñ, ïîñëå ýòîãî  âçëîìàòü;\n ëó÷øå íå èñïîëüçîâàòü ñêðèïò âîâñå, ÷åì ïîòîì ñòðàäàòü îò ïîòåðÿííîãî âðåìåíè;\n\n{E78284}Ïîäïèñêà íà ñêðèïò îòêðîåò âàì ôóíêöèîíàë êî âñåìó ñêðèïòó!', 'Çàêðûòü', _, 0)
				thisScript():unload()
            end
        else
            sampAddChatMessage(pinktag..'Ó âàñ îòñóòñòâóåò ïîäïèñêà íà AdminTools, ñêðèïò âûêëþ÷àåòñÿ.', -1)
...
function update()
	local raw = 'https://moonlightperson.github.io/tools/update.json'
	local dlstatus = require('moonloader').download_status
	local requests = require('requests')
	local f = {}
		...
		if response.status_code == 200 then
			downloadUrlToFile(decodeJson(response.text)['url'], thisScript().path, function(id, status, p1, p2)
				if status == dlstatus.STATUSEX_ENDDOWNLOAD then
					sampAddChatMessage(tag .. 'Ñêðèïò îáíîâëåí, ïåðåçàãðóçêà...', -1)
					thisScript():reload()
				end
			end)
		else
			sampAddChatMessage(tag .. 'Îøèáêà, íåâîçìîæíî óñòàíîâèòü îáíîâëåíèå, êîä: ' .. response.status_code, -1)
		end
	end
...
function ArenaHelper()
	if imgui.Button(u8 'Çàïóñê ðàçäà÷è cchas', imgui.ImVec2(330, 25)) then
		if not start_cchas then
			imgui.OpenPopup(u8 'Âûáåðèòå êîíôèãóðàöèþ##cchas')
		else
			sampAddChatMessage(tag .. 'Ðàçäà÷à cchas óæå çàïóùåíà.')
		end
	end
	if imgui.BeginPopupModal(u8 'Âûáåðèòå êîíôèãóðàöèþ##cchas', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		imgui.PushItemWidth(100)
		imgui.InputInt(u8 'Âðåìÿ ðàáîòû â ñåêóíäàõ', ctime)
		imgui.PopItemWidth()
		imgui.PushItemWidth(100)
		imgui.InputInt(u8 'Êîëè÷åñòâî ðàçäà÷è', actime)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Çàïóñòèòü cchas', imgui.ImVec2(-1, 30)) then
			cchas(actime.v)
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Îòìåíèòü', imgui.ImVec2(-1, 30)) then
			imgui.CloseCurrentPopup()
		end
		imgui.EndPopup()
	end
	imgui.SameLine()
	if imgui.Button(u8 'Çàïóñê TikTok ñóíäóêà', imgui.ImVec2(330, 25)) then
		if not sd then
			imgui.OpenPopup(u8 'Âûáåðèòå êîíôèãóðàöèþ##sunduk')
		else
			sampAddChatMessage(tag .. 'TikTok ñóíäóê óæå çàïóùåí.', -1)
		end
	end
	if imgui.BeginPopupModal(u8 'Âûáåðèòå êîíôèãóðàöèþ##sunduk', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if arena_type.v == 0 and (arena_amount.v == "" or arena_amount.v == nil or arena_amount.v == "1000" or arena_amount.v == "2000" or arena_amount.v == "5000000") then
			arena_amount.v =
			"1000000"
		end
		if arena_type.v == 1 and (arena_amount.v == "" or arena_amount.v == nil or arena_amount.v == "1000000" or arena_amount.v == "2000" or arena_amount.v == "5000000") then
			arena_amount.v =
			"300"
		end
		if arena_type.v == 2 and (arena_amount.v == "" or arena_amount.v == nil or arena_amount.v == "1000000" or arena_amount.v == "1000" or arena_amount.v == "5000000") then
			arena_amount.v =
			"2000"
		end
		if arena_type.v == 3 and (arena_amount.v == "" or arena_amount.v == nil or arena_amount.v == "1000000" or arena_amount.v == "1000" or arena_amount.v == "2000") then
			arena_amount.v =
			"5000000"
		end
		if (arena_type.v == 0 or arena_type.v == 1 or arena_type.v == 2 or arena_type.v == 3) and (arena_people.v == "" or arena_people.v == nil) then
			arena_people.v =
			"10"
		end
		if (arena_type.v == 0 or arena_type.v == 1 or arena_type.v == 2 or arena_type.v == 3) and (arena_time.v == "" or arena_time.v == nil) then
			arena_time.v =
			"2"
		end
		imgui.PushItemWidth(100)
		imgui.Combo(u8 "Òèï ïðèçà", arena_type, view, #view)
		imgui.InputText(u8 "Êîëè÷åñòâî ïðèçà", arena_amount)
		imgui.InputText(u8 'Êîëè÷åñòâî ëþäåé', arena_people)
		imgui.InputText(u8 'Âðåìÿ äåéñòâèÿ ñóíäóêà', arena_time)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Çàïóñòèòü TikTok ñóíäóê', imgui.ImVec2(-1, 30)) then
			sampSendChat('/sunduk')
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Îòìåíèòü', imgui.ImVec2(-1, 30)) then
	...
	if imgui.Button(u8 "Ñîçäàòü ðàíäîìíûé êîä", imgui.ImVec2(330, 25)) then
		generationCode()
	end
	imgui.SameLine()
	if imgui.Button(u8 "Ðàíäîìíûé êîä äëÿ àäì", imgui.ImVec2(330, 25)) then
		generationMpAdm()
	end
	if imgui.Button(u8 "Êîä players", imgui.ImVec2(330, 25)) then
		addcodeplayer()
	end
	imgui.SameLine()
	if imgui.Button(u8 "Êîä admins", imgui.ImVec2(330, 25)) then
		addcodeadmins()
	end
	imgui.Separator()
	if imgui.Button(u8 "Âûäàòü áèòêîèíû [RANDOM]", imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Âûáåðèòå êîíôèãóðàöèþ##givebtc')
	end
	if imgui.BeginPopupModal(u8 'Âûáåðèòå êîíôèãóðàöèþ##givebtc', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_numbtc.v == "" or arena_numbtc.v == nil) then arena_numbtc.v = "10" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Êîëè÷åñòâî", arena_numbtc)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Âûäàòü BTC', imgui.ImVec2(-1, 30)) then
			GiveBtcRandom(u8:decode(arena_numbtc.v))
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Îòìåíèòü', imgui.ImVec2(-1, 30)) then
			imgui.CloseCurrentPopup()
	...
	imgui.SameLine()
	if imgui.Button(u8 "Âûäàòü ñòîê êàòàíó [RANDOM]", imgui.ImVec2(330, 25)) then
		DistributionArena(2)
	end
	if imgui.Button(u8 "Âûäàòü êðàñèòåëü [RANDOM]", imgui.ImVec2(330, 25)) then
		DistributionArena(4)
	end
	imgui.SameLine()
	if imgui.Button(u8 "Âûäàòü íàøèâêó [RANDOM]", imgui.ImVec2(330, 25)) then
		DistributionArena(5)
	end
	if imgui.Button(u8 "Âûäàòü AZ Coins [RANDOM]", imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Âûáåðèòå êîíôèãóðàöèþ##giveaz')
	end
	if imgui.BeginPopupModal(u8 'Âûáåðèòå êîíôèãóðàöèþ##giveaz', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_numcoins.v == "" or arena_numcoins.v == nil) then arena_numcoins.v = "10" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Êîëè÷åñòâî", arena_numcoins)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Âûäàòü AZ-Êîèíû', imgui.ImVec2(-1, 30)) then
			GiveAzCoinsRandom(u8:decode(arena_numcoins.v))
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Îòìåíèòü', imgui.ImVec2(-1, 30)) then
			imgui.CloseCurrentPopup()
	...
	imgui.SameLine()
	if imgui.Button(u8 "Âûäàòü AZ-Rub [RANDOM]", imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Âûáåðèòå êîíôèãóðàöèþ##giverub')
	end
	if imgui.BeginPopupModal(u8 'Âûáåðèòå êîíôèãóðàöèþ##giverub', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_numrub.v == "" or arena_numrub.v == nil) then arena_numrub.v = "10" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Êîëè÷åñòâî", arena_numrub)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Âûäàòü ðóáëè', imgui.ImVec2(-1, 30)) then
			GiveRoublesRandom(u8:decode(arena_numrub.v))
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Îòìåíèòü', imgui.ImVec2(-1, 30)) then
			imgui.CloseCurrentPopup()
	...
	if imgui.Button(u8 "Âûäàòü ÷àñû [RANDOM]", imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Âûáåðèòå êîíôèãóðàöèþ##givechas')
	end
	if imgui.BeginPopupModal(u8 'Âûáåðèòå êîíôèãóðàöèþ##givechas', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_numchas.v == "" or arena_numchas.v == nil) then arena_numchas.v = "10" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Êîëè÷åñòâî", arena_numchas)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Âûäàòü ÷àñû', imgui.ImVec2(-1, 30)) then
			GiveChasRandom(u8:decode(arena_numchas.v))
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Îòìåíèòü', imgui.ImVec2(-1, 30)) then
			imgui.CloseCurrentPopup()
	...
	imgui.SameLine()
	if imgui.Button(u8 'Âûäàòü ïðåäìåò [RANDOM]', imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Âûáåðèòå êîíôèãóðàöèþ##randacs')
	end
	if imgui.BeginPopupModal(u8 'Âûáåðèòå êîíôèãóðàöèþ##randacs', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_nameacs.v == "" or arena_nameacs.v == nil) then arena_nameacs.v = "Êàòàíà" end
		if (arena_acs.v == "" or arena_acs.v == nil) then arena_acs.v = "789" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Íàçâàíèå", arena_nameacs)
		imgui.InputText(u8 "ID", arena_acs)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Âûäàòü ïðåäìåò', imgui.ImVec2(-1, 30)) then
			giverndacs(act, u8:decode(arena_acs.v), u8:decode(arena_nameacs.v))
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Îòìåíèòü', imgui.ImVec2(-1, 30)) then
			imgui.CloseCurrentPopup()
	...
	imgui.Separator()
	if imgui.ToggleButton(u8 'Àâòî-ñóíäóê', imgui.ImBool(cfg.main.autosunduk)) then
		cfg.main.autosunduk = not cfg.main.autosunduk
	end
	imgui.SameLine()
	imgui.PushFont(minimalfont)
	imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
	imgui.PopFont()
	imgui.Hint('helpas', u8 'Ñêðèïò àâòîìàòè÷åñêè áóäåò ñòàâèòü òèê-òîê ñóíäóê (äëÿ /astats)')
end

function giverndacs(id, acs, acsname)
	id = random(0, 250)
	result = sampIsPlayerConnected(id)

	if not result then
		giverndacs(id, acs, acsname)
		return
	else
		lua_thread.create(function()
			imgui.CloseCurrentPopup()

			sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò ïðåäìåò '%s'", acsname))
			wait(5000)
			sampSendChat(string.format("/giveitem %d %d 1", id, acs))
			wait(700)

			if result then
				sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë ïðåäìåò '%s'", sampGetPlayerNickname(id), id,
					acsname))
			end
		end)
	end
...
function cchas(amount)
	lua_thread.create(function()
		for i = 1, amount do
			sampSendChat("/givechass")
			start_cchas = true
			wait(500)
			sampSendChat("/ao /cchas x" .. i .. " [" .. u8:decode(ctime.v) .. " ñåêóíä]")
			wait((ctime.v) * 1000)
			sampSendChat("/givechass")
			start_cchas = false
		end
...
function generationCode()
	cd = true
	lua_thread.create(function()
		local code, act, cd = getRandomWord(7), random(4, 8), true
		prize = random(10, 150)
		sampSendChat(string.format("/addcode %s %d", code, act))
		wait(2000)
		sampSendChat(string.format("/ao Ñîçäàë êîä %s [Àêòèâàöèé: %d] ñ ïðèçîì %d AZ-RUB.", code, act, prize))
		wait(20000)
		sampSendChat(string.format("/dellcode %s", code))
	end)
end

function generationMpAdm()
	cd = true
	lua_thread.create(function()
		local code, act, cd = getRandomWord(7), random(4, 8), true
		prize = random(10, 150)
		sampSendChat(string.format("/addcode %s %d", code, act))
		wait(2000)
		sampSendChat(string.format("/a Ñîçäàë êîä %s [Àêòèâàöèé: %d] ñ ïðèçîì %d AZ-RUB.", code, act, prize))
		wait(20000)
		sampSendChat(string.format("/dellcode %s", code))
	end)
end

function addcodeplayer()
	cd = true
	lua_thread.create(function()
		local act, cd = random(4, 8), true
		prize = random(10, 150)
		sampSendChat(string.format("/addcode players %d", act))
		wait(2000)
		sampSendChat("/ao Ñîçäàë êîä players [Àêòèâàöèé: " .. act .. "] ñ ïðèçîì " .. prize .. " AZ-RUB")
		wait(20000)
		sampSendChat("/dellcode players")
	end)
end

function addcodeadmins()
	cd = true
	lua_thread.create(function()
		local act, cd = random(4, 8), true
		prize = random(10, 150)
		sampSendChat(string.format("/addcode admins %d", act))
		wait(2000)
		sampSendChat(string.format("/a Ñîçäàë êîä admins [Àêòèâàöèé: %d] ñ ïðèçîì %d AZ-RUB", act, prize))
		wait(20000)
		sampSendChat("/dellcode admins")
	end)
end
...
function getRandomPlayer()
	local randomID = random(0, 250)
	local result = sampIsPlayerConnected(randomID)
...
function GiveRoublesRandom(num)
	local playerID = getRandomPlayer()

	lua_thread.create(function()
		sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò %d AZ-Rub", num))
		wait(5000)
		sampSendChat(string.format("/giverub %d %d", playerID, num))
		wait(700)
		if result then
			sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë %d AZ-Rub", sampGetPlayerNickname(playerID), playerID,
				num))
		end
	end)
end

function GiveChasRandom(num)
	local playerID = getRandomPlayer()

	lua_thread.create(function()
		sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò %d ÷àñ(à, îâ)", num))
		wait(5000)
		sampSendChat(string.format("/givechas %d %d", playerID, num))
		wait(700)
		if result then
			sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë %d ÷àñ(à, îâ)", sampGetPlayerNickname(playerID),
				playerID, num))
		end
	end)
end

function GiveAzCoinsRandom(num)
	local playerID = getRandomPlayer()

	lua_thread.create(function()
		sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò %d AZ-Coins", num))
		wait(5000)
		sampSendChat(string.format("/giveaz %d %d", playerID, num))
		wait(700)
		if result then
			sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë %d AZ-Coins", sampGetPlayerNickname(playerID), playerID,
				num))
		end
	end)
end

function GiveBtcRandom(num)
	local playerID = getRandomPlayer()

	lua_thread.create(function()
		sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò %d BTC", num))
		wait(5000)
		sampSendChat(string.format("/giveitem %d 1383 %d", playerID, num))
		wait(700)
		if result then
			sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë %d BTC", sampGetPlayerNickname(playerID), playerID, num))
		end
	end)
end

function DistributionArena(type)
	local playerID = getRandomPlayer()

	if type == 1 then
		lua_thread.create(function()
			local BTC, cd = getRandomWord(100), true
			prize = random(200, 500)
			sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò %s BTC.", prize))
			wait(5000)
			sampSendChat(string.format("/giveitem %d 1383 %s", playerID, prize))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë %d BTC.", sampGetPlayerNickname(playerID), playerID,
					prize))
			end
		end)
	elseif type == 2 then
		lua_thread.create(function()
			local katana, cd = getRandomWord(100), true
			sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò ïðåäìåò 'Êàòàíà'", prize))
			wait(5000)
			sampSendChat(string.format("/giveitem %d 789 1", playerID))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë ïðåäìåò 'Êàòàíà'", sampGetPlayerNickname(playerID),
					playerID))
			end
		end)
	elseif type == 3 then
		lua_thread.create(function()
			local az, cd = getRandomWord(100), true
			prize = random(10000, 30000)
			sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò %s Az-Coins", prize))
			wait(5000)
			sampSendChat(string.format("/giveaz %d %s", playerID, prize))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë %s Az-Coins", sampGetPlayerNickname(playerID),
					playerID, prize))
			end
		end)
	elseif type == 4 then
		lua_thread.create(function()
			local az, cd = getRandomWord(100), true
			sampSendChat("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò êðàñèòåëü")
			wait(5000)
			sampSendChat(string.format("/giveitem %d 2081 1", playerID))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë êðàñèòåëü", sampGetPlayerNickname(playerID),
					playerID))
			end
		end)
	elseif type == 5 then
		lua_thread.create(function()
			local az, cd = getRandomWord(100), true
			sampSendChat("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò Ñóïåð Ìåãà Íàøèâêó")
			wait(5000)
			sampSendChat(string.format("/giveitem %d 2083 1", playerID))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë Ñóïåð Ìåãà Íàøèâêó", sampGetPlayerNickname(playerID),
					playerID))
			end
		end)
	elseif type == 6 then
		lua_thread.create(function()
			local az, cd = getRandomWord(100), true
			prize = random(15, 30)
			sampSendChat(string.format("/ao ×åðåç 5 ñåêóíä ðàíäîìíûé èä ïîëó÷èò %d AZ-Rub", prize))
			wait(5000)
			sampSendChat(string.format("/giverub %d %s", playerID, prize))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Èãðîê %s[%d] ïîëó÷èë %d AZ-Rub", sampGetPlayerNickname(playerID),
					playerID, prize))
			end
		end)
	end
