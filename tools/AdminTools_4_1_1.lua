...
local view                 = { u8 "Евро", u8 "Биткоины", u8 "АЗ Рубли", u8 "AZ Coins" }
local arena_code           = {}
...
local selector_list = {
	...
	fa.ICON_FA_REGISTERED .. u8 ' Функции для Арены',
...
locations = {
	{ 'Автосалон ЛС', -484.0909, -557.7690, 25.5634 },
	{ 'Ферма новичков', -86.9788, 88.2501, 3.1172 },
	{ 'Автосалон ЛВ', 947.2376, 2185.3940, 10.8203 },
	{ 'Спавн LS №2', 2219.6565, -1146.2649, 25.7819 },
	{ 'Центральный рынок', 1125.7731, -1425.4581, 15.7969 },
	{ 'ВайнВуд', 1383.2190, -894.9438, 37.1505 },
	{ 'Информационный центр', -2238.2156, 575.6461, 35.1719 },
	{ 'Аммо LS', 1363.6930, -1279.8029, 13.5469 },
	{ 'Спавн LS №1', 1759.4504, -1900.2614, 13.6300 },
	{ 'Завод для МП', 2508.1938, 2767.6387, 10.8203 },
	{ 'Шахта', 522.3231, 885.0934, -37.4623 },
	{ 'Автосалон СФ', -2663.9944, -1.8084, 4.3267 },
	{ 'Казино ЛВ', 2030.6858, 998.3827, 10.8203 },
	{ 'Автозавод', 1576.6055, 713.8749, 10.8203 },
	{ 'Автосалон СФ поддельных авто (нелегалки)', -2466.1277, 2251.1570, 4.7997 },
	{ 'Спавн LV №1', 2844.6235, 1291.8997, 11.3906 },
	{ 'Спавн LV №2', -71.9333, 1224.7753, 19.6904 },
	{ 'Спавн SF', -1986.2416, 147.1740, 27.6875 },
	{ 'Автобазар', -2136.6956, -750.2112, 32.0234 },
	{ 'Лес', -1649.5634, -2237.9788, 30.1431 },
	{ 'Банк LV', 2375.4661, 2316.0024, 8.1406 },
	{ 'Центральный Банк', 1469.8745, -1743.1466, 13.5469 },
	{ 'Заброшенный аэропорт', 376.8152, 2511.9451, 16.5445 },
	{ 'Центр занятости', 1287.5400, -1275.4004, 13.5407 }
}
...
changelog = {
	...
	[3] = {
		...
		['info'] = {
			...
			'Теперь конфиг скрипта хранится в папке скрипта, а не в папке config для всего мунлоадера (путь: moonloader/config/admintoolsgromov/AdminTools.ini) '
		},
		['patches'] = {
			['show'] = false,
			['info'] = {
...
pathOffLog = getWorkingDirectory() .. "\\AdminToolsGromov\\Логи\\AutoOpraLogs.txt"
log_full = {}
...
function main()
	...
	if thisScript().version ~= lastver then
        ...
        sampAddChatMessage(pinktag..'Вышло обновление для скрипта! Ваша версия: '..thisScript().version..', актуальная: '..lastver, -1)
        sampAddChatMessage(pinktag..'Для установки обновления используйте команду /load.update или выберите необходимый пункт в всплывающем окне.', -1)
		UpdateWindow.v = true
    end
	...
	if not checkServer(sampGetCurrentServerAddress()) and mynick ~= "moonlight" then
		sampAddChatMessage(tag .. 'Скрипт работает только на сервере Arizona Arena!', -1)
		thisScript():unload()
	end
	...
	if not doesFileExist("moonloader/AdminToolsGromov/authors.png") then
		sampAddChatMessage(tag .. 'Ресурс для работы скрипта не была найден, началось автоматическое скачивание..', -1)
		download_id = downloadUrlToFile(
			...
			'moonloader/AdminToolsGromov/authors.png', function(id, status, p1, p2)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					sampAddChatMessage(tag .. 'Скачивание ресурса успешно завершено.', -1)
		...
		{ 'carnames', function(param)
			if param:find('^%d') and tonumber(param) <= 2 then
				if tonumber(param) ~= tonumber(cfg.Settings.mode) then
					...
					sampAddChatMessage(
						string.format('В данный момент итак включено отображение %s [{1E90FF}%d{FFFFFF}].',
							messageChangeMode[param + 1], param), -1)
				end
			else
				sampAddChatMessage(
					string.format('Такого параметра не существует! Количество доступных параметров: {1E90FF}%d{FFFFFF}.',
						#messageChangeMode), -1)
				for k, v in pairs(messageChangeMode) do
	...
	while true do
		...
		if cfg.main.leaveChecker then
			...
			renderFontDrawTextAlign(leave, "Чекер выходов: ", cfg.leavePosition.x, cfg.leavePosition.y, -1,
				cfg.main.aligntwo)
			for k, v in pairs(leavePlayers) do
				...
				renderFontDrawTextAlign(leave,
					...
					"[" .. leavePlayers[k].id .. "] отключился. Причина: " .. leavePlayers[k].reason, x, y, -1,
					cfg.main.aligntwo)
		...
		if cfg.main.connectChecker then
			...
			renderFontDrawTextAlign(connect, "Чекер входов: ", cfg.connectPosition.x, cfg.connectPosition.y, -1,
				cfg.main.alignthree)
			for k, v in pairs(connectPlayers) do
				...
				renderFontDrawTextAlign(connect,
					...
					"] " .. connectPlayers[k].nickname .. "[" .. connectPlayers[k].id .. "] подключился.", x, y, -1,
					cfg.main.alignthree)
		...
		if cfg.main.regChecker then
			...
			renderFontDrawTextAlign(reg, "Чекер регистраций: ", cfg.RegPosition.x, cfg.RegPosition.y, -1,
				cfg.main.alignfour)
			for k, v in pairs(regPlayers) do
				...
				renderFontDrawTextAlign(reg,
					...
					"[" ..
					regPlayers[k].id ..
					"] зарегистрировался | IP: " .. regPlayers[k].ip1 .. ".xx.xxx." .. regPlayers[k].ip4 .. "", x, y, -1,
					cfg.main.alignfour)
		...
		if isPlayerUsingJetpack(PLAYER_HANDLE) then
			...
			renderFontDrawText(my_font,
				'Изменение скорости: {E78284}' ..
				klavisha .. ' {84A6E7}и {E78284}' .. klavisha1 .. '\n{84A6E7}Текущая скорость: {E78284}' .. boost,
		...
		if sampGetGamestate() ~= 3 and cfg.main.lovlyareporta then
			sampAddChatMessage(
				tag .. 'Автоловля репорта выключена, так как вы находитесь в статусе "Подключение к серверу"', -1)
			cfg.main.lovlyareporta = false
		end

		if sampGetGamestate() ~= 3 and cfg.main.autopiar then
			sampAddChatMessage(tag .. 'Автореклама выключена, так как вы находитесь в статусе "Подключение к серверу"',
				-1)
			...
			if isKeysDown(flovlyareporta.v) then
				if sampGetGamestate() ~= 3 then
					...
					sampAddChatMessage(tag ..
						'Автоловля репорта ' .. (cfg.main.lovlyareporta and 'включена' or 'выключена'), -1)
				end
			end
		end
		...
		if enAirBrake then
			...
			renderFontDrawText(my_font,
				'Изменение скорости: {E78284}' ..
				klavisha ..
				' {84A6E7} и {E78284}' .. klavisha1 .. '\n{84A6E7}Текущая скорость: {E78284}' .. cfg.main.speed_airbrake,
				cx + 500, windowCoordinates[2] - 32, 0xFF84A6E7)
			...
			if active_report == 2 then
				lua_thread.create(function()
					...
					sampAddChatMessage(
						...
						cfg.main.twocolorforms .. "}<< выдана!", "0xFF" .. cfg.main.colorforms)
					local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//AcceptForms.txt", 'a')
					f:write('[' ..
						os.date("%d.%m.%Y %X", os.time()) ..
						'] Форма: /' ..
						cmd .. ' ' .. paramssss .. ' | Отправитель: ' .. admin_nick .. ' | Тип принятия формы: вручную\n')
					f:close()
...
function imgui.OnDrawFrame()
	...
	local colours = { u8 'Розовый', u8 'Зелёный', u8 'Красный', u8 'Фиолетовый', u8 'Чёрный' }
	local binds = { u8 'Защита аккаунта', u8 'Множитель доната', u8 'Правила сервера', u8 'Дискорд', u8 'Паблик', u8 'Собственный №1',
		u8 'Собственный №2', u8 'Собственный №3', u8 'Собственный №4' }
	if MainWindow.v then
		imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(900, 550), 1)
		imgui.Begin(fa.ICON_FA_TOOLS .. u8 ' Настройки скрипта ' .. fa.ICON_FA_TOOLS, MainWindow,
		...
		if menusel.v == 1 then
			...
			imgui.PopFont()
			imgui.Hint('helpa13',
				u8 'Аналог метлы, только в разы быстрее\nВкл./выкл.: R.SHIFT\nУскорение: Num +\nЗамедление:Num -')
			...
			imgui.PopFont()
			imgui.Hint('helpa3',
				u8 'Введите ID фракции в которую вы будете вступать при авторизации\nФункция доступна с 4+ LVL')
			...
			imgui.PopFont()
			imgui.Hint('helpa2',
				u8 'Введите ID скина который будет выдаваться при авторизации\nФункция доступна с 4+ LVL')
			...
			imgui.PopFont()
			imgui.Hint('helpa1',
				u8 'Для работы данной функции укажите свой пароль от аккаунта в соответствующем пункте.\nПри подключении на сервер скрипт сам моментально введет пароль')
			...
			if imgui.ToggleButton(u8 'Ноп F5', imgui.ImBool(cfg.main.nopf5)) then
				...
				if not cfg.main.nopf5 then
					sampAddChatMessage('[Информация] После перезахода в игру клавиша F5 снова будет работаспособа',
						14628149)
				end
			end
			imgui.SameLine()
			...
			if imgui.ToggleButton(u8 'Чекер отключений', imgui.ImBool(cfg.main.leaveChecker)) then
			...
			imgui.PopFont()
			imgui.Hint('antitps ',
				u8 'При входе под админку у вас автоматически будет\nвключаться /antitp\n{fe4e4e}Доступно с 4 LVL FD!')
			imgui.NextColumn()
			...
			imgui.Hint('heloa388433',
				u8 'При нажатии кнопки > или < вы перейдете в слежку за следующим или прошлым игроком')
			...
			if imgui.ToggleButton(u8 'Ноп F4', imgui.ImBool(cfg.main.nopf4)) then
				...
				if not cfg.main.nopf4 then
					sampAddChatMessage('[Информация] После перезахода в игру клавиша F4 снова будет работаспособа',
						14628149)
				end
			end
			imgui.SameLine()
			...
			imgui.PopFont()
			imgui.Hint('helpa21',
				u8 'Отключает работу клавиши F4, при частом попадании в больку отключите функцию и перезайдите')
			...
			imgui.PopFont()
			imgui.Hint('helpa26',
				u8 'Данная функция логгирует все принятые формы в отдельный файл, дабы сократить случаи подставы')
			...
			imgui.PopFont()
			imgui.Hint('helpa27',
				u8 'При входе в админку скрипт будет автоматически устанавливать уровень администратора на серверный')
			...
			imgui.Hint('helpa2311384122381',
				u8 'При стрельбе у вас автоматически будет скроллиться оружие, тем самым отсутствовать перезарядка')
			...
			imgui.Hint('helpa1122311384122381',
				u8 'При прописывании /getip скрипт автоматически будет определять местоположение и выводить в /a либо локальный чат')
			...
			imgui.Hint('warningtkss',
				u8 'Если игрок убьет своего напарника по организации, то скрипт оповестит вас об этом в чате')
			...
			imgui.Hint('spectatekillaa',
				u8 'При нажатии по никнейму игрока в килл-листе\nвы автоматически перейдете в слежку за ним')
			imgui.Columns(1)
			imgui.Separator()
		...
		if menusel.v == 5 then
			...
			imgui.Separator()
			imgui.CenterTextColoredRGB('{fe4e4e}Стиль панели управления в слежке')
			...
			if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '##panelvzaim') then
				if ReconWindow.v then
					...
					sampAddChatMessage(tag .. 'Зайдите в рекон для получения возможности изменения данного параметра.',
						-1)
				end
			end
			...
			if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '##ocheredslezhka') then
				if reconusers_state.v then
					...
					sampAddChatMessage(
						tag .. 'Для изменения данного параметра у вас должна быть открыта панель очереди слежки.', -1)
				end
			end
			...
			if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '##saddsaejjadd') then
				if PunishWindow.v then
					...
					MainWindow.v = false
					sampAddChatMessage(tag .. 'Для подтверждения изменения нажмите 1, чтобы отменить сохранение - 2.', -1)
				else
					sampAddChatMessage(tag .. 'Зайдите в рекон для получения возможности изменения данного параметра.',
						-1)
				end
			...
			if imgui.Button(fa.ICON_FA_ARROWS_ALT .. u8 '##infaslezhka') then
				if InfoWindow.v then
					...
					sampAddChatMessage(
						tag .. 'Для изменения данного параметра у вас должна быть включена информационная панель.', -1)
				end
			end
			imgui.Separator()
			imgui.EndChild()
			...
			if imgui.BeginPopupModal(u8("Добавление кнопки"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				...
				if retpor_text_edit then
					if imgui.Button(u8("Изменить"), imgui.ImVec2(400, 20)) then
						if inputreportaddname.v == "" or inputreportaddname.v == nil or inputreportaddotvet.v == nil or inputreportaddotvet.v == "" then
							...
							sampAddChatMessage(
								"Вы успешно отредактировали кнопку \"" .. u8:decode(inputreportaddname.v) .. "\"",
								15352900)
				...
				elseif imgui.Button(u8("Добавить"), imgui.ImVec2(400, 20)) then
					if inputreportaddname.v == "" or inputreportaddname.v == nil or inputreportaddotvet.v == nil or inputreportaddotvet.v == "" then
						...
						inicfg.save(cfg, "AdminToolsGromov\\AdminTools.ini")
						sampAddChatMessage("Вы успешно добавили кнопку \"" .. u8:decode(inputreportaddname.v) .. "\"",
							15352900)
	...
	if AmemberWindow.v then
		...
		imgui.SetNextWindowPos(imgui.ImVec2((sw / 2), sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(u8 "Выберите организацию", AmemberWindow,
	...
	if TeleportWindow.v then
		...
		if imgui.Button(u8 'ТП по метке', imgui.ImVec2(-1, 25)) then
			...
			if not _ then
				sampAddChatMessage(tag .. "Метка для телепорта {E78284}не найдена{84A6E7}.", -1)
			else
	...
	if CaptchaWindow.v then
		...
		imgui.Begin(fa.ICON_FA_COGS .. u8 ' Настройки | Капча', CaptchaWindow,
			imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)
		...
		if when_captcha.v == 2 then
			imgui.CenterText(u8('/vr Внимание! Капча: "' .. u8:decode(cfg.main.kapchas)) ..
				u8('". Приз: ' .. u8:decode(cfg.main.present)) .. '!')
		else
			imgui.CenterText(u8('/ao Внимание! Капча: "' .. u8:decode(cfg.main.kapchas)) ..
				u8('". Приз: ' .. u8:decode(cfg.main.present)) .. '!')
		end
		imgui.EndChild()
		if imgui.Button(u8 'Отправить капчу', imgui.ImVec2(-1, 25)) then
			if when_captcha.v == 2 then
				sampSendChat('/vr Внимание! Капча: "' ..
					u8:decode(cfg.main.kapchas) .. '". Приз: ' .. u8:decode(cfg.main.present) .. '!')
			else
				sampSendChat('/ao Внимание! Капча: "' ..
					u8:decode(cfg.main.kapchas) .. '". Приз: ' .. u8:decode(cfg.main.present) .. '!')
			end
		end
		imgui.End()
	...
	if ColorsWindow.v then
		...
		imgui.Begin(u8 'Таблица цветов', ColorsWindow,
	...
	if TeleportPlayerWindow.v then
		...
		imgui.Begin(fa.ICON_FA_TH_LIST .. u8 ' Телепорт игроков | STools', TeleportPlayerWindow,
			imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoSavedSettings)
		imgui.Text(u8 'Введите ид игрока:')
		imgui.SameLine()
		imgui.PushItemWidth(-1)
		imgui.InputInt(u8 "Введите ид игрока:#tpwindow", buffer104, 0)
		imgui.PopItemWidth()
		local result = sampIsPlayerConnected(buffer104.v)
		if not result then
			imgui.Text(u8 'Игрок под таким ID не в сети!')
		else
			imgui.Text(u8 'Выбранный игрок: ' .. sampGetPlayerNickname(buffer104.v) .. '[' .. buffer104.v .. ']')
		end
		imgui.Separator()
		if imgui.Button(u8 'Телепорт на популярные точки', imgui.ImVec2(530, 20)) then
			imgui.OpenPopup(u8 'Популярные точки')
		end
		if imgui.BeginPopupModal(u8 'Популярные точки', TeleportPlayerWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
			for i = 1, #locations do
				if imgui.Button(u8(locations[i][1]), imgui.ImVec2(530, 20)) then
					sampAddChatMessage(
						tag ..
						'Телепортирую игрока ' ..
						sampGetPlayerNickname(buffer104.v) .. '[' .. buffer104.v .. '] на точку ' .. locations[i][1], -1)
					lua_thread.create(function()
						sampSendChat('/tpcor ' ..
						...
						sampSendChat('/fakesms ' ..
							buffer104.v ..
							' Вы были телепортированы на точку ' ..
							locations[i][1] .. '. Приятной игры by samsa center <3')
						sampAddChatMessage(tag .. 'Телепорт прошел успешно!', -1)
					end)
					imgui.CloseCurrentPopup()
				end
			end
		...
		imgui.Separator()
		imgui.PushItemWidth(-1)
		imgui.CenterTextColoredRGB('{ff5606}Телепорт на дом/бизнес')
		imgui.PopItemWidth()
		imgui.RadioButton(u8 "Дом", when_tp, 1)
		imgui.RadioButton(u8 "Бизнес", when_tp, 2)
		imgui.Text(u8 'Введите ид имущества:')
		imgui.SameLine()
		imgui.PushItemWidth(-1)
		imgui.InputText(u8 "Введите ид имущества:#tpwindow", buffer105)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Телепортировать на имущество', imgui.ImVec2(530, 30)) then
			lua_thread.create(function()
				if when_tp.v == 1 then
				...
				if not _ then
					sampAddChatMessage(tag .. 'Не удалось обнаружить маркер имущества', -1)
				else
					sampAddChatMessage(
						tag .. 'Телепортирую игрока ' .. sampGetPlayerNickname(buffer104.v) .. '[' .. buffer104.v .. ']',
						-1)
					...
					sampSendChat('/fakesms ' ..
						buffer104.v ..
						' Вы были телепортированы на точку ' ..
						test__ .. ' ' .. buffer105.v .. '. Приятной игры by samsa center <3')
					sampAddChatMessage(tag .. 'Телепорт прошел успешно!', -1)
				end
			end)
		end
	...
	if PunishWindow.v and cfg.main.menurecon then
		...
		imgui.Begin(u8("Быстрые наказания"), _,
			imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove +
		...
		for count = 0, #changelog do
			...
			if #patches > 0 then
				...
				imgui.Hint('show_patches:' .. count,
					u8('Кликните, что бы ' .. (changelog[count]['patches']['show'] and 'свернуть' or 'развернуть')))
				if imgui.IsItemClicked() then
	...
	if ReportWindow.v then
		...
		imgui.Begin(u8 'Жалоба/Вопрос', ReportWindow,
			imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove +
		...
		if imgui.Button(fa.ICON_FA_ROBOT .. u8 ' Слежу за наруш', imgui.ImVec2(145, 20)) then
			if report_text:match("(%d+)") ~= nil then
				if sampIsPlayerConnected(report_text:gmatch("(%d+)")) then
					...
					sampAddChatMessage(
						"{808080}[Слежка] Вы не можете начать следить за нарушителем,т.к не указан айди в репорте или игрок оффлайн.",
						-1)
				end
			end
		end
		...
		if imgui.Button(fa.ICON_FA_PEOPLE_CARRY .. u8 ' Помочь автору', imgui.ImVec2(145, 20)) then
			...
			ReportWindow.v = false
			sampSendChat('/re ' .. report_id)
			sampAddChatMessage("[Автоматическая слежка] Вы ушли в слежку за " .. nick_rep .. "[" .. report_id .. "]",
				14628149)
		end
		imgui.SameLine()
		...
		if imgui.BeginPopupModal(u8 'Подтверждение закрытия', ReportWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
			...
			if imgui.Button(u8 ' Да', imgui.ImVec2(btns_sizeX, 20)) then
...
function flood(id)
	if id == nil or id == " " or id == "" then
		...
		if not result then
			sampAddChatMessage(
				tag .. "Игрок не в сети {E78284}& {84A6E7}данный игрок администратор. Возможно Вы ошиблись {E78284}ID'ом",
				-1)
		else
	...
	if id == nil or id == " " or id == "" then
		...
		if not result then
			...
			sampSendChat('/' .. cfg.main.typeoskadm .. ' ' ..
				nick .. ', ' .. cfg.main.oskadm .. ' оскорбление адми[н]истрации')
		end
	end
end
...
function spcarall(arg)
	if tonumber(arg) and tonumber(arg) >= 5 and tonumber(arg) <= 60 then
		lua_thread.create(function()
			sampSendChat('/ao [Спавн Транспорта] Уважаемые игроки, через ' ..
				arg .. ' секунд произойдет респавн всего транспорта.')
...
function tpcssss()
	_, x, y, z = SearchMarker()
	if not _ then
		sampAddChatMessage(tag .. "Маркер для телепорта {E78284}не найден{84A6E7}.", -1)
	else
		setCharCoordinates(PLAYER_PED, x, y, z)
	end
end
...
function ev.onShowDialog(id, dialogStyle, dialogTitle, okButtonText, cancelButtonText, dialogText)
	...
	if cd and id == 9991 and dialogTitle:find('Выбор приза') then
		sampSendDialogResponse(9991, 1, 1, nil)
	end
	if cd and id == 9992 and dialogTitle:find("Выбор приза") then
		sampSendDialogResponse(9992, 1, 0, prize)
		sampCloseCurrentDialogWithButton(0)
		cd = false
	end
	if id == 15316 and arena_type.v == 0 and dialogTitle:find("Выбор приза") then
		sampSendDialogResponse(15316, 1, 0, nil)
	end
	if id == 15316 and arena_type.v == 1 and dialogTitle:find("Выбор приза") then
		sampSendDialogResponse(15316, 1, 1, nil)
	end
	if id == 15316 and arena_type.v == 2 and dialogTitle:find("Выбор приза") then
		sampSendDialogResponse(15316, 1, 2, nil)
	end
	if id == 15316 and arena_type.v == 3 and dialogTitle:find("Выбор приза") then
		sampSendDialogResponse(15316, 1, 3, nil)
	end
	if id == 15317 and dialogTitle:find("Количество") then
...
function ev.onServerMessage(color, text)
	if not isGamePaused() then
		...
		if cfg.main.underb then
			...
			if text:find('%[A%] (.*)%[(%d+)%] использовал код%: (.*) Приз%: (.*) %(Количество%: (%d+)%)') then
				nickname, playerid, code, prize, amount = text:match(
					'%[A%] (.*)%[(%d+)%] использовал код%: (.*) Приз%: (.*) %(Количество%: (%d+)%)')
			end

			if text:find('%[Ошибка%] {......}Внимания, сундук уже стоит, %/settings %> Включить ТикТок сундук , для того чтобы увидеть его!') then
				sampAddChatMessage(tag .. 'TikTok сундук уже запущен.', -1)
				return false
			end

			if text:find("{......}%[TIK%-TOK SUNDUK%] {......}По окончанию таймера, у вас появиться 'Курсор' для открытия") then
				lua_thread.create(function()
					sampAddChatMessage(tag .. 'Кто-то поставил TikTok сундук!', -1)
					wait(140000)
					sampAddChatMessage(tag .. 'Вы снова можете поставить TikTok сундук!', -1)
					if cfg.main.autosunduk then
						...
						arena_people.v = "10"
						sampSendChat('/sunduk')
						sampAddChatMessage(tag .. 'Скрипт автоматически поставил TikTok сундук!', -1)
					end
				end)
			end
	...
	if cfg.main.logging then
		if text:find("Администратор (.*) выпустил игрока (.*), причина: (.*)") and cfg.main.logunjail then
			...
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnjailsLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Администратор ' .. adminnick .. ' выпустил игрока ' .. playernick .. '. Причина: ' .. reason .. '\n')
			f:close()
		end
		...
		if text:find("Администратор (.*) снял заглушку с игрока (.*), причина: (.*)") and cfg.main.logunmute then
			...
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnmutesLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Администратор ' .. adminnick .. ' снял заглушку с игрока ' .. playernick .. '. Причина: ' .. reason ..
				'\n')
			f:close()
		end
		...
		if text:find("Администратор (.*) разбанил игрока (.*), причина: (.*)") and cfg.main.logunban then
			...
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnbanLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Администратор ' .. adminnick .. ' разбанил игрока ' .. playernick .. '. Причина: ' .. reason .. '\n')
			f:close()
		end

		if text:find("%[A%] (.*)%[(%d+)%] разблокировал IP: (.*)") and cfg.main.logunbanip then
			adminnick, adminid, unbanip = text:match("%[A%] (.*)%[(%d+)%] разблокировал IP: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnbanipLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) .. '] Администратор ' .. adminnick .. ' разблокировал IP: ' ..
				unbanip .. '\n')
			f:close()
		end
		...
		if text:find("Администратор (.*) забанил в оффлайне игрока (.*)%. Причина: (.*)") and cfg.main.logbanoff then
			adminnick, playernick, reason = text:match(
				"Администратор (.*) забанил в оффлайне игрока (.*)%. Причина: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//BanoffLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Администратор ' .. adminnick ..
				' заблокировал в оффлайне игрока ' .. playernick .. '. Причина: ' .. reason .. '\n')
			f:close()
		end
		...
		if text:find("Администратор (.*) установил в оффлайне (%d+) минут тюрьмы игроку (.*)%. Причина: (.*)") and cfg.main.logjailoff then
			adminnick, srok, playernick, reason = text:match(
				"Администратор (.*) установил в оффлайне (%d+) минут тюрьмы игроку (.*)%. Причина: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//JailoffLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Администратор ' ..
				adminnick .. ' установил в оффлайне ' ..
				srok .. ' минут тюрьмы игроку ' .. playernick .. '. Причина: ' .. reason .. '\n')
			f:close()
		end

		if text:find("Администратор (.*) установил в оффлайне (%d+) минут молчанки игроку (.*)%. Причина: (.*)") and cfg.main.logmuteoff then
			adminnick, srok, playernick, reason = text:match(
				"Администратор (.*) установил в оффлайне (%d+) минут молчанки игроку (.*)%. Причина: (.*)")
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//MuteoffLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Администратор ' ..
				adminnick ..
				' установил в оффлайне ' .. srok .. ' минут молчанки игроку ' .. playernick .. '. Причина: ' .. reason ..
				'\n')
			f:close()
		end
		...
		if text:find("Администратор (.*) забанил ip (.*)%. Причина: (.*)") and cfg.main.logbanipoff then
			...
			local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//BanipoffLogs.txt", 'a')
			f:write('[' ..
				os.date("%d.%m.%Y %X", os.time()) ..
				'] Администратор ' .. adminnick .. ' забанил IP ' .. banipoffip .. '. Причина: ' .. reason .. '\n')
			f:close()
		end
	end
	if text:find("Приветствуем нового игрока нашего сервера: {FF9900}(.*) {FFFFFF}%(ID: (%d+)%)  {cccccc}IP: (%d+)%.(%d+)%.(%d+)%.(%d+)") then
		regnick, regid, ip1, ip2, ip3, ip4 = text:match(
			"Приветствуем нового игрока нашего сервера: {FF9900}(.*) {FFFFFF}%(ID: (%d+)%)  {cccccc}IP: (%d+)%.(%d+)%.(%d+)%.(%d+)")
		if cfg.main.regChecker then
		...
		if cfg.main.removeip then
			sampAddChatMessage(
				'Приветствуем нового игрока нашего сервера: {FF9900}' ..
	...
	if text:find("Игрок (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] перевел игроку (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] на счет $(.*) %[$(.*)%]") then
		nickplayer, idplayer, ip1, ip2, ip3, ip4, nickget, idget, ip5, ip6, ip7, ip8, summ, allmoney = text:match(
			"Игрок (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] перевел игроку (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] на счет $(.*) %[$(.*)%]")
		if cfg.main.removeip then
			sampAddChatMessage(
				'{BA2B29}Игрок ' ..
				nickplayer ..
				...
				ip4 ..
				'] перевел игроку ' ..
				nickget .. '[' .. idget .. '] [IP: ' .. ip5 .. '.xx.xx.' .. ip8 .. '] на счет $' ..
				summ .. ' [$' .. allmoney .. ']', -1)
			return false
		end
	end
	...
	if text:find("Игрок/админ (.*)%[(%d+)%] зашел %[reg ip:(%d+).(%d+).(%d+).(%d+)%] %-%- %[norm ip: (%d+).(%d+).(%d+).(%d+)%]") then
		nickadm, idadm, ip1, ip2, ip3, ip4, ip5, ip6, ip7, ip8 = text:match(
			"Игрок/админ (.*)%[(%d+)%] зашел %[reg ip:(%d+).(%d+).(%d+).(%d+)%] %-%- %[norm ip: (%d+).(%d+).(%d+).(%d+)%]")
		if cfg.main.removeip then
			sampAddChatMessage(
				'{BA2B29}Лидер/администратор ' ..
				nickadm ..
				'[' .. idadm ..
				'] зашел на сервер. [REG IP: ' ..
	...
	if text:find("Niсk %[(.*)%]  R%-IP %[(.*)%]  IP | A%-IP %[{FF6A78}(.*) | (.*){6ab1ff}%]") then
		nickplayer, regip, lastip, xzip = text:match(
			"Niсk %[(.*)%]  R%-IP %[(.*)%]  IP | A%-IP %[{FF6A78}(.*) | (.*){6ab1ff}%]")
		if cfg.main.regi then
								...
								if style_reg.v == 1 then
									if style_admin.v == 1 then
										if cmd == 'getip' then
											lua_thread.create(function()
												wait(700)
												sampSendChat('/a [Tools] Проверяю IP: ' .. rdata[1]["query"] .. ' | №1')
												wait(700)
												sampSendChat('/a [1 IP] Страна: ' ..
													rdata[1]["country"] ..
													' | Город: ' ..
													rdata[1]["city"] .. ' | Провайдер: ' .. rdata[1]["isp"])
												wait(700)
												sampSendChat('/a [1 IP] Расстояние от регистрационного: ' ..
													distances .. 'км')
												wait(700)
												sampSendChat('/A [Tools] Проверяю IP: ' .. rdata[2]["query"] .. ' | №2')
												wait(700)
												sampSendChat('/a [2 IP] Страна: ' ..
													rdata[2]["country"] ..
													' | Город: ' ..
													rdata[2]["city"] .. ' | Провайдер: ' .. rdata[2]["isp"])
												wait(700)
												sampSendChat('/a [2 IP] Расстояние от регистрационного: ' ..
													distances .. 'км')
											end)
										end
									else
										lua_thread.create(function()
											...
											sampSendChat('/a [1 IP] Страна: ' ..
												rdata[1]["country"] ..
												' | Город: ' .. rdata[1]["city"] .. ' | Провайдер: ' .. rdata[1]["isp"])
											wait(700)
											sampSendChat('/a [1 IP] Расстояние от регистрационного: ' .. distances ..
												'км')
											wait(700)
											sampSendChat('/a [Tools] Проверяю IP: ' .. rdata[2]["query"] .. ' | №2')
											wait(700)
											sampSendChat('/a [2 IP] Страна: ' ..
												rdata[2]["country"] ..
												' | Город: ' .. rdata[2]["city"] .. ' | Провайдер: ' .. rdata[2]["isp"])
											wait(700)
											sampSendChat('/a [2 IP] Расстояние от регистрационного: ' .. distances ..
												'км')
										end)
									end
								else
									if cfg.main.removeip then
										sampAddChatMessage(tag .. 'Проверяю IP: {E78284}скрыто {84A6E7}| {E78284}№' .. i,
											-1)
									else
										sampAddChatMessage(
											tag ..
											'Проверяю IP: {E78284}' .. rdata[i]["query"] .. ' {84A6E7}| {E78284}№' .. i,
											-1)
									end
									sampAddChatMessage(
										tag ..
										...
										' IP] Страна: ' ..
										rdata[i]["country"] ..
										' | Город: ' .. rdata[i]["city"] .. ' | Провайдер: ' .. rdata[i]["isp"], -1)
									sampAddChatMessage(
										tag .. '[' .. i .. ' IP] Расстояние от регистрационного: ' .. distances .. 'км',
										-1)
								end
							end
						end
			...
			if cfg.main.removeip then
				sampAddChatMessage(
					tag ..
					'Игровой никнейм: {E78284}' ..
					nickplayer .. '{84A6E7}. REG-IP: {E78284}скрыто {84A6E7}| LAST-IP: {E78284}скрыто', -1)
				return false
			end
		end
	end
...
sampSendChat('/acceptadmin '..idadmina)
	...
	if text:find("%[Ошибка%]{FFFFFF} Используйте: /tpcor %[id%] %[x%], %[y%], %[z%]") then
		sampAddChatMessage('{E78284}[AdminTools] {FFFFFF}Используйте: /tpc [id] [x] [y] [z]', -1)
		return false
	end
					...
					if cmd == "mute" and cfg.formssettings.autoforma_mute or cmd == "rmute" and cfg.formssettings.autoforma_rmute or cmd == "slap" and cfg.formssettings.autoforma_slap or cmd == "jail" and cfg.formssettings.autoforma_jail or cmd == "kick" and cfg.formssettings.autoforma_kick
						...
						lua_thread.create(function()
							sampAddChatMessage(
								...
								"} принята автоматически!", "0xFF" .. cfg.main.colorforms)
							...
							f:write('[' ..
								...
								'] Форма: /' ..
								cmd ..
								' ' .. paramssss .. ' | Отправитель: ' ..
								admin_nick .. ' | Тип принятия формы: автоматически\n')
							f:close()
							cfg.main.formsday = cfg.main.formsday + 1
							cfg.main.formsvse = cfg.main.formsvse + 1
							sampSendChat("/" .. cmd .. " " .. paramssss)
						...
						if cmd == "mute" and cfg.formssettings.forma_mute or cmd == "jail" and cfg.formssettings.forma_jail or cmd == "rmute" and cfg.formssettings.forma_rmute or cmd == "unrmute" and cfg.formssettings.forma_unrmute
							...
							if not isGamePaused() then
								sampAddChatMessage(
									...
									" {" ..
									cfg.main.twocolorforms ..
									"}<< Отправитель: {" .. cfg.main.colorforms .. "}" ..
									"[" ..
									admin_tag ..
								...
								sampAddChatMessage(
									...
									"}Нажмите клавишу >> {" ..
									cfg.main.colorforms ..
									"}" .. klavisha .. " {" .. cfg.main.twocolorforms .. "}<< чтобы принять форму!",
									"0xFF" .. cfg.main.colorforms)
								lua_thread.create(function()
									for i = 0, 5 do
									...
									if prikoll == "true" then
										sampAddChatMessage(
											...
											cfg.main.colorforms ..
											"}/" .. cmd .. " " .. paramssss ..
											" {" .. cfg.main.twocolorforms .. "}<< не принята!", "0xFF" ..
											cfg.main.colorforms)
	...
	if not isGamePaused() then
		...
		if text:find("%[Жалоба%] от (.*)%[(%d+)%]: {ffffff}(.*)%. Уже {BE2D2D}(%d+){FFFFFF} жалоб!!!") then
			nickp, idp, sute, kolvo = text:match(
				"%[Жалоба%] от (.*)%[(%d+)%]: {ffffff}(.*)%. Уже {BE2D2D}(%d+){FFFFFF} жалоб!!!")
			sampAddChatMessage(
				'[Репорт] от ' ..
				nickp ..
				'[' ..
				...
				'}' .. sute .. '. Уже {' .. cfg.main.colorreport .. '}' .. kolvo ..
				' {' .. cfg.main.twocolorreport .. '}репортов!', '0xFF' .. cfg.main.colorreport)
		...
		if text:find("(.*) %[(.*)%] купил бизнес ID: (.*) по гос. цене за (.*) ms! Капча: %((%d+) | (%d+)%)") and cfg.autoopra.settingbiz then
			bnickp, bidp, bidbiz, bvremya, bcaptcha1, bcaptcha2 = text:match(
				"(.*) %[(.*)%] купил бизнес ID: (.*) по гос. цене за (.*) ms! Капча: %((%d+) | (%d+)%)")
			nick_cutch = bnickp
			idp_cutch = bidp
			id_cutch = bidbiz
			time_cutch = bvremya
			...
			if not cfg.autoopra.status then
				...
				sampAddChatMessage(
					tag ..
					'Используйте команду {E78284}/' ..
					cmdBusiness.v .. ' {84A6E7}для запроса опровержения на бизнес {E78284}№' .. bidbiz, -1)
			end
			local f = io.open(getGameDirectory() .. "\\moonloader\\AdminToolsGromov\\Логи\\AutoOpraLogs.txt", 'a')
			f:write(string.format(
				'%s | Игрок ' ..
				bnickp .. ' купил бизнес ID: ' .. bidbiz .. ' по гос. цене за ' .. bvremya .. ' ms! Капча: ' ..
		...
		local admin, victim, tj = text:match(
			"Администратор ([a-zA-Z_]+).* посадил игрока ([a-zA-Z_]+).* в деморган на %d+ минут")
		...
		local adminoff, tjoff, victimoff = text:match(
			"Администратор ([A-z_]+) установил в оффлайне (%d+) минут деморгана игроку ([0-9A-z_]+)%.")
		for k, box in pairs(tLog) do
		...
		local adminunjoff, victimunjoff = text:match(
			"Администратор ([a-zA-Z_]+) выпустил в оффлайне с деморгана ([a-zA-Z_]+)")
		...
		if text:find("(.*) %[(.*)%] купил дом ID: (.*) по гос. цене за (.*) ms! Капча: %((%d+) | (%d+)%)") and cfg.autoopra.settinghouse then
			nickp, idp, idhouse, vremya, captcha1, captcha2 = text:match(
				"(.*) %[(.*)%] купил дом ID: (.*) по гос. цене за (.*) ms! Капча: %((%d+) | (%d+)%)")
			nick_cutch = nickp
			idp_cutch = idp
			id_cutch = idhouse
			time_cutch = vremya
			...
			if not cfg.autoopra.status then
				...
				sampAddChatMessage(
					tag ..
					'Используйте команду {E78284}/' ..
					cmdHouse.v .. ' {84A6E7}для запроса опровержения на дом {E78284}№' .. idhouse, -1)
			end
			local f = io.open(getGameDirectory() .. "\\moonloader\\AdminToolsGromov\\Логи\\AutoOpraLogs.txt", 'a')
			f:write(string.format(
				'%s | Игрок ' ..
				nickp ..
				' купил дом ID: ' .. idhouse .. ' по гос. цене за ' .. vremya .. ' ms! Капча: ' .. captcha1 .. '\n',
				os.date('%d.%m %H:%M', os.time())))
			f:close()
		...
		if text:find("%[MakeAdmin%] .*%[%d+%] установил (%d+) %[.*%] уровень администратора игроку (.*) %(был.*") then
			lvladmin, nickchela = text:match(
				"%[MakeAdmin%] .*%[%d+%] установил (%d+) %[.*%] уровень администратора игроку (.*) %(был.*")
		...
		if text:find("Администратор " .. mynick .. "%[(%d+)%] ответил игроку (.*)%[(%d+)%]:{ffffff} (.*)") and cfg.main.inforeport then
			local _, myid = sampGetPlayerIdByCharHandle(playerPed)
			mynick = sampGetPlayerNickname(myid)
			myidplayer, nickplayer, idplayer, textplayer = text:match("Администратор " ..
				mynick .. "%[(%d+)%] ответил игроку (.*)%[(%d+)%]:{ffffff} (.*)")
		...
		if text:find("Администратор (.*)%[(%d+)%] забанил игрока (.*)%[(%d+)%] на (%d+) дней%. Причина: (.*)") then
			nickadmin, idadmin, nickp, idp, days, reason = text:match(
				"Администратор (.*)%[(%d+)%] забанил игрока (.*)%[(%d+)%] на (%d+) дней%. Причина: (.*)")
		end
	...
	if text:find("Администратор (.*)%[(%d+)%] ответил игроку (.*)%[(%d+)%]:{ffffff} (.*)") then
		nickadmina, idadmina, nickplayer, idplayer, textadmin = text:match(
			"Администратор (.*)%[(%d+)%] ответил игроку (.*)%[(%d+)%]:{ffffff} (.*)")
		sampAddChatMessage(
			'[A] ' .. nickadmina .. '[' .. idadmina ..
			'] ответил игроку ' .. nickplayer .. '[' .. idplayer .. ']: {ffffff}' .. textadmin, 0xFFFAAC58)
		return false
	end
	...
	if not isGamePaused() then
		...
		if text:find("%[A%] Вы успешно авторизовались как {......}(.*)") then
			...
			if cfg.main.mylvladmin == 8 then
				sampAddChatMessage(
					'%[A%] Вы успешно авторизовались как {fe4e4e}Начальник [LVL: ' ..
...
function kotofeevstatsadm()
	...
	if imgui.RadioButton(u8 'Отображать онлайн (текущая сессия)', Radio['sesOnline']) then
		Radio['sesOnline'] = not Radio['sesOnline']; cfg.statTimers.sesOnline = Radio['sesOnline']
	end
	if imgui.RadioButton(u8 'Отображать онлайн за день', Radio['dayOnline']) then
		Radio['dayOnline'] = not Radio['dayOnline']; cfg.statTimers.dayOnline = Radio['dayOnline']
	end
	if imgui.RadioButton(u8 'Отображать онлайн в афк', Radio['dayAfk']) then
		Radio['dayAfk'] = not Radio['dayAfk']; cfg.statTimers.dayAfk = Radio['dayAfk']
	end
	if imgui.RadioButton(u8 'Отображать онлайн за неделю (без учета афк)', Radio['weekOnline']) then
		Radio['weekOnline'] = not Radio['weekOnline']; cfg.statTimers.weekOnline = Radio['weekOnline']
	end
	if imgui.RadioButton(u8 'Отображать онлайн афк за неделю', Radio['weekAfk']) then
		Radio['weekAfk'] = not Radio['weekAfk']; cfg.statTimers.weekAfk = Radio['weekAfk']
	end
	if imgui.RadioButton(u8 'Отображать общий онлайн за неделю (с учетом афк)', Radio['weekFull']) then
		Radio['weekFull'] = not Radio['weekFull']; cfg.statTimers.weekFull = Radio['weekFull']
	end
	if imgui.RadioButton(u8 'Отображать выданные наказания за день', Radio['nakazaniyaDay']) then
		Radio['nakazaniyaDay'] = not Radio['nakazaniyaDay']; cfg.statTimers.nakazaniyaDay = Radio['nakazaniyaDay']
	end
	if imgui.RadioButton(u8 'Отображать выданные наказания за все время', Radio['nakazaniyaVse']) then
		Radio['nakazaniyaVse'] = not Radio['nakazaniyaVse']; cfg.statTimers.nakazaniyaVse = Radio['nakazaniyaVse']
	end
	if imgui.RadioButton(u8 'Отображать выданные форм за день', Radio['formsDay']) then
		Radio['formsDay'] = not Radio['formsDay']; cfg.statTimers.formsDay = Radio['formsDay']
	end
	if imgui.RadioButton(u8 'Отображать выданные форм за все время', Radio['formsVse']) then
		Radio['formsVse'] = not Radio['formsVse']; cfg.statTimers.formsVse = Radio['formsVse']
	end
	if imgui.RadioButton(u8 'Отображать репортов отвечено за день', Radio['reportsDay']) then
		Radio['reportsDay'] = not Radio['reportsDay']; cfg.statTimers.reportsDay = Radio['reportsDay']
	end
	if imgui.RadioButton(u8 'Отображать репортов отвечено за все время', Radio['reportsVse']) then
		Radio['reportsVse'] = not Radio['reportsVse']; cfg.statTimers.reportsVse = Radio['reportsVse']
	end
	if imgui.RadioButton(u8 'Отображать кол-во репортов на данный момент', Radio['reportnow']) then
		Radio['reportnow'] = not Radio['reportnow']; cfg.statTimers.reportnow = Radio['reportnow']
	end
	if imgui.RadioButton(u8 'Отображать текущее время', Radio['nowTime']) then
...
function kotofeevautoopra()
	...
	elseif logBut["selected"] == 5 then
		...
		imgui.Text(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
		imgui.Hint('bizopra',
			u8 'При вводе команды /autoopra будет включена авто-опра на бизнесы (т.е. если человек поймал бизнес - его посадит в деморган)')
		...
		imgui.Hint('houseopra',
			u8 'При вводе команды /autoopra будет включена авто-опра на дома (т.е. если человек поймал дом - его посадит в деморган)')
	...
	elseif logBut["selected"] == 2 then
		...
		for k, box in pairs(tLog) do
			if box[1] == 1 then
				...
				imgui.SameLine(30)
				imgui.Text(string.format(u8 "| Игрок %s купил дом ID: %s по гос. цене за %s! Капча: %s", box[4][1],
			...
			if box[1] == 3 then
				...
				imgui.SameLine(30)
				imgui.Text(string.format(u8 "| Игрок %s купил бизнес ID: %s по гос. цене за %s! Капча: %s", box[4][1],
					box[4][2], box[4][3], box[4][4]))
				imgui.Hint("plInt:" .. k, u8(os.date("%H:%M:%S", box[2]) .. "\nПКМ - Взаимодействовать"))
				if imgui.IsItemClicked(1) then
					contK = k; imgui.OpenPopup("##toolContext")
		...
		if contK ~= nil and imgui.BeginPopupContextItem("##toolContext") then
			...
			if not tLog[contK][5][1] then
				if imgui.Button(fa.ICON_FA_BED .. u8(" Запросить опру" .. (plOnline and "" or " в оффлайне")), imgui.ImVec2(220, 30)) then
					...
					sampSendChat(string.format("/jail%s %s %s опра [№%s | %s]", (plOnline and "" or "off"),
				...
				if imgui.Button(fa.ICON_FA_CHECK .. u8(" Одобрить"), imgui.ImVec2(107.5, 30)) then
					sampSendChat(string.format("/unjail%s %s опра принята", (plOnline and "" or "off"), tLog[contK][4]
						[1]))
					imgui.CloseCurrentPopup()
				end
				imgui.SameLine()
				if imgui.Button(fa.ICON_FA_TIMES .. u8(" Отклонить"), imgui.ImVec2(107.5, 30)) then
					sampSendChat(string.format("/unjail%s %s опра отказана", (plOnline and "" or "off"),
						tLog[contK][4][1]))
	...
	elseif logBut["selected"] == 4 then
		...
		imgui.PopFont()
		imgui.Hint('helplist',
			u8 'Дополнительное оповещение в чат в случае если игрок, поймавший\nимущество, находился в белом списке')
		...
		imgui.Hint('helplis1t',
			u8 'Скрипт будет удалять серверное сообщение о покупке имущества игроком из белого списка')
...
function spawnset()
	...
	sampAddChatMessage(
		'{E78284}[AdminTools] {84A6E7}Вы установили новую точку спавна {E78284}[' ..
...
function rek_off()
	...
	sampAddChatMessage(
		tag ..
		'Отображение сообщений о подозрении на рекламу ' ..
		(cfg.main.deletereklama and '{ff4040}выключено{84A6E7}.' or '{00ff00}включено{84A6E7}.'), -1)
end
...
function SettingsTraicer()
	...
	imgui.SameLine()
	imgui.Text(u8 "Укажите цвет никнейма стрелка")
...
function ajailThread()
	while true do
		...
		if isGamePaused() and cfg.autoopra.status then
			...
			cfg.autoopra.status = false
			sampShowDialog(5454, tag .. 'Предупреждение',
				'{488cff}Автоматический режим был выключен из-за сворачивания игры', 'Понял', _, 0)
		end
	end
end
...
function regcommands()
	...
	sampRegisterChatCommand('autoopra', function()
		...
		sampAddChatMessage(tag ..
			'Автоопровержение ' .. (cfg.autoopra.status and '{E78284}включено' or '{E78284}выключено'), -1)
	end)
...
function cccc()
	if cfg.main.playersChecker then
		...
		renderFontDrawTextAlign(fonts.playersChecker, "Игроки онлайн: ", cfg.checkerPosition.x, cfg.checkerPosition.y, -1,
			cfg.main.align)
		for k, v in ipairs(J_.PLAYERS_CHECKER[1]) do
...
function loadbinders()
	...
	if not doesDirectoryExist(getGameDirectory() .. '//moonloader//AdminToolsGromov//Логи//') then
		createDirectory(getGameDirectory() .. '//moonloader//AdminToolsGromov//Логи//')
	...
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//AdminVipChatLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//AdminVipChatLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи сообщений от администраторов в VIP чате создан.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnjailsLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnjailsLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи выпусков из деморгана создан.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnmutesLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnmutesLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи снятий мутов создан.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnbanLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnbanLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи разблокировок создан.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnbanipLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//UnbanipLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи разблокировок IP создан.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//BanoffLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//BanoffLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи блокировок в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//JailoffLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//JailoffLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи выдачи деморганов в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//MuteoffLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//MuteoffLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи выдачи мутов в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//BanipoffLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//BanipoffLogs.txt", 'w')
		if not f then return False end
		f:close()
		sampAddChatMessage(tag .. 'Файл для записи выдачи блокировок IP в оффлайне создан.', -1)
	end

	if not doesFileExist(getWorkingDirectory() .. "\\AdminToolsGromov\\Логи\\AutoOpraLogs.txt") then
		local f = io.open(getGameDirectory() .. "//moonloader//AdminToolsGromov//Логи//AutoOpraLogs.txt", 'w')
		if not f then return False end
		f:close()
		...
		for _, v in pairs(peds) do
			...
			if result and id ~= sampGetPlayerIdByCharHandle(PLAYER_PED) and distance < tonumber(arg) then
				if id ~= myid then
					...
					if lvl < 15 then
						sampAddChatMessage(
							tag .. 'Никнейм: {fe4e4e}' .. nick ..
							' [' .. id .. '] {84A6E7}| Уровень: {fe4e4e}' .. lvl .. ' [ПУСТЫШКА]', -1)
					else
						sampAddChatMessage(
							tag .. 'Никнейм: {E78284}' .. nick .. ' [' .. id .. '] {84A6E7}| Уровень: {E78284}' .. lvl,
							-1)
					end
				end
			end
...
function connectwithplayer()
	if check_input_available() and isKeysZazhat(fvzaim.v) then
		...
		renderFontDrawTextCenter(fonts1.circle, "Режим: " ..
			(cfg.main.targetState == "player" and "Игроки" or "Транспорт"), cx, windowCoordinates[2] - 40, 0xFF84A6E7)
		renderFontDrawTextCenter(fonts1.circle,
			"Размер: " ..
			(cfg.main.circleRadius < maxRad and cfg.main.circleRadius > 30 and cfg.main.circleRadius or (cfg.main.circleRadius <= 30 and "Минимум" or (cfg.main.circleRadius >= maxRad and "Максимум"))),
			windowCoordinates[1] / 2, windowCoordinates[2] - 29, 0xFF84A6E7)
		renderFontDrawTextCenter(fonts1.circle, "Изменение радиуса: колёсико мыши", cx, windowCoordinates[2] - 18,
			0xFF84A6E7)
			...
			if doesCharExist(nearPed) then
				...
				if isCharOnScreen(nearPed) then
					...
					local dist = math.floor(getDistanceBetweenCoords3d(plCoordinates[1], plCoordinates[2],
						plCoordinates[3], myCoordinates[1], myCoordinates[2], myCoordinates[3])) .. ' метров'
					renderFontDrawTextCenter(fonts1.circle, dist, cx, cy + cfg.main.circleRadius + 10, ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '1 - Телепортироваться', cx + cfg.main.circleRadius + 10, cy - 30,
						ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '2 - Телепортировать к себе', cx + cfg.main.circleRadius + 10,
						cy - 15, ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '3 - Уйти в рекон за игроком', cx + cfg.main.circleRadius + 10, cy,
						ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '4 - Выдать 100 HP', cx + cfg.main.circleRadius + 10, cy + 15,
						ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '5 - Заспавнить игрока', cx + cfg.main.circleRadius + 10, cy + 30,
						ACTIVE_COLOR)
					renderFontDrawText(fonts1.circle, '6 - Открыть меню взаимодействия с игроком',
					...
					if isKeyJustPressed(VK_2) then
						if cfg.main.mylvladmin >= 2 then
							...
							sampAddChatMessage(tag .. 'Ваш админ-уровень слишком низок для выполнения данного действия.',
								-1)
						end
					end
				...
				for i = 1, #rdata do
					if rdata[i]["status"] == "success" then
						...
						if cfg.main.removeip then
							...
							sampAddChatMessage(
								tag .. 'Проверяю IP: {E78284}' .. rdata[i]["query"] .. ' {84A6E7}| {E78284}№' .. i, -1)
						end
						sampAddChatMessage(
							tag ..
							'[' ..
							...
							' IP] Страна: ' ..
							rdata[i]["country"] ..
							' | Город: ' .. rdata[i]["city"] .. ' | Провайдер: ' .. rdata[i]["isp"],
							-1)
						sampAddChatMessage(tag .. '[' .. i .. ' IP] Расстояние от регистрационного: ' .. distances ..
							'км', -1)
					end
				end
				if text == "" then
				...
				if sampGetPlayerColor(killedId) < 368966908 or sampGetPlayerColor(killedId) > 368966908 then
					...
					if not isGamePaused() then
						sampAddChatMessage(
							...
							' [' ..
							killerId ..
							'] {84A6E7}убил своего напарника по организации {' ..
...
function reconusers()
	...
	imgui.Begin(fa.ICON_FA_EYE .. u8 ' Слежка', reconusers_state,
		imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove +
		imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
	imgui.ShowCursor = true
	...
	if tonumber(arg) then
		...
		if result then
			...
			lua_thread.create(function()
				...
				sampAddChatMessage(
					("[AdminTools] Вы телепортировали игрока " .. name .. " на ближайшую от Вас дорогу (%dm.)"):format(
						dist),
					0xAAFFAA)
				wait(500)
				sampSendChat('/gethere ' .. arg)
				wait(500)
				sampSendChat('/a [AdminTools] Телепортировал игрока ' .. name .. ' на координаты ' .. x .. ' ' ..
					y .. ' ' .. z)
			end)
		else
...
--[[function checkKey()
        ...
        if not response.text:match("<body>(.*)</body>"):find("podpiska otsutstvuet") then
            if not response.text:match("<body>(.*)</body>"):find("podpiska istekla") then
                sampAddChatMessage(tag..'Вы успешно авторизовались в скрипте!', -1)
                sampAddChatMessage(tag..'Подписка на текущий сервер активна до'..response.text:match("<body>(.*)</body>"), -1)
				sampAddChatMessage('', -1)
				sampAddChatMessage(tag..'/amenu & '..klavisha..' {E78284}- меню настроек скрипта.', -1)
				sampAddChatMessage(tag..'/helpa {E78284}- команды скрипта.', -1)
				sampfuncsLog(pinktag..'Скрипт загружен, подписка активна до '..response.text:match("<body>(.*)</body>"))
            else
                sampAddChatMessage(pinktag..'У вас отсутствует подписка на AdminTools, скрипт выключается.', -1)
				sampAddChatMessage(pinktag..'Ваш уникальный ключ: '..getserial(), -1)
				sampfuncsLog(pinktag..'Ваш уникальный ключ: '..getserial())
				sampAddChatMessage(pinktag..'Для получения/покупки доступа к использованию скрипта обратитесь к — vk.com/gromovboss.', -1)
				sampfuncsLog(pinktag..'Скрипт отключен, подписка неактивна.')
				sampShowDialog(6666, '{E78284}Платная подписка AdminTools', '{84A6E7}Чтобы купить подписку на скрипт обратитесь к — {E78284}vk.com/gromovboss\n\n{84A6E7}Почему не стоит использовать обходы?\n— вы не будете получать регулярные обновления;\n— скрипт будет работать нестабильно, будут возникать ошибки;\n— каждый раз нужно что-то заменять руками/перезагружать;\n— вам спокойно могут подгрузить вирус, после этого — взломать;\n— лучше не использовать скрипт вовсе, чем потом страдать от потерянного времени;\n\n{E78284}Подписка на скрипт откроет вам функционал ко всему скрипту!', 'Закрыть', _, 0)
				thisScript():unload()
            end
        else
            sampAddChatMessage(pinktag..'У вас отсутствует подписка на AdminTools, скрипт выключается.', -1)
		...
		if response.status_code == 200 then
			downloadUrlToFile(decodeJson(response.text)['url'], thisScript().path, function(id, status, p1, p2)
				if status == dlstatus.STATUSEX_ENDDOWNLOAD then
					sampAddChatMessage(tag .. 'Скрипт обновлен, перезагрузка...', -1)
					thisScript():reload()
				end
			end)
		else
			sampAddChatMessage(tag .. 'Ошибка, невозможно установить обновление, код: ' .. response.status_code, -1)
		end
	end
...
function ArenaHelper()
	if imgui.Button(u8 'Запуск раздачи cchas', imgui.ImVec2(330, 25)) then
		if not start_cchas then
			imgui.OpenPopup(u8 'Выберите конфигурацию##cchas')
		else
			sampAddChatMessage(tag .. 'Раздача cchas уже запущена.')
		end
	end
	if imgui.BeginPopupModal(u8 'Выберите конфигурацию##cchas', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		imgui.PushItemWidth(100)
		imgui.InputInt(u8 'Время работы в секундах', ctime)
		imgui.PopItemWidth()
		imgui.PushItemWidth(100)
		imgui.InputInt(u8 'Количество раздачи', actime)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Запустить cchas', imgui.ImVec2(-1, 30)) then
			cchas(actime.v)
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Отменить', imgui.ImVec2(-1, 30)) then
			imgui.CloseCurrentPopup()
		end
		imgui.EndPopup()
	end
	imgui.SameLine()
	if imgui.Button(u8 'Запуск TikTok сундука', imgui.ImVec2(330, 25)) then
		if not sd then
			imgui.OpenPopup(u8 'Выберите конфигурацию##sunduk')
		else
			sampAddChatMessage(tag .. 'TikTok сундук уже запущен.', -1)
		end
	end
	if imgui.BeginPopupModal(u8 'Выберите конфигурацию##sunduk', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		...
		imgui.PushItemWidth(100)
		imgui.Combo(u8 "Тип приза", arena_type, view, #view)
		imgui.InputText(u8 "Количество приза", arena_amount)
		imgui.InputText(u8 'Количество людей', arena_people)
		imgui.InputText(u8 'Время действия сундука', arena_time)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Запустить TikTok сундук', imgui.ImVec2(-1, 30)) then
			sampSendChat('/sunduk')
			imgui.CloseCurrentPopup()
		end
		if imgui.Button(u8 'Отменить', imgui.ImVec2(-1, 30)) then
	...
	if imgui.Button(u8 "Создать рандомный код", imgui.ImVec2(330, 25)) then
		generationCode()
	end
	imgui.SameLine()
	if imgui.Button(u8 "Рандомный код для адм", imgui.ImVec2(330, 25)) then
		generationMpAdm()
	end
	if imgui.Button(u8 "Код players", imgui.ImVec2(330, 25)) then
		addcodeplayer()
	end
	imgui.SameLine()
	if imgui.Button(u8 "Код admins", imgui.ImVec2(330, 25)) then
		addcodeadmins()
	end
	imgui.Separator()
	if imgui.Button(u8 "Выдать биткоины [RANDOM]", imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Выберите конфигурацию##givebtc')
	end
	if imgui.BeginPopupModal(u8 'Выберите конфигурацию##givebtc', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_numbtc.v == "" or arena_numbtc.v == nil) then arena_numbtc.v = "10" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Количество", arena_numbtc)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Выдать BTC', imgui.ImVec2(-1, 30)) then
	...
	imgui.SameLine()
	if imgui.Button(u8 "Выдать сток катану [RANDOM]", imgui.ImVec2(330, 25)) then
		DistributionArena(2)
	end
	if imgui.Button(u8 "Выдать краситель [RANDOM]", imgui.ImVec2(330, 25)) then
		DistributionArena(4)
	end
	imgui.SameLine()
	if imgui.Button(u8 "Выдать нашивку [RANDOM]", imgui.ImVec2(330, 25)) then
		DistributionArena(5)
	end
	if imgui.Button(u8 "Выдать AZ Coins [RANDOM]", imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Выберите конфигурацию##giveaz')
	end
	if imgui.BeginPopupModal(u8 'Выберите конфигурацию##giveaz', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_numcoins.v == "" or arena_numcoins.v == nil) then arena_numcoins.v = "10" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Количество", arena_numcoins)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Выдать AZ-Коины', imgui.ImVec2(-1, 30)) then
	...
	imgui.SameLine()
	if imgui.Button(u8 "Выдать AZ-Rub [RANDOM]", imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Выберите конфигурацию##giverub')
	end
	if imgui.BeginPopupModal(u8 'Выберите конфигурацию##giverub', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_numrub.v == "" or arena_numrub.v == nil) then arena_numrub.v = "10" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Количество", arena_numrub)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Выдать рубли', imgui.ImVec2(-1, 30)) then
	...
	if imgui.Button(u8 "Выдать часы [RANDOM]", imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Выберите конфигурацию##givechas')
	end
	if imgui.BeginPopupModal(u8 'Выберите конфигурацию##givechas', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_numchas.v == "" or arena_numchas.v == nil) then arena_numchas.v = "10" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Количество", arena_numchas)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Выдать часы', imgui.ImVec2(-1, 30)) then
	...
	imgui.SameLine()
	if imgui.Button(u8 'Выдать предмет [RANDOM]', imgui.ImVec2(330, 25)) then
		imgui.OpenPopup(u8 'Выберите конфигурацию##randacs')
	end
	if imgui.BeginPopupModal(u8 'Выберите конфигурацию##randacs', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar) then
		if (arena_nameacs.v == "" or arena_nameacs.v == nil) then arena_nameacs.v = "Катана" end
		if (arena_acs.v == "" or arena_acs.v == nil) then arena_acs.v = "789" end

		imgui.PushItemWidth(300)
		imgui.InputText(u8 "Название", arena_nameacs)
		imgui.InputText(u8 "ID", arena_acs)
		imgui.PopItemWidth()
		if imgui.Button(u8 'Выдать предмет', imgui.ImVec2(-1, 30)) then
	...
	imgui.Separator()
	if imgui.ToggleButton(u8 'Авто-сундук', imgui.ImBool(cfg.main.autosunduk)) then
	...
	imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE .. u8 '')
	imgui.PopFont()
	imgui.Hint('helpas', u8 'Скрипт автоматически будет ставить тик-ток сундук (для /astats)')
end
...
function giverndacs(id, acs, acsname)
	...
	if not result then
		...
		lua_thread.create(function()
			...
			sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит предмет '%s'", acsname))
			wait(5000)
			sampSendChat(string.format("/giveitem %d %d 1", id, acs))
			wait(700)

			if result then
				sampSendChat(string.format("/ao Игрок %s[%d] получил предмет '%s'", sampGetPlayerNickname(id), id,
					acsname))
			end
		end)
	end
...
function cchas(amount)
	lua_thread.create(function()
		for i = 1, amount do
			...
			start_cchas = true
			wait(500)
			sampSendChat("/ao /cchas x" .. i .. " [" .. u8:decode(ctime.v) .. " секунд]")
			wait((ctime.v) * 1000)
			sampSendChat("/givechass")
			start_cchas = false
		end
...
function addcodeplayer()
	...
	lua_thread.create(function()
		...
		wait(2000)
		sampSendChat("/ao Создал код players [Активаций: " .. act .. "] с призом " .. prize .. " AZ-RUB")
		wait(20000)
...
function addcodeadmins()
	...
	lua_thread.create(function()
		...
		wait(2000)
		sampSendChat(string.format("/a Создал код admins [Активаций: %d] с призом %d AZ-RUB", act, prize))
		wait(20000)
...
function GiveRoublesRandom(num)
	...
	lua_thread.create(function()
		sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит %d AZ-Rub", num))
		wait(5000)
		sampSendChat(string.format("/giverub %d %d", playerID, num))
		wait(700)
		if result then
			sampSendChat(string.format("/ao Игрок %s[%d] получил %d AZ-Rub", sampGetPlayerNickname(playerID), playerID,
				num))
		end
	end)
end
...
function GiveChasRandom(num)
	...
	lua_thread.create(function()
		sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит %d час(а, ов)", num))
		wait(5000)
		sampSendChat(string.format("/givechas %d %d", playerID, num))
		wait(700)
		if result then
			sampSendChat(string.format("/ao Игрок %s[%d] получил %d час(а, ов)", sampGetPlayerNickname(playerID),
				playerID, num))
		end
	end)
end
...
function GiveAzCoinsRandom(num)
	...
	lua_thread.create(function()
		sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит %d AZ-Coins", num))
		wait(5000)
		sampSendChat(string.format("/giveaz %d %d", playerID, num))
		wait(700)
		if result then
			sampSendChat(string.format("/ao Игрок %s[%d] получил %d AZ-Coins", sampGetPlayerNickname(playerID), playerID,
				num))
		end
	end)
end
...
function GiveBtcRandom(num)
	...
	lua_thread.create(function()
		sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит %d BTC", num))
		wait(5000)
		sampSendChat(string.format("/giveitem %d 1383 %d", playerID, num))
		wait(700)
		if result then
			sampSendChat(string.format("/ao Игрок %s[%d] получил %d BTC", sampGetPlayerNickname(playerID), playerID, num))
		end
	end)
end
...
function DistributionArena(type)
	...
	if type == 1 then
		lua_thread.create(function()
			...
			sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит %s BTC.", prize))
			wait(5000)
			sampSendChat(string.format("/giveitem %d 1383 %s", playerID, prize))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Игрок %s[%d] получил %d BTC.", sampGetPlayerNickname(playerID), playerID,
					prize))
			end
		end)
	elseif type == 2 then
		lua_thread.create(function()
			...
			sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит предмет 'Катана'", prize))
			wait(5000)
			sampSendChat(string.format("/giveitem %d 789 1", playerID))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Игрок %s[%d] получил предмет 'Катана'", sampGetPlayerNickname(playerID),
					playerID))
			end
		end)
	elseif type == 3 then
		lua_thread.create(function()
			...
			sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит %s Az-Coins", prize))
			wait(5000)
			sampSendChat(string.format("/giveaz %d %s", playerID, prize))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Игрок %s[%d] получил %s Az-Coins", sampGetPlayerNickname(playerID),
					playerID, prize))
			end
		end)
	elseif type == 4 then
		lua_thread.create(function()
			local az, cd = getRandomWord(100), true
			sampSendChat("/ao Через 5 секунд рандомный ид получит краситель")
			wait(5000)
			sampSendChat(string.format("/giveitem %d 2081 1", playerID))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Игрок %s[%d] получил краситель", sampGetPlayerNickname(playerID),
					playerID))
			end
		end)
	elseif type == 5 then
		lua_thread.create(function()
			local az, cd = getRandomWord(100), true
			sampSendChat("/ao Через 5 секунд рандомный ид получит Супер Мега Нашивку")
			wait(5000)
			sampSendChat(string.format("/giveitem %d 2083 1", playerID))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Игрок %s[%d] получил Супер Мега Нашивку", sampGetPlayerNickname(playerID),
					playerID))
			end
		end)
	elseif type == 6 then
		lua_thread.create(function()
			local az, cd = getRandomWord(100), true
			prize = random(15, 30)
			sampSendChat(string.format("/ao Через 5 секунд рандомный ид получит %d AZ-Rub", prize))
			wait(5000)
			sampSendChat(string.format("/giverub %d %s", playerID, prize))
			wait(700)
			if result then
				sampSendChat(string.format("/ao Игрок %s[%d] получил %d AZ-Rub", sampGetPlayerNickname(playerID),
					playerID, prize))
			end
		end)
	end
